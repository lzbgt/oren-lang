...
- Keep implementing artifact policy manifests and package capability manifests. `@oren.package(...)`
  ...
  available, and enforces gas budgets from captured `native_safepoint_tick_v0` evidence.
  ...
  safepoint-granular native gas. AVM run JSON reports gas, heap, and wall package budgets through
...
- `scripts/run_backend_semantic_diff.sh` is now the first small semantic-diff consumer: it emits
  ...
  work is finer native gas parity, not only expanding fixture scripts.
