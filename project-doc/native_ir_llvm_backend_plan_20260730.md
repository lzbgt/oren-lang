...
48. Done: add runtime-varying bytes copy coverage without expanding the runtime
    ...
    contract.
49. Done: add the first map descriptor ABI slice. Empty hash literals lower to
    map descriptors, descriptor-backed `index_set`/`index_get` mutate/read exact
    i64 keys, and proven `oren_map_len` calls lower to a generated helper instead
    of the generic dispatcher. The real C runtime registers map descriptors plus
    key/value storage, roots proven map locals across safepoints, and the focused
    map gate proves linked execution parity plus forced GC-at-safepoint.
50. Done: broaden map key semantics beyond exact descriptor identity for string
    keys. `%oren_llvm_map { len, keys, values, key_kinds, owner_kind, capacity }`
    now carries a key-kind sidecar, so generated string-key find/get/set helpers
    can safely compare descriptor-backed strings by length plus `memcmp` without
...
58. Done: add dynamic string-key `index_set` dispatch and growth for unknown
    descriptor containers. The lowerer routes unproven-container string-key
    writes to `oren_llvm_runtime_dynamic_index_set_string`, and the C runtime
    validates the container as a registered map, replaces semantic string keys,
