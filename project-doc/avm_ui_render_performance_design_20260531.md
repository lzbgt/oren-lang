...
- retained 3D mesh records: `mesh3d {id,triangles,color}` uploads packed
  ...
  it, and `destroy_mesh3d {id}` releases the virtual resource. `mesh3d_rgba`
  stores the same signed XYZ triangles plus per-triangle RGBA bytes for visible
  depth conformance. The v0 default renderers use deterministic orthographic XY
...
19. Done: add retained 2D mesh resource records (`mesh2d`, `draw_mesh2d`,
    ...
    verifier coverage.
20. Done: add first retained 3D mesh resource records (`mesh3d`, `mesh3d_rgba`,
    `draw_mesh3d`, `destroy_mesh3d`) across validation, binary frames, AVM protocol checks,
