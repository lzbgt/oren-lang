...
- iOS `OrenAVMGraphicsView` renders the current CoreGraphics fallback subset:
  ...
  `mesh2d`, `draw_mesh2d`, `destroy_mesh2d`, `mesh3d`, `draw_mesh3d`, `destroy_mesh3d`, `text_resource`, `draw_text`, `draw_texts`, `destroy_text`, `image_rgba`, `draw_image`,
...
- retained 3D mesh records: `mesh3d {id,triangles,color}` uploads packed
  little-endian `i32 x,y,z` triangle geometry once, `draw_mesh3d {id}` reuses
  it, and `destroy_mesh3d {id}` releases the virtual resource. The v0 default
  renderers use deterministic orthographic XY projection while preserving Z in
  the frame protocol for future camera/depth renderers;
...
19. Done: add retained 2D mesh resource records (`mesh2d`, `draw_mesh2d`,
    ...
    verifier coverage.
20. Done: add first retained 3D mesh resource records (`mesh3d`, `draw_mesh3d`,
    `destroy_mesh3d`) across validation, binary frames, AVM protocol checks,
    deterministic raster, CoreGraphics fallback, Metal geometry draws, iOS
    verifier coverage, and release conformance through orthographic projection.
