...
- iOS `OrenAVMMetalView` is the first Metal/`MTKView` path: it owns the Metal draw
  ...
  without exposing UIKit/Metal objects to OBC. Current text rendering uses transient
  per-frame textures; retained text atlas and richer resources remain the next
  performance step.
