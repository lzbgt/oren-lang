...
scene-level camera depth metadata, declares a read-only `assets/` VFS mount, and
proves OBC can load retained UI scene metadata from package assets without JSON
parsing in the hot path. `make verify-libavm-ios` also carries an SDK-local
