...
- Fresh landing (2026-04-23): timeout-aware watcher helpers now ship above that same
  ...
  - Optimizer subprofile and list prefilter (2026-05-05): the optimizer now emits a gated
    ...
    total from ~3.16s to ~1.98s, `list_int_ms` from ~1.16s to ~0.21s, `reserve_ms` from ~0.45s to ~0.25s, and the
    largest list-int body from ~135ms to ~28ms. The same summary now reports list-pass internals: skipping an unnecessary
    recursive-lowering seed clone, sharing the conservative list-touch detector across top-level candidate filtering and
    nested list-int lowering, skipping no-op recursive lowering for statement forms with no lowerable/list-touching body,
    scanning list uses only after a current-block candidate exists, and delaying touch marking until an empty-list candidate
    exists moves `list_int_scan_ms2` from ~863ms to ~173ms, with remaining scan-loop buckets split into `lower` ~68ms,
    `use_scan` ~40ms, and `touch` ~47ms. Reserve is dominated by safe-int maintenance; skipping the duplicate top-level
    `Var`/`Assign` safe-int update in the reserve pass moves `reserve_ms` to ~0.25s and `reserve_safe_ms` to ~220-225ms. A
    narrower reserve-state guard lowered `reserve_safe_ms` by only ~9ms and worsened repeated optimizer totals to
  ...
  - Module parse profiling (2026-05-05): the serial module parse path now emits `link.parse_module.done` rows
    ...
    `task.validate_stop_policy(...)` wrapper removed the duplicated task-group body and passed task, task-group,
    C, and native checks, but repeated the bytecode-only mixed task-group failure at `191096`. A narrowed bytecode
    probe showed the mechanism: after prior runtime stop/cancel cases, helper-routed invalid-policy preflight took
    about 278ms, then cancel-wait took about 321ms, letting the observer task return uncancelled `1800` instead of
    the expected cancel-observed `2712`. Do not route live runtime-group preflight through `std:task` policy helpers
    unless bytecode preflight latency is removed.
    ...
    visible `gen_core_ms` to ~41ms but made the native profile time out later in emit/codegen, so cache designs
    must avoid pulling ASTBIN codec codegen into parser builds.
...
- Dense 2026-05-06 Mach-O resolver, generator carrier, dead-helper, stale-import, and runtime-native
  rejection history moved to `project-doc/rolling_cleanup_notes_20260506.md` so this coroutine note
  stays below the working line guardrail while preserving the measurement trail.
- 2026-05-06: rejected user-function preseed for the Mach-O append-only first-byte bucket resolver.
  The probe seeded `422` `ctx["function_names"]` targets and passed `make stage2`, but it regressed
  default first-4096 BL timing from the retained `~598ms` to about `1039ms`; the preseed phase alone
  cost about `518ms` and raised BL scan/candidate counts (`56888` / `1430`), so lazy bucketed lookup
