...
func buildFixtureCasesFull(target string, gcArg string) []fixtureCase {
	full := true
	...
	if envBool("OREN_REMOTE_RUN", false) {
		...
		// - Batch all remote x64 fixtures into one upload and two remote script runs (Win + WSL).
		// - Run Win and WSL in parallel when possible.
