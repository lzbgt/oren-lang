...
src="tests/fixtures/boxed_list_dot_sum_smoke.oren"
out_c="build/tmp/boxed_list_dot_sum_c${exe_ext}"
out_native="build/tmp/boxed_list_dot_sum_native${exe_ext}"
out_obc="build/tmp/boxed_list_dot_sum.obc"

log_c="build/logs/boxed_list_dot_sum_c_build.log"
log_native="build/logs/boxed_list_dot_sum_native_build.log"
log_obc="build/logs/boxed_list_dot_sum_obc_build.log"
run_c="build/logs/boxed_list_dot_sum_c_run.log"
run_native="build/logs/boxed_list_dot_sum_native_run.log"
run_obc="build/logs/boxed_list_dot_sum_obc_run.log"
...
grep -F "ok: boxed list dot sum" "$run_c.norm" >/dev/null || { echo "FAIL: C output missing ok marker" >&2; cat "$run_c.norm" >&2; exit 6; }
grep -F "ok: boxed list dot sum" "$run_native.norm" >/dev/null || { echo "FAIL: native output missing ok marker" >&2; cat "$run_native.norm" >&2; exit 7; }
grep -F "ok: boxed list dot sum" "$run_obc.norm" >/dev/null || { echo "FAIL: obc output missing ok marker" >&2; cat "$run_obc.norm" >&2; exit 8; }
...
echo "OK: boxed list dot/sum parity (C/native/obc)" >&2
