...
summary_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-list-int-${ts}.log"
default_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-list-int-default-${ts}.run.log"
enabled_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-list-int-enabled-${ts}.run.log"

programs="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_LIST_INT_PROGRAMS:-array_sum_int}"
run_test="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_LIST_INT_RUN_TEST:-0}"
enable_env="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_LIST_INT_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_GET_SUM_DUAL_ACCUM=1}"
...
print("arm64 fast get-sum dual-accum list<int> probe summary")
print("")
...
for label, key in [
    ("steady_array_sum_int_native_median_delta_pct", "steady_array_sum_int_native_median_s"),
    ("gate_array_sum_int_native_median_delta_pct", "gate_array_sum_int_native_median_s"),
]:
...
echo "arm64 fast get-sum dual-accum list<int> probe complete; summary: $summary_log"
...
if [[ "$enabled_rc" != "0" ]]; then
    echo "enabled fast get-sum dual-accum list<int> experiment failed (exit=$enabled_rc); see summary for details"
fi
