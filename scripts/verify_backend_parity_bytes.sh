...
src="tests/fixtures/bytes_backend_parity_smoke.oren"
out_c="build/tmp/bytes_backend_parity_c${exe_ext}"
out_native="build/tmp/bytes_backend_parity_native${exe_ext}"
out_obc="build/tmp/bytes_backend_parity.obc"

log_c="build/logs/bytes_backend_parity_c_build.log"
log_native="build/logs/bytes_backend_parity_native_build.log"
log_obc="build/logs/bytes_backend_parity_obc_build.log"
run_c="build/logs/bytes_backend_parity_c_run.log"
run_native="build/logs/bytes_backend_parity_native_run.log"
run_obc="build/logs/bytes_backend_parity_obc_run.log"
...
grep -F "ok: bytes parity" "$run_c.norm" >/dev/null || { echo "FAIL: C output missing ok marker" >&2; cat "$run_c.norm" >&2; exit 6; }
grep -F "ok: bytes parity" "$run_native.norm" >/dev/null || { echo "FAIL: native output missing ok marker" >&2; cat "$run_native.norm" >&2; exit 7; }
grep -F "ok: bytes parity" "$run_obc.norm" >/dev/null || { echo "FAIL: obc output missing ok marker" >&2; cat "$run_obc.norm" >&2; exit 8; }
...
echo "OK: bytes parity (C/native/obc)" >&2
