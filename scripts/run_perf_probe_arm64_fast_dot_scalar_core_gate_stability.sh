...
summary_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-gate-stability-${ts}.log"
manifest_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-gate-stability-${ts}.cases.tsv"

programs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_PROGRAMS:-dot_product}"
sweeps="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_SWEEPS:-4}"
runs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_RUNS:-5}"
warmups="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_WARMUPS:-1}"
bench_args="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_ARGS:-2000000 1}"
case_set="${OREN_ARM64_FAST_DOT_SCALAR_CORE_GATE_STABILITY_CASES:-baseline cursor_disabled scalar_enabled cursor_scalar_enabled}"
...
paths = sorted(
    glob.glob("build/benchmarks/results/dot_product_darwin_arm64_*.json"),
...
run_case() {
    ...
    local env_desc="$4"
    local build_env="$5"
    local run_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-gate-stability-${label}-sweep${sweep}-${ts}.run.log"
    local status=0
    local result_json=""
...
print("arm64 fast dot scalar-core gate stability summary")
print("")
...
echo "arm64 fast-dot scalar-core gate-stability probe complete; summary: $summary_log"
