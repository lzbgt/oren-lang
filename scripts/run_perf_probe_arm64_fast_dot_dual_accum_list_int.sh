...
summary_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-list-int-${ts}.log"
default_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-list-int-default-${ts}.run.log"
enabled_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-list-int-enabled-${ts}.run.log"

programs="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_LIST_INT_PROGRAMS:-dot_product_int}"
run_test="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_LIST_INT_RUN_TEST:-0}"
enable_env="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_LIST_INT_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_DOT_DUAL_ACCUM=1}"
...
print("arm64 fast dot dual-accum list<int> probe summary")
print("")
...
echo "arm64 fast-dot dual-accum list<int> probe complete; summary: $summary_log"
...
if [[ "$enabled_rc" != "0" ]]; then
    echo "enabled fast-dot dual-accum list<int> experiment failed (exit=$enabled_rc); see summary for details"
fi
