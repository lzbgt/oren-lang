...
MODULEMAP
  cat > "$OUT_ROOT/lib/pkgconfig/libavm.pc" <<PC
prefix=\${pcfiledir}/../..
includedir=\${prefix}/include
libdir=\${prefix}/lib/${TARGET}

Name: LibAVM
Description: Oren AVM C embedder SDK
Version: 0.1.0-rolling
Cflags: -I\${includedir}
Libs: \${libdir}/libavm.a
PC
}

main() {
  command -v "$ZIG" >/dev/null || {
    echo "ERROR: zig is required for x64 Linux LibAVM cross build" >&2
    exit 2
  }
