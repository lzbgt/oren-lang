...
gas_sidecar_missing_run_json_out="$TMP/gas-sidecar-missing-run-json.out"
gas_sidecar_missing_run_json_err="$TMP/gas-sidecar-missing-run-json.err"
gas_sidecar_missing_run_json_json="$TMP/gas-sidecar-missing-run-json.run.json"
...
OREN_NATIVE_PACKAGE_POLICY_RUN_JSON="$gas_sidecar_missing_run_json_json" \
  OREN_NATIVE_PACKAGE_POLICY_TEST_AVM_SIDECAR_DROP_RUN_JSON=1 \
  ./scripts/run_package_policy.sh --backend native --gas-profile avm-sidecar tests/fixtures/native_package_policy_runner_gas_ok.oren \
  >"$gas_sidecar_missing_run_json_out" 2>"$gas_sidecar_missing_run_json_err"
gas_sidecar_missing_run_json_rc=$?
OREN_NATIVE_PACKAGE_POLICY_RUN_JSON="$gas_sidecar_schema_mismatch_json" \
...
OREN_NATIVE_PACKAGE_POLICY_RUN_JSON="$gas_sidecar_build_fail_json" \
  OREN_NATIVE_PACKAGE_POLICY_TEST_AVM_SIDECAR_BUILD_FAIL=1 \
  ./scripts/run_package_policy.sh --backend native --gas-profile avm-sidecar tests/fixtures/native_package_policy_runner_gas_ok.oren \
...
def assert_sidecar_run_json(path, sidecar, *, present, schema="avm.run.v1", status=None, error_code=None, error_msg=None):
    ...
    if not present:
        ...
        return
    if sidecar.get("sidecar_run_json_schema") != schema:
        fail(f"{path}: AVM sidecar run JSON schema mismatch, expected {schema!r}, got {sidecar!r}")
...
_, _, sidecar_missing_run_json = assert_sidecar_unavailable_run(
...
if sidecar_missing_run_json.get("same_run_exit_code_equal") is not True:
    fail(f"{gas_sidecar_missing_run_json_path}: missing-run-json sidecar should preserve exit parity, got {sidecar_missing_run_json!r}")
if sidecar_missing_run_json.get("native_exit_code") != 0 or sidecar_missing_run_json.get("sidecar_exit_code") != 0:
    fail(f"{gas_sidecar_missing_run_json_path}: missing-run-json sidecar should expose zero native/sidecar exits, got {sidecar_missing_run_json!r}")
if sidecar_missing_run_json.get("gas_surface") is not None or sidecar_missing_run_json.get("gas_executed") is not None:
    fail(f"{gas_sidecar_missing_run_json_path}: missing-run-json sidecar should omit AVM canonical gas evidence, got {sidecar_missing_run_json!r}")
if sidecar_missing_run_json.get("sidecar_error") is not None:
    fail(f"{gas_sidecar_missing_run_json_path}: missing-run-json sidecar should not synthesize AVM run error evidence, got {sidecar_missing_run_json!r}")

_, _, sidecar_schema_mismatch = assert_sidecar_unavailable_run(
    gas_sidecar_schema_mismatch_path,
    ["missing_or_noncanonical_avm_run_json", "missing_or_noncanonical_avm_gas_surface"],
    "schema",
    run_json_schema="not.avm.run.v1",
)
if sidecar_schema_mismatch.get("sidecar_run_json_schema") != "not.avm.run.v1":
    fail(f"{gas_sidecar_schema_mismatch_path}: schema-mismatched sidecar should preserve the bad schema for audit, got {sidecar_schema_mismatch!r}")
if sidecar_schema_mismatch.get("same_run_stdout_equal") is not True or sidecar_schema_mismatch.get("same_run_stderr_equal") is not True:
    fail(f"{gas_sidecar_schema_mismatch_path}: schema-mismatched sidecar should preserve stdout/stderr parity, got {sidecar_schema_mismatch!r}")
if sidecar_schema_mismatch.get("same_run_exit_code_equal") is not True:
    fail(f"{gas_sidecar_schema_mismatch_path}: schema-mismatched sidecar should preserve exit parity, got {sidecar_schema_mismatch!r}")
if sidecar_schema_mismatch.get("native_exit_code") != 0 or sidecar_schema_mismatch.get("sidecar_exit_code") != 0:
    fail(f"{gas_sidecar_schema_mismatch_path}: schema-mismatched sidecar should expose zero native/sidecar exits, got {sidecar_schema_mismatch!r}")
schema_mismatch_surface = sidecar_schema_mismatch.get("gas_surface") or {}
if schema_mismatch_surface.get("id") != "avm_opcode_cost_v0" or schema_mismatch_surface.get("unit_scope") != "avm_canonical":
    fail(f"{gas_sidecar_schema_mismatch_path}: schema-mismatched sidecar should preserve gas evidence but not certify it, got {sidecar_schema_mismatch!r}")
if int(sidecar_schema_mismatch.get("gas_executed") or 0) <= 0:
...
gas_sidecar_build_fail = load(gas_sidecar_build_fail_path)
if gas_sidecar_build_fail.get("schema") != "oren.native-package-policy-run.v0":
    fail(f"{gas_sidecar_build_fail_path}: schema mismatch: {gas_sidecar_build_fail.get('schema')!r}")
if gas_sidecar_build_fail.get("status") != "budget_unavailable" or gas_sidecar_build_fail.get("exit_code") != 77:
    fail(
        f"{gas_sidecar_build_fail_path}: expected budget_unavailable/77 for build-failed sidecar, got "
        f"status={gas_sidecar_build_fail.get('status')!r} exit={gas_sidecar_build_fail.get('exit_code')!r}"
    )
gas_sidecar_build_fail_budget = ((gas_sidecar_build_fail.get("budgets") or {}).get("gas") or {})
if gas_sidecar_build_fail_budget.get("enforced") is not False or gas_sidecar_build_fail_budget.get("enforcement_profile") != "avm-sidecar":
    fail(f"{gas_sidecar_build_fail_path}: build-failed AVM sidecar gas must not be marked enforced, got {gas_sidecar_build_fail_budget!r}")
if gas_sidecar_build_fail_budget.get("reason") != "AVM canonical sidecar gas was not certified":
    fail(f"{gas_sidecar_build_fail_path}: build-failed AVM sidecar gas reason mismatch, got {gas_sidecar_build_fail_budget!r}")
sidecar_build_fail = gas_sidecar_build_fail.get("avm_canonical_sidecar_gas") or {}
if sidecar_build_fail.get("schema") != "oren.avm-canonical-sidecar-gas.v0":
...
if sidecar_build_fail.get("certification_failure_reasons") != ["sidecar_build_failed"]:
    fail(f"{gas_sidecar_build_fail_path}: expected sidecar_build_failed failure reason, got {sidecar_build_fail!r}")
if sidecar_build_fail.get("certification_warnings") != []:
    fail(f"{gas_sidecar_build_fail_path}: build-failed sidecar should not carry warnings, got {sidecar_build_fail!r}")
if sidecar_build_fail.get("test_injection") != "build_fail":
    fail(f"{gas_sidecar_build_fail_path}: expected auditable build_fail verifier injection, got {sidecar_build_fail!r}")
if sidecar_build_fail.get("package_policy_may_use_reason") != "avm_canonical_sidecar_build_failed":
    fail(f"{gas_sidecar_build_fail_path}: build-failed sidecar reason mismatch, got {sidecar_build_fail!r}")
if sidecar_build_fail.get("sidecar_build_exit_code") != 99:
    fail(f"{gas_sidecar_build_fail_path}: expected injected sidecar build exit 99, got {sidecar_build_fail!r}")
if not sidecar_build_fail.get("sidecar_build_log"):
    fail(f"{gas_sidecar_build_fail_path}: expected sidecar build log evidence, got {sidecar_build_fail!r}")
if sidecar_build_fail.get("native_exit_code") is not None or sidecar_build_fail.get("sidecar_exit_code") is not None:
    fail(f"{gas_sidecar_build_fail_path}: build-failed sidecar should not expose runtime exit codes, got {sidecar_build_fail!r}")
if (
    sidecar_build_fail.get("same_run_stdout_equal") is not None
    or sidecar_build_fail.get("same_run_stderr_equal") is not None
    or sidecar_build_fail.get("same_run_exit_code_equal") is not None
):
...
if [[ "$gas_sidecar_missing_run_json_rc" -eq 0 ]]; then
  echo "ERROR: expected missing-run-json AVM sidecar evidence to fail closed" >&2
  cat "$gas_sidecar_missing_run_json_out" >&2 || true
  cat "$gas_sidecar_missing_run_json_err" >&2 || true
  exit 1
fi
grep -Fq "package AVM canonical sidecar gas could not be certified for native run" "$gas_sidecar_missing_run_json_out" "$gas_sidecar_missing_run_json_err" || {
  echo "ERROR: missing missing-run-json AVM sidecar diagnostic" >&2
  cat "$gas_sidecar_missing_run_json_out" >&2 || true
  cat "$gas_sidecar_missing_run_json_err" >&2 || true
  exit 1
}

if [[ "$gas_sidecar_schema_mismatch_rc" -eq 0 ]]; then
  echo "ERROR: expected schema-mismatched AVM sidecar evidence to fail closed" >&2
  cat "$gas_sidecar_schema_mismatch_out" >&2 || true
  cat "$gas_sidecar_schema_mismatch_err" >&2 || true
  exit 1
fi
grep -Fq "package AVM canonical sidecar gas could not be certified for native run" "$gas_sidecar_schema_mismatch_out" "$gas_sidecar_schema_mismatch_err" || {
  echo "ERROR: missing schema-mismatched AVM sidecar diagnostic" >&2
  cat "$gas_sidecar_schema_mismatch_out" >&2 || true
  cat "$gas_sidecar_schema_mismatch_err" >&2 || true
  exit 1
}
...
if [[ "$gas_sidecar_build_fail_rc" -eq 0 ]]; then
  echo "ERROR: expected build-failed AVM sidecar evidence to fail closed" >&2
  cat "$gas_sidecar_build_fail_out" >&2 || true
  cat "$gas_sidecar_build_fail_err" >&2 || true
  exit 1
fi
grep -Fq "package AVM canonical sidecar build failed" "$gas_sidecar_build_fail_out" "$gas_sidecar_build_fail_err" || {
