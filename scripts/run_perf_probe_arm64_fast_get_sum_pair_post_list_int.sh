...
summary_log="$log_dir/perf-probe-arm64-fast-get-sum-pair-post-list-int-${ts}.log"
default_log="$log_dir/perf-probe-arm64-fast-get-sum-pair-post-list-int-default-${ts}.run.log"
enabled_log="$log_dir/perf-probe-arm64-fast-get-sum-pair-post-list-int-enabled-${ts}.run.log"

programs="${OREN_ARM64_FAST_GET_SUM_PAIR_POST_LIST_INT_PROGRAMS:-array_sum_int}"
run_test="${OREN_ARM64_FAST_GET_SUM_PAIR_POST_LIST_INT_RUN_TEST:-0}"
enable_env="${OREN_ARM64_FAST_GET_SUM_PAIR_POST_LIST_INT_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_GET_SUM_PAIR_POST=1}"
...
print("arm64 fast get-sum pair-post list<int> probe summary")
print("")
...
echo "arm64 fast get-sum pair-post list<int> probe complete; summary: $summary_log"
...
if [[ "$enabled_rc" != "0" ]]; then
    echo "enabled get-sum pair-post experiment failed (exit=$enabled_rc); see summary for details"
fi
