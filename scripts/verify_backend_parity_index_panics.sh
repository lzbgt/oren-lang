...
run_expect_panic() {
  ...
  if [[ "$expect" == "INDEX_OOB" ]]; then
    ...
    expect="map key type not supported|map_set: unsupported key type|map_get: unsupported key type"
    grep_flags=(-i -E)
    use_fixed=0
  fi
  set +e
...
cases=(
  ...
  "index_get_map_bad_key|tests/native/fixtures/index_get_map_bad_key.oren|MAP_KEY_UNSUPPORTED"
)
...
for entry in "${cases[@]}"; do
  ...
  run_expect_panic "c" "$run_c" "$expect_msg" env "${trace_env_arr[@]}" "$out_c"

  echo "== run: native backend ($name) ==" >&2
  run_expect_panic "native" "$run_native" "$expect_msg" env "${trace_env_arr[@]}" "$out_native"

  echo "== run: OBC (avm) ($name) ==" >&2
  run_expect_panic "obc" "$run_obc" "$expect_msg" env "${trace_env_arr[@]}" ./avm "$out_obc"
