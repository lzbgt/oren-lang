...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute readiness index tag summary."
    )
    parser.add_argument(
        "--index",
    ...
    parser.add_argument(
        "--out-md",
        default="build/reports/readiness_index_tags.md",
        help="Output markdown path",
    )
    parser.add_argument(
        "--out-json",
        default="build/reports/readiness_index_tags.json",
...
def compute_tags(entries: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    tags: Dict[str, List[Dict[str, Any]]] = {}
    for entry in entries:
        tag = str(entry.get("tag", "none")) or "none"
        tags.setdefault(tag, []).append(entry)
    return {tag: summarize(items) for tag, items in tags.items()}


def write_markdown(path: str, tags: Dict[str, Dict[str, Any]], total_entries: int) -> None:
    ensure_parent_dir(path)
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Readiness index tags\n\n")
        f.write(f"- total entries: {total_entries}\n\n")
        f.write("| Tag | Total | Pass | Fail | Pass % | Latest | Duration avg/min/max |\n")
        f.write("| --- | --- | --- | --- | --- | --- | --- |\n")
...
def write_json(path: str, tags: Dict[str, Dict[str, Any]], total_entries: int) -> None:
    ensure_parent_dir(path)
    payload = {
        "total": total_entries,
        "tags": tags,
...
def main() -> int:
    ...
    if args.profile:
        entries = [e for e in entries if e.get("profile", "") == args.profile]
    if args.limit and args.limit > 0:
        entries = entries[-args.limit :]
    tags = compute_tags(entries)
    write_markdown(args.out_md, tags, len(entries))
    write_json(args.out_json, tags, len(entries))
