...
summary_log="$log_dir/perf-probe-arm64-fast-dot-prefix-zero-list-int-${ts}.log"
default_log="$log_dir/perf-probe-arm64-fast-dot-prefix-zero-list-int-default-${ts}.run.log"
enabled_log="$log_dir/perf-probe-arm64-fast-dot-prefix-zero-list-int-enabled-${ts}.run.log"

run_test="${OREN_ARM64_FAST_DOT_PREFIX_ZERO_LIST_INT_RUN_TEST:-0}"
programs="${OREN_ARM64_FAST_DOT_PREFIX_ZERO_LIST_INT_PROGRAMS:-dot_product_int}"
enable_env="${OREN_ARM64_FAST_DOT_PREFIX_ZERO_LIST_INT_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_DOT_PREFIX_ZERO=1}"
...
print("arm64 fast dot prefix-zero list<int> probe summary")
print("")
...
if [[ "$enabled_rc" != "0" ]]; then
    echo "enabled fast-dot prefix-zero list<int> experiment failed (exit=$enabled_rc); see summary for details"
fi
