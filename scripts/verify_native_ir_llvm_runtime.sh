...
src="tests/fixtures/native_ir_llvm_runtime_main.oren"
out_dir="build/native_ir/llvm_runtime"
log_dir="build/logs"
...
toolchain_log="$log_dir/native_ir_llvm_runtime_toolchain.log"
native_log="$log_dir/native_ir_llvm_runtime_native.log"
llvm_build_log="$log_dir/native_ir_llvm_runtime_build.log"
link_log="$log_dir/native_ir_llvm_runtime_link.log"
llvm_run_log="$log_dir/native_ir_llvm_runtime_run.log"
summary="$out_dir/summary.txt"
harness="$out_dir/harness.c"
object="$out_dir/runtime_arm64_macos.o"
native_bin="$out_dir/native_oracle"
llvm_bin="$out_dir/llvm_probe"
...
if [ -z "$clang_path" ] || [ "$clang_path" = "missing" ]; then
  cat "$toolchain_log"
  echo "ERROR: clang is required for native IR LLVM runtime linking" >&2
  exit 1
fi
...
test -s "$object"
test -s "$object.ll"
grep -Fq "br i1" "$object.ll"
grep -Fq "mul i64" "$object.ll"
...
{
  printf 'native_ir_llvm_runtime_v0\n'
  ...
  printf 'coverage=host-arm64-macos,native-oracle,llvm-link,llvm-execute,cfg,locals,comparisons,arithmetic\n'
} >"$summary"

echo "OK: native IR LLVM runtime parity passed; summary: $summary"
