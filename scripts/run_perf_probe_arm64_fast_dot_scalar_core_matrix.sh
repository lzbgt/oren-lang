...
summary_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-${ts}.log"
manifest_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-${ts}.cases.tsv"

programs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_MATRIX_PROGRAMS:-dot_product}"
run_test="${OREN_ARM64_FAST_DOT_SCALAR_CORE_MATRIX_RUN_TEST:-0}"

run_case() {
    local label="$1"
    local env_desc="$2"
    local run_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-${label}-${ts}.run.log"
    local status=0
    local build_env=""
    shift 2
    if (($# > 0)); then
...
print("arm64 fast dot scalar-core matrix summary")
print("")
...
echo "arm64 fast-dot scalar-core matrix probe complete; summary: $summary_log"
