...
if ! grep -Fq 'fn _intr_tmp_pool_off(ctx, idx)' <<<"$x64_parser_helper_split_impl" ||
  ...
  ! grep -Fq 'fn _x64_varargs_named_call_emit_prepared(ctx, locals, st)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_emit_while_cond_jump(ctx, op, locals, local_fixups, end_label2)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_index_emit_map_runtime_fast(ctx, locals, known_kk, tmp0, tmp_idx, base0)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_index_set_emit_non_nil_container(ctx, state)' <<<"$x64_parser_helper_split_impl" ||
  ! grep -Fq 'fn _x64_member_index_expr(ctx, expr)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_local_var_offset(locals, name)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_emit_load_global_to_reg(ctx, name, dst_reg)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_resolve_unique_fn_suffix(ctx, fn_name)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_emit_list_int_get_unchecked_negative_guard(ctx, labels, local_fixups, l_idx_nonneg)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_emit_cmp_string_byte_order(ctx, names, fixups)' <<<"$x64_parser_helper_split_impl" || ! grep -Fq 'fn _x64_gc_root_insert_sorted(offs, seen, off)' <<<"$x64_parser_helper_split_impl"; then
...
./scripts/verify_avm_bytes_hotpath_guards_x64_sys_data.sh
...
./scripts/verify_avm_bytes_hotpath_guards_artifacts.sh
...
if grep -nE 'while .*< oren_list_len\(' \
  lib/compiler/optimizer_loops_list.oren \
  lib/compiler/optimizer_loops_arena.oren \
  lib/compiler/optimizer_loops_list_reset.oren \
  lib/compiler/optimizer_loops_bounds.oren \
  lib/compiler/optimizer_loops_split.oren \
  lib/compiler/optimizer_helpers.oren \
  lib/compiler/optimizer_tco.oren \
  lib/compiler/optimizer_constprop.oren \
  lib/compiler/analysis.oren \
  lib/compiler/optimizer_list_reserve.oren \
  lib/compiler/optimizer_list_int.oren \
  ...
  lib/compiler/serde_json_lowering.oren \
  lib/compiler/module_visibility.oren \
  ...
  lib/compiler/codegen_bytecode/030_tail_loops.oren \
  lib/compiler/obc_link.oren \
  lib/compiler/type_ann_lowering.oren \
  lib/compiler/type_name_resolve.oren \
  lib/compiler/native_runtime_inject.oren \
  lib/compiler/native_toplevel.oren \
  lib/compiler/native_ops_v0.oren \
  ...
  lib/compiler/parser_core.oren \
  lib/compiler/parser_parse/000_prelude.oren \
  lib/compiler/parser_parse/005_generator_core.oren \
  lib/compiler/parser_parse/010_parser_a.oren \
  ...
  lib/compiler/x64_native_program/010_data_io.oren \
  ...
  lib/compiler/parser_parse/000_prelude_generator.oren \
