...
./scripts/readiness_report_index_audit.py --index "$index_path" --out-md "$out_md" --out-json "$out_json" --out-csv "$out_csv" \
...
rg -n "checked,missing_any,missing_report,missing_json,missing_log_dir" "$out_csv" >/dev/null
