...
./scripts/readiness_report_index_audit.py --index "$index_path" --out-md "$out_md" --out-json "$out_json" --out-csv "$out_csv" --out-samples-csv "$out_samples_csv" \
  --max-missing-report 10 --max-missing-json 10 --max-missing-log-dir 10 --max-missing-status-matrix-md 10
...
rg -n "checked,missing_any,missing_report,missing_json,missing_log_dir,missing_status_snapshot_md,missing_status_snapshot_json,missing_status_faq_md,missing_status_faq_json,missing_status_matrix_md,missing_status_matrix_json" "$out_csv" >/dev/null
rg -n "timestamp,profile,tag,missing,report,json,log_dir,status_snapshot_md,status_snapshot_json,status_faq_md,status_faq_json,status_matrix_md,status_matrix_json" "$out_samples_csv" >/dev/null
