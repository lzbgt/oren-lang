...
grep -Fq "ret i64 99" "$object.ll"
grep -Fq "ret i64 102" "$object.ll"
...
end="$(date +%s)"
{
  ...
  printf 'coverage=host-arm64-macos,native-oracle,llvm-link,llvm-execute,named-oren-string-byte-at-unchecked-helper,named-oren-string-char-at-helper,unchecked-byte-access,checked-and-unchecked-char-access,string-eq-compose\n'
} >"$summary"
