...
rm -f "${snapshot_dir}/status_overview_"*.md 2>/dev/null || true
...
rg -n "\"status_overview_md\"" "$json_path" >/dev/null
...
rg -n "\"status_overview_md\"" "$index_path" >/dev/null
...
test -f "$matrix_md"
test -f "$matrix_json"
overview_md="$(ls -t "${snapshot_dir}/status_overview_"*.md | head -n 1)"
test -f "$overview_md"
