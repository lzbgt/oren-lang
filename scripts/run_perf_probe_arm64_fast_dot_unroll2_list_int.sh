...
summary_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-list-int-${ts}.log"
default_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-list-int-default-${ts}.run.log"
enabled_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-list-int-enabled-${ts}.run.log"

programs="${OREN_ARM64_FAST_DOT_UNROLL2_LIST_INT_PROGRAMS:-dot_product_int}"
run_test="${OREN_ARM64_FAST_DOT_UNROLL2_LIST_INT_RUN_TEST:-0}"
enable_env="${OREN_ARM64_FAST_DOT_UNROLL2_LIST_INT_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_DOT_UNROLL2=1}"
...
print("arm64 fast-dot unroll2 list<int> probe summary")
print("")
...
for label, key in [
    ("enabled_steady_dot_product_int_native_median_delta_pct", "steady_dot_product_int_native_median_s"),
    ("enabled_gate_dot_product_int_native_median_delta_pct", "gate_dot_product_int_native_median_s"),
]:
...
echo "arm64 fast-dot unroll2 list<int> probe complete; summary: $summary_log"
...
if [[ "$enabled_rc" != "0" ]]; then
    echo "enabled unroll2 list<int> experiment failed (exit=$enabled_rc); see summary for details"
fi
