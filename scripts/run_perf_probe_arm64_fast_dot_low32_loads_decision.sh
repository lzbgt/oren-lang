...
case_set="${OREN_ARM64_FAST_DOT_LOW32_LOADS_DECISION_CASES:-baseline low32_enabled low32_scalar_enabled unroll2_low32_enabled unroll2_low32_dual_madd_enabled}"
candidate="${OREN_ARM64_FAST_DOT_LOW32_LOADS_DECISION_CANDIDATE:-unroll2_low32_dual_madd_enabled}"

env \
    OREN_ARM64_FAST_DOT_CASE_DECISION_TAG="perf-probe-arm64-fast-dot-low32-loads-decision" \
    OREN_ARM64_FAST_DOT_CASE_DECISION_TITLE="arm64 fast dot low32 slot-load decision summary" \
    OREN_ARM64_FAST_DOT_CASE_DECISION_COMPLETE_LABEL="arm64 fast-dot low32 slot-load decision probe" \
