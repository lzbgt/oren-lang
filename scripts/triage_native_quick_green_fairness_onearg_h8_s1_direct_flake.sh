...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  ...
  echo "Builds the focused green fairness binary once and reruns the direct h8/s1 one-arg slice." >&2
  exit 0
fi
...
label="${OREN_QI_LABEL:-native_quick_green_fairness_full_notopology_onearg_h8_s1_direct}"
src="tests/native/test_quick_integration_green_fairness_focus.oren"
exe_ext=""
...
echo "== build focused fairness direct binary ==" >&2
...
while [[ "$run" -le "$runs" ]]; do
  ...
  env \
    ...
    OREN_QI_GREEN_FAIRNESS_MODE="${OREN_QI_GREEN_FAIRNESS_MODE:-full}" \
    OREN_QI_GREEN_FAIRNESS_SHORT_ARG_MODE="${OREN_QI_GREEN_FAIRNESS_SHORT_ARG_MODE:-one_arg}" \
    OREN_QI_GREEN_FAIRNESS_INCLUDE_TOPOLOGY="${OREN_QI_GREEN_FAIRNESS_INCLUDE_TOPOLOGY:-0}" \
    OREN_QI_GREEN_FAIRNESS_HOGS="${OREN_QI_GREEN_FAIRNESS_HOGS:-8}" \
    OREN_QI_GREEN_FAIRNESS_SHORTS="${OREN_QI_GREEN_FAIRNESS_SHORTS:-1}" \
