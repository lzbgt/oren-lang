...
if native_surface.get("schema") != "oren.gas-surface.v0" or native_surface.get("id") != "native_block_weighted_tick_v0":
...
if calibration.get("native_surface_id") != "native_block_weighted_tick_v0":
...
if native_gas.get("kind") != "native_block_weighted_tick_v0":
    fail(f"native gas kind mismatch: {native_gas!r}")
if (native_gas.get("surface") or {}).get("id") != "native_block_weighted_tick_v0":
...
if native_gas.get("executed") is None or int(native_gas.get("executed")) <= 0:
    fail(f"native gas executed should be positive under semantic-diff block-weighted gas, got {native_gas!r}")
...
if native_deltas.get("gas_surface_id") != "native_block_weighted_tick_v0":
