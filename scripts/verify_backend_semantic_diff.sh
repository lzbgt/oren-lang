...
if avm_sidecar.get("test_injection") is not None:
    fail(f"semantic-diff AVM canonical sidecar should not carry verifier test injection, got {avm_sidecar!r}")
...
if avm_sidecar.get("sidecar_error") is not None:
    fail(f"semantic-diff AVM canonical sidecar should have no sidecar error, got {avm_sidecar!r}")
...
if avm_sidecar.get("native_stderr_sha256") != avm_sidecar.get("sidecar_stderr_sha256"):
    fail(f"AVM canonical sidecar stderr hashes should match for semantic-diff fixtures, got {avm_sidecar!r}")
