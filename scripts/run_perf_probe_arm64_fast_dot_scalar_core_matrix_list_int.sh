...
summary_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-list-int-${ts}.log"
manifest_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-list-int-${ts}.cases.tsv"

programs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_MATRIX_LIST_INT_PROGRAMS:-dot_product_int}"
run_test="${OREN_ARM64_FAST_DOT_SCALAR_CORE_MATRIX_LIST_INT_RUN_TEST:-0}"

run_case() {
    local label="$1"
    local env_desc="$2"
    local run_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-matrix-list-int-${label}-${ts}.run.log"
    local status=0
    local build_env=""
    shift 2
    if (($# > 0)); then
...
print("arm64 fast dot scalar-core matrix list<int> summary")
print("")
...
echo "arm64 fast-dot scalar-core matrix list<int> probe complete; summary: $summary_log"
