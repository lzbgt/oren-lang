...
summary_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-read-split-${ts}.log"
manifest_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-read-split-${ts}.cases.tsv"

programs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_READ_SPLIT_PROGRAMS:-dot_product}"
runs="${OREN_ARM64_FAST_DOT_SCALAR_CORE_READ_SPLIT_RUNS:-5}"
warmups="${OREN_ARM64_FAST_DOT_SCALAR_CORE_READ_SPLIT_WARMUPS:-1}"
...
run_case() {
    local label="$1"
    local env_desc="$2"
    local run_log="$log_dir/perf-probe-arm64-fast-dot-scalar-core-read-split-${label}-${ts}.run.log"
    local status=0
    local build_env=""
    shift 2
    if (($# > 0)); then
    ...
    env \
        ...
        make perf-gate-native-read-split >"$run_log" 2>&1
    status=$?
    set -e
...
def parse_summary(run_log):
    ...
    patterns = {
        "dot_product_c_short_s": r"dot_product\s+  c: short=([0-9.]+)s",
        "dot_product_c_long_s": r"dot_product\s+  c: short=[0-9.]+s long=([0-9.]+)s",
        "dot_product_c_setup_s": r"dot_product\s+  c: .*?setup≈([0-9.\-]+)s",
        "dot_product_c_delta_s": r"dot_product\s+  c: .*?delta≈([0-9.\-]+)s",
        "dot_product_c_long_per_rep_s": r"dot_product\s+  c: .*?long/reps≈([0-9.]+)s",
        "dot_product_c_cov": r"dot_product\s+  c: .*?cov=([0-9.]+)",
        "dot_product_native_short_s": r"dot_product\s+.*?oren_native: short=([0-9.]+)s",
        "dot_product_native_long_s": r"dot_product\s+.*?oren_native: short=[0-9.]+s long=([0-9.]+)s",
        "dot_product_native_setup_s": r"dot_product\s+.*?oren_native: .*?setup≈([0-9.\-]+)s",
        "dot_product_native_delta_s": r"dot_product\s+.*?oren_native: .*?delta≈([0-9.\-]+)s",
        "dot_product_native_long_per_rep_s": r"dot_product\s+.*?oren_native: .*?long/reps≈([0-9.]+)s",
        "dot_product_native_cov": r"dot_product\s+.*?oren_native: .*?cov=([0-9.]+)",
        "dot_product_native_over_c_long_per_rep": r"dot_product\s+.*?native/C long-per-rep ratio≈([0-9.]+x)",
        "dot_product_native_over_c_delta": r"dot_product\s+.*?native/C delta ratio≈([0-9.]+x)",
    }
...
print("arm64 fast dot scalar-core read-split summary")
print("")
...
case_metrics = {}
ordered_keys = [
    "dot_product_c_short_s",
    "dot_product_c_long_s",
    "dot_product_c_setup_s",
    "dot_product_c_delta_s",
    "dot_product_c_long_per_rep_s",
    "dot_product_c_cov",
    "dot_product_native_short_s",
    "dot_product_native_long_s",
    "dot_product_native_setup_s",
    "dot_product_native_delta_s",
    "dot_product_native_long_per_rep_s",
    "dot_product_native_cov",
    "dot_product_native_over_c_long_per_rep",
    "dot_product_native_over_c_delta",
]
...
for label, metrics in case_metrics.items():
    ...
    for metric_key, out_key in [
        ("dot_product_native_short_s", f"{label}_dot_product_native_short_delta_pct"),
        ("dot_product_native_setup_s", f"{label}_dot_product_native_setup_delta_pct"),
        ("dot_product_native_delta_s", f"{label}_dot_product_native_delta_delta_pct"),
        ("dot_product_native_long_per_rep_s", f"{label}_dot_product_native_long_per_rep_delta_pct"),
    ]:
...
echo "arm64 fast-dot scalar-core read-split probe complete; summary: $summary_log"
