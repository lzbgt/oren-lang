...
summary_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-decision-${ts}.log"
accept_wrapper_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-decision-${ts}.acceptance.log"
manifest_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-decision-${ts}.manifest.tsv"

sweeps="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_DECISION_SWEEPS:-3}"
run_acceptance="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_DECISION_RUN_ACCEPTANCE:-1}"
enabled_env="${OREN_ARM64_FAST_GET_SUM_DUAL_ACCUM_DECISION_ENABLE_ENV:-OREN_ARM64_FAST_LIST_INT_GET_SUM_DUAL_ACCUM=1}"

accept_summary=""
if [[ "$run_acceptance" == "1" ]]; then
    make perf-probe-arm64-fast-get-sum-dual-accum-list-int >"$accept_wrapper_log" 2>&1
...
m = re.search(r"summary:\s+(build/logs/perf-probe-arm64-fast-get-sum-dual-accum-list-int-[^ ]+\.log)", text)
if m:
    print(m.group(1))
PY
)"
...
for sweep in $(seq 1 "$sweeps"); do
    ...
    for label in $order; do
        run_log="$log_dir/perf-probe-arm64-fast-get-sum-dual-accum-decision-${ts}-${label}-s${sweep}.run.log"
...
accept_steady_pref = accept_pref(accept_metrics, "steady_array_sum_int_native_median_delta_pct")
accept_gate_pref = accept_pref(accept_metrics, "gate_array_sum_int_native_median_delta_pct")
...
print("arm64 fast get-sum dual-accum decision summary")
print("")
...
if os.environ["RUN_ACCEPTANCE"] == "1":
    ...
    if os.environ.get("ACCEPT_SUMMARY"):
        ...
        for key in [
            ...
            "steady_array_sum_int_native_median_delta_pct",
            "gate_array_sum_int_native_median_delta_pct",
        ]:
...
echo "arm64 fast get-sum dual-accum decision probe complete; summary: $summary_log"
