...
if [ -z "$platform" ]; then
  echo "verify_task_group_surface_v0: could not determine host platform; set OREN_PLATFORM" >&2
  exit 1
fi
...
src="tests/fixtures/task_group_surface_v0.oren"
bytecode_out="$tmpdir/task_group_bytecode.obc"
c_out="$tmpdir/task_group_c"
native_out="$tmpdir/task_group_native"
...
echo "verify_task_group_surface_v0: OK ($log)"
