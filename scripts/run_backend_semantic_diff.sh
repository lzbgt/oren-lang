...
Native run-JSON parity builds and runs with OREN_NATIVE_GAS_ACCOUNTING=block-weighted
so semantic-diff reports the scoped v0 native weighted lowering-block gas surface.
EOF
}
...
run_with_timeout "$build_timeout_secs" env "${trace_env_arr[@]}" OREN_NATIVE_GAS_ACCOUNTING=block-weighted "$COMPILER" build "$src" --backend native --platform "$platform" --no-debug -o "$out_native" >"$build_native" 2>&1
...
rc_c=$?
run_with_timeout "$run_timeout_secs" env "${trace_env_arr[@]}" OREN_NATIVE_GAS_ACCOUNTING=block-weighted "$out_native" >"$run_native_out" 2>"$run_native_err"
rc_native=$?
run_with_timeout "$run_timeout_secs" env "${trace_env_arr[@]}" ./avm "$out_obc" >"$run_obc_out" 2>"$run_obc_err"
rc_obc=$?
run_with_timeout "$run_timeout_secs" env "${trace_env_arr[@]}" OREN_NATIVE_GAS_ACCOUNTING=block-weighted OREN_NATIVE_RUN_JSON=1 "$out_native" >"$run_native_json" 2>"$run_native_json_err"
rc_native_json=$?
