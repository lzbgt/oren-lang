...
summary_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-${ts}.log"
smoke_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-smoke-${ts}.log"
steady_default_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-steady-default-${ts}.run.log"
steady_enabled_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-steady-enabled-${ts}.run.log"
gate_default_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-gate-default-${ts}.run.log"
gate_enabled_log="$log_dir/perf-probe-arm64-fast-dot-dual-accum-gate-enabled-${ts}.run.log"

runs="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_RUNS:-5}"
warmups="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_WARMUPS:-1}"
steady_n="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_STEADY_N:-2000000}"
steady_reps="${OREN_ARM64_FAST_DOT_DUAL_ACCUM_STEADY_REPS:-100}"
smoke="${OREN_PERF_SMOKE_NATIVE_FAST_LOOPS:-1}"
enable_env="OREN_ARM64_FAST_LIST_INT_DOT_DUAL_ACCUM=1"
...
run_capture "$steady_enabled_log" env \
    OREN_PERF_SMOKE_NATIVE_FAST_LOOPS=0 \
    OREN_BENCH_PROGRAMS=dot_product \
    OREN_BENCH_RUNS="$runs" \
    OREN_BENCH_WARMUPS="$warmups" \
...
run_capture "$gate_enabled_log" env \
    OREN_BENCH_PROGRAMS=dot_product \
    OREN_BENCH_RUNS="$runs" \
    OREN_BENCH_WARMUPS="$warmups" \
    OREN_BENCH_ENV_BUILD_OREN="$enable_env" \
...
STEADY_ENABLED_LOG="$steady_enabled_log" \
GATE_DEFAULT_LOG="$gate_default_log" \
GATE_ENABLED_LOG="$gate_enabled_log" \
...
cases = [
    ...
    ("steady enabled", os.environ["STEADY_ENABLED_LOG"], "steady"),
    ("gate default", os.environ["GATE_DEFAULT_LOG"], "gate"),
    ("gate enabled", os.environ["GATE_ENABLED_LOG"], "gate"),
]

print("arm64 fast-dot dual-accum probe summary")
print("")
...
echo "arm64 fast-dot dual-accum probe complete; summary: $summary_log"
if [[ "$smoke" == "1" ]]; then
    echo "smoke log: $smoke_log"
fi
echo "steady default log: $steady_default_log"
echo "steady enabled log: $steady_enabled_log"
echo "gate default log: $gate_default_log"
echo "gate enabled log: $gate_enabled_log"
