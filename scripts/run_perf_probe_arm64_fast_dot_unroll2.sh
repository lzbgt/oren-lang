...
summary_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-${ts}.log"
smoke_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-smoke-${ts}.log"
steady_default_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-steady-default-${ts}.run.log"
steady_disabled_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-steady-disabled-${ts}.run.log"
gate_default_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-gate-default-${ts}.run.log"
gate_disabled_log="$log_dir/perf-probe-arm64-fast-dot-unroll2-gate-disabled-${ts}.run.log"

runs="${OREN_ARM64_FAST_DOT_UNROLL2_RUNS:-5}"
warmups="${OREN_ARM64_FAST_DOT_UNROLL2_WARMUPS:-1}"
steady_n="${OREN_ARM64_FAST_DOT_UNROLL2_STEADY_N:-2000000}"
steady_reps="${OREN_ARM64_FAST_DOT_UNROLL2_STEADY_REPS:-100}"
smoke="${OREN_PERF_SMOKE_NATIVE_FAST_LOOPS:-1}"
disable_env="OREN_ARM64_FAST_LIST_INT_DOT_UNROLL2=0"
...
print("arm64 fast-dot unroll2 probe summary")
print("")
...
echo "arm64 fast-dot unroll2 probe complete; summary: $summary_log"
if [[ "$smoke" == "1" ]]; then
