...
func (t *Transpiler) transpileExpression(exp ast.Expression) (string, error) {
	...
	case *ast.FunctionLiteral:
		...
		return fmt.Sprintf("oren_closure(%s, %d, %s)", name, len(args), strings.Join(args, ", ")), nil
	...
	case *ast.SpawnExpression:
		...
		return fmt.Sprintf("oren_spawn_call_list(%s, oren_new_list(%d, %s))", fnExpr, len(args), strings.Join(args, ", ")), nil
	case *ast.CallExpression:
