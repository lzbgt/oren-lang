...
# Focused flake triage (stage1): split fairness into topology/full, full-only, short-only,
# and hogs-only slices to narrow the current rc=138 runtime corruption path.
...
# Focused runtime guard (stage1): cover the plain/workers local-ptr slices serially with
# fairness skipped by default, while leaving the mixed-mode blended slice available as triage.
