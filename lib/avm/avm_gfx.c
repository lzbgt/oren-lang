...
int avm_gfx_validate_frame(const uint8_t* data, size_t len, char* err, size_t err_cap) {
    ...
    for (uint32_t i = 0; i < op_count; i++) {
        ...
        if (opcode == 1u) {
            ...
            if (payload_len < 48u || ((payload_len - 8u) % 40u) != 0u) {
                avm_gfx_err(err, err_cap, "invalid OGF0 frame: bad mesh3d_rgba payload");
                return 0;
            }
            uint32_t triangle_count = avm_gfx_u32le(payload + 4);
            if (avm_gfx_u32le(payload) == 0u || triangle_count == 0u ||
                triangle_count != ((uint32_t)payload_len - 8u) / 40u) {
...
int avm_gfx_validate_event(const uint8_t* data, size_t len, char* err, size_t err_cap) {
    ...
    if (opcode == 112u || opcode == 113u) {
        if (payload_len != 8u) {
            avm_gfx_err(err, err_cap, "invalid OGE0 event: bad focus payload");
            return 0;
        }
        return 1;
    }
