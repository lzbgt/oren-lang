...
// Minimal SHA-256 implementation (internal use for AVM state hashing).
// Derived from the existing SHA-256 logic already present in this repo's C runtime (lib/runtime.c),
// adapted for AVM usage and streaming updates.
...
static void transform(AvmSha256Ctx* ctx, const uint8_t data[64]) {
    uint32_t m[64];
    for (int i = 0; i < 16; i++) {
