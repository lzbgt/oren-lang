...
typedef int (*AvmNetSessionReadTypedFn)(void* user_data, uint32_t session_id, size_t max_len, uint32_t payload_kind, uint32_t timeout_ms, uint8_t** out_data, size_t* out_len);
...
#define AVM_NET_SESSION_PAYLOAD_ANY 0u
...
typedef struct {
    ...
    AvmNetSessionReadTypedFn net_session_read_typed_fn;
    AvmNetSessionPollFn net_session_poll_fn;
    AvmNetSessionSelectFn net_session_select_fn;
    AvmNetSessionAcceptFn net_session_accept_fn;
