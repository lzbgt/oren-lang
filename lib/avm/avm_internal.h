...
// required so `avm_state.inc` can serialize/restore scheduler state for pause/resume.
//
// Rolling invariants:
// - Single-threaded, deterministic, no host syscalls
// - `vm->sched` is either NULL (disabled) or points to an AvmSched allocated by avm_vm.c
//
// NOTE: Any fields added here must be reflected in snapshot versioning (AVMSNAP7+).
typedef struct {
    int used;
    int done;
    int blocked;
