...
| AVM-BYTES-HOTPATH | P1 | W3 | doing | Remove legacy list-of-byte/string conversions from hot AVM app-facing APIs; keep bytes as the performance path and text conversion only at explicit boundaries. Current slices add `std:ui/avm` `text_bytes` for frame text payloads, `std:net/avm.try_get_bytes` over AVM NET op 8 for byte-native HTTP/VNET responses, direct `std:bytes.to_string` byte-slice conversion, and SHA-1/SHA-256 virtual padding over indexed byte access instead of whole-message list materialization. | `make test-avm && make verify-libavm-ios` |
