...
| NATIVE-X64-LIST-INT-FAST-SAFE | P1 | W3 | todo | Restore x64 LIST_INT sum/dot loop fast paths with safepoint-safe state: rewrite the emitters to avoid allocator-sensitive registers across internal Oren/runtime calls, re-enable the matcher gate, and prove with Arch x64 quick-integration plus focused LIST_INT fast-lowering fixtures. | `OREN_TRACE_X64_LIST_FAST=1` x64-linux build trace plus remote Arch execution |
