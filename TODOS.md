...
- LLVM-native now has a typed `%oren_llvm_bytes { len, data, owner_kind }`
  ...
  `oren_bytes_from_hex`/`oren_bytes_len`/`oren_bytes_get_u8` plus u16/u32
...
- The public LLVM lowerer script is now a small stable wrapper around
  `scripts/native_ir_llvm_lower_impl.py`; new byte-endian/slice emitters now
  live in `scripts/native_ir_llvm_bytes_helpers.py`, and moving the remaining
  older byte emitters there is the next source-line headroom cleanup.
