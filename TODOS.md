...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles, requires explicit safepoint/GC-root/runtime-helper ABI records, has `make verify-native-ir-toolchain` writing `build/native_ir/toolchain.txt`, has `lib/compiler/native_ir_v0.oren` plus `make verify-native-ir-validator` for target/function/block/terminator/helper/root/operation schema validation, has `oren dump native-ir` plus `make verify-native-ir-dump` producing validated source-operation and CFG records for `examples/hello.oren`, and has `make verify-native-ir-parity` checking linked function-surface parity plus required operation kinds and closed branch/jump targets for hello, integer arithmetic, direct calls, string concat, list ops, division panic path, and print/runtime-helper coverage across x64 Linux, x64 Windows, and ARM64 macOS without switching production backends. This host currently has Apple `clang` but no `llvm-config`/`llc` on `PATH`, so full LLVM backend work remains gated on an explicit toolchain. | Add explicit runtime-helper ABI and safepoint/root records before any LLVM object emission or default-backend migration |
