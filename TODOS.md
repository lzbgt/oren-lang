...
- `std:bytes`, `std:buffer`, `std:strings`, `std:list`, `std:linalg`,
  ...
  `std:buffer/raw` and `std:buffer/mat_u8` also use canonical names at the
  importable helper boundary.
