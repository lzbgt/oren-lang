...
| OBC-STORE-SERVICE | P1 | W4 | doing | Complete the `store.hubstack.cn` Go registry/API service. Current slice has publisher/package/release publish, signed public index, browser browse/detail/operator pages, search/download, yanking, iOS SDK install/run smoke, deploy-safe admin bearer auth, publisher-scoped bearer auth, token rotation/revocation APIs, and a documented `.obc.zip` release bundle format; remaining work is live deployment, bundle upload/download support in the service/SDK, and richer operator/browser UX. | `make verify-obc-store-service && make verify-libavm-ios` |
