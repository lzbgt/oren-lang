...
- Native WebSocket binary sends/receives now expose unfragmented opcode-2 payloads as validated byte-backed `u8_buf` values, byte-native ping/pong helpers send bounded `u8_buf` control payloads directly, and fragmented binary assembly plus ping/pong/close control receives use scratch/accumulator storage until the exact-size returned byte buffer is required; >4096-byte masked binary plus byte ping/pong plus fragmented text/binary loopback fixtures guard the byte-native path.
