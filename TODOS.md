...
| NATIVE-X64-ARCH-STAGE2-CMD | P1 | W4 | todo | Fix the dedicated Arch x64 host stage2 command crash: on 2026-07-29 the host moved to `bruce@192.168.0.102`, both the current synced tree and a clean `HEAD` baseline self-host-build `oren_stage2`, but `./oren_stage2 test examples/hello.oren --backend bytecode --platform x64-linux --no-cache` exits `139` before output. Stage1 x64 Linux bytecode hello and focused serde/visibility C/bytecode/native smokes still run, so this is tracked separately from the current traversal cleanup. | Remote Arch `./oren_stage2 test examples/hello.oren --backend bytecode --platform x64-linux --no-cache` exits `0` |
...
| NATIVE-X64-ARCH-QI | P1 | W4 | done | Fixed the Linux x64 quick-integration failures on the dedicated Arch host `bruce@192.168.3.208`. Root causes were x64 unchecked list-int reduction intrinsics clobbering allocator-sensitive registers, x64 intrinsic tracked-node lookup ignoring arena nodes, and unsafe x64 LIST_INT sum/dot fast-loop matchers around safepoints. | Remote `tests/native/test_quick_integration_native.oren` x64-linux run on `bruce@192.168.3.208` exits `0` |
