...
- Continue the iOS host-adapter SDK (`OrenAVMKit`) after the retained TIME/NET/GFX-mailbox slices. Current SDK package builds `OrenAVMKit.xcframework`, exposes deterministic and interactive app defaults, runs OBC on virtual FS/NET/PROC with stdout capture, verifies wall-clock `time.sleep_ms`, verifies allowlisted `URLSession` prefetch into VirtualNET so OBC can read the fetched body through `oren_net_get(url)`, and verifies binary GFX frame retrieval plus host-injected binary pointer events through the GFX mailbox. Next: explicit app-sandbox FileProvider, live/asynchronous network sessions if needed, compiler helper package, package store helper, iOS UIKit/Metal rendering, and broader keyboard/resize/text input helpers. Design note: `project-doc/ios_avm_sdk_design_20260531.md`.
...
- Continue the AVM graphics bridge for iOS: binary frame and input mailboxes are implemented for current `std:ui` v0 frames and pointer events; next add a host renderer adapter so Note can present frames through UIKit/Metal without hand-writing every bridge. Design note: `project-doc/avm_ios_graphics_design_20260529.md`.
