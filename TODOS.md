...
| AVM-BYTES-HOTPATH | P1 | W3 | doing | Remove legacy list-of-byte/string conversions from hot AVM app-facing APIs; keep bytes as the performance path and text conversion only at explicit boundaries. Current slices add `std:ui/avm` `text_bytes`, direct text/composition event payload string slicing, and exact-size `u8_buf` OGF0 frame encoding, `std:ui/scene3d` coordinate/face/color asset packing through exact-size `u8_buf` builders, `std:net/avm/http.get(...).bytes()` over AVM NET op 8 for byte-native HTTP/VNET responses, direct `std:bytes.to_string` byte-slice conversion, `std:buffer` u8 view/matrix byte copies through `u8_buf` instead of unpacked lists, `std:buffer` zero-copy Slice/Strided/Matrix wrapper methods for chainable view operations, SHA-1/SHA-256 virtual padding over indexed byte access instead of whole-message list materialization, and canonical codec/parser/RNG/protocol/UI/string/list/checked-cast fallible verbs or `{ok,...}` records instead of public `try_*` API names. | `make test-avm && make verify-libavm-ios` |
| NATIVE-NUMERIC-GENERICS | P1 | W3 | todo | Fix native generic-helper numeric cast semantics so helper parameters that receive float carriers can still use `oren_trunc_int` / width casts correctly without unsafe heuristics. Direct cast sugar and direct `ints.u8(1.9)` are covered; helper-wrapped `fn f(x) { ints.u8(x) }` still exposes the untagged native carrier limitation. | native/AVM cast fixtures plus `make test` |
...
- `std:bytes`, `std:buffer`, `std:strings`, `std:list`, and checked
  ...
  `strings.slice`, `strings.trim`, `list.get`, `list.slice_copy`,
  `ints.checked_u8`, `casts.checked_u64`, and
  chainable `buf.slice(1, 3).text()` / `buf.matrix(2, 3).row(1).text()`.
