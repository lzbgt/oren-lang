...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles, requires explicit safepoint/GC-root/runtime-helper ABI records, and records that this host has Apple `clang` but no `llvm-config`/`llc` on `PATH`, so LLVM toolchain detection must be explicit before backend work. | Native IR validator/dump gate, then backend parity fixtures before any default-backend migration |
