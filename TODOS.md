...
| AVM-BYTES-HOTPATH | P1 | W3 | doing | Remove legacy list-of-byte/string conversions from hot AVM app-facing APIs; keep bytes as the performance path and text conversion only at explicit boundaries. Current slices add `std:ui/avm` `text_bytes`, direct text/composition event payload string slicing, and exact-size directly-returned `u8_buf` OGF0 frame encoding with direct string-byte writes for plain `text` commands, OGE0 event decode plus `std:ui/commands` validation and `std:ui/raster` geometry/text/image payload reads over hoisted shared byte-view backing pointers, `std:ui/color` hex parsing over direct ASCII byte reads, `std:ui/scene3d` coordinate/face/color package asset packing through exact-size directly-returned `u8_buf` builders plus color hex emission through string slices, `std:ui/scene3d_binary` `.os3d` decode through hoisted byte-view backing pointers, `std:net/avm/http.get(...).bytes()` over AVM NET op 8 for byte-native HTTP/VNET responses, native `std:net/http` response reads directly into reserved response storage plus `.bytes()` over cached typed body bytes for content-length responses and exact-sized decoded chunked bytes, direct `std:bytes.to_string` byte-slice conversion, `std:bytes.from_string`, `std:strings.to_bytes`, and `std:buffer` whole-string/string-slice u8 construction plus contiguous slice/dense-matrix string-slice and dense row-string copies routed through validated byte-native runtime conversion, `std:bytes.from_hex` exact-size output writes, direct `to_hex` exact-size lowercase hex emission plus public signed 64-bit endian getters and signed checked byte-view helpers reading `u8_buf` carriers directly, with C/native runtime hex/string/slice/unpack/pack/endian-read/write helpers reading or writing list/u8-buffer backing storage directly after one validation pass, and `std:strings` byte roundtrips over byte-native u8 buffers, `std:bytes` direct u8-buffer reads for get/unpack/concat/copy sources, exact-size u8-buffer `pack`, u8/endian writes with unrolled u64 `u8_buf` stores, unrolled `std:buffer` raw i64 stores for typed-buffer builders, concat/copy u8-buffer spans, overlap-safe u8 `copy_into`, `std:buffer` view/matrix `copy_from_bytes` direct byte-carrier reads with contiguous slice/dense-matrix u8 destinations routed through `bytes.copy_into`, contiguous slice/dense-matrix byte/text exports routed through direct byte-slice conversion, raw u8 buffer loads/stores plus u8 slice/strided/matrix view loads/stores through direct pointer access after public validation, and `copy_into` u8-buffer destinations through raw pointer stores after public span checks, JSON full decode, scalar parse, tag equality, and escape output over direct source-string byte reads or exact-size `u8_buf` buffers, CBOR canonical key ordering/text encoding through growable `u8_buf` output plus byte-string and recursive/sequence decode over cached backing views with u8-backed byte carriers, and full regex pattern/text matching over direct string byte reads, YAML comment stripping, quoted-scalar parse/escape, line/trim/key split, key sort, bare-identifier, prefix, and suffix helpers without list-of-byte reconstruction, XML/HTML parser literal matching, class-selector scans, DOM parsing, and streaming readers over direct source-string byte reads without repeated input byte-list materialization, WebSocket accept hashing through direct SHA-1 UTF-8 string hashing plus upgrade header reads directly into reserved header storage, byte-native ping/pong/close control payloads, WSS byte ping/pong loopback coverage, and unmasked frame sends streaming raw payload spans, Base64/Base64URL decode/encode exact-size output writes through direct u8-buffer stores, Base64 range decoders for PEM body spans without sliced/compacted temporary strings, per-byte position maps, or a separate strict-body validation pass, PPM header/body output and software raster clear/pixel writes through raw exact-size buffer stores, native `oren_write_file` direct string syswrites, compiler source-policy scans, cache line/number parsing, bounded scan-cache load/save and delimiter writes, C-runtime include scanning, manifest JSON escaping, bytecode metadata payload writing, bytecode final artifact return as `u8_buf`, compiler SHA digest hex routing through direct runtime bytes hex, OBC linker byte reads from `u8_buf` with overflow-safe shared byte-view spans and guarded unchecked readers, parsed OBC code and byte constants kept as `u8_buf`, builder-patched linked code, linked OBC final emission as `u8_buf`, plus malformed-OBC/OBX span guards, bytecode string constants, OBX string/prefix encoding through compact byte builders, AST binary v1 full-value raw writes plus bounded u8 decode reads, native Mach-O/ELF object string payloads, runtime-object debug-name blobs, x64 native debug-table names, ARM64 native panic-message payloads, shared compiler byte-builder append/list/string/set stores, lexer string-literal decoding through compact byte builders, and C identifier escaping over direct string byte reads plus raw exact-size output writes, AVM `oren_bytes_len`/`get_u8`/`pack` parity for byte-list and optimized int-list carriers, `std:buffer` u8 matrix packs through direct writes plus u8 view/matrix byte copies through `u8_buf` instead of unpacked lists, direct string-byte copy into raw u8 buffers, slice/strided views, whole u8 matrices, strided u8 and non-dense u8 matrix exports through direct gather into exact-size output, and u8 slice/strided/matrix stores through raw pointer writes after u8-carrier checks, `std:buffer` zero-copy Slice/Strided/Matrix wrapper methods for chainable view operations, canonical `std:buffer` facade plus raw/view/core/numeric/u8-matrix helper names, checked canonical matrix projection/view helpers, SHA-1/SHA-256 virtual padding over a shared indexed byte-source view plus direct fixed-size digest `u8_buf` output through unchecked final stores, SHA-1/SHA-256 direct string hashing for UTF-8 string inputs through the same compression path, SHA hex helpers plus Windows Schannel certificate hash formatting through direct std bytes hex emission, Windows Schannel passphrase cache keys, native RNG bytes filled directly into result buffers, native IPv6 sockaddr address bytes copied directly from `u8_buf` or list backing storage, HPACK Huffman encode/decode, decoded-string boundaries, and full header-block encoding through exact-size `u8_buf` payloads or byte-slice conversion, native HTTP/2 content-length DATA bodies through exact-capacity `u8_buf` accumulation with mismatch checks, fragmented HEADERS writes through raw header-block spans across all CONTINUATION frames, and inbound single-CONTINUATION header blocks through shared exact-capacity accumulator assembly, TLS ALPN decoded-byte strings through byte-slice conversion, PEM/base64 body handling without intermediate Oren byte lists plus strict PEM body raw exact-size writes, and canonical codec/parser/RNG/protocol/UI/crypto/string/list/linalg/iter/checked-cast fallible verbs or `{ok,...}` records instead of public `try_*` API names. | `make test-avm && make verify-libavm-ios` |
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles, has `make verify-native-ir-toolchain` writing `build/native_ir/toolchain.txt`, has `lib/compiler/native_ir_v0.oren` plus `make verify-native-ir-validator` for target/function/block/terminator/helper/root/operation schema validation, has `oren dump native-ir` plus `make verify-native-ir-dump` producing validated source-operation and CFG records for `examples/hello.oren`, has explicit runtime-helper ABI/safepoint/root records for runtime builtin calls (`print`, `exit`, and `oren_*`), has backend-neutral `tagged`/`void` type layout records plus function return/value typing and runtime-helper arg/result typing, has `make verify-native-ir-parity` checking linked function-surface parity plus required operation kinds, closed branch/jump targets, helper-call mirrors, ABI-specific clobbers, call-depth mode, tagged root records, and typed helper/value surfaces for hello, integer arithmetic, direct calls, string concat, list ops, division panic path, and print/runtime-helper coverage across x64 Linux, x64 Windows, and ARM64 macOS, has reusable `scripts/native_ir_llvm_lower.py` lowering validated native IR to textual LLVM IR, has `make verify-native-ir-llvm-object` validating native IR/type metadata, lowering the typed `main` CFG subset to textual LLVM IR with real blocks, branches, local slots, constants, arithmetic/comparison ops, opaque call/container shims, and runtime-helper call markers, then writing a deterministic object-emission manifest while skipping object emission unless a full LLVM toolchain is present or failing fast when `NATIVE_IR_REQUIRE_LLVM=1`, has `make verify-native-ir-llvm-lowering` proving textual LLVM lowering over the same seven-fixture, three-platform parity matrix without switching production backends, has `oren build --backend llvm-native` plus `make verify-native-ir-llvm-backend` proving opt-in compile-only relocatable object emission with adjacent native-IR/LLVM/manifest sidecars plus explicit `oren test --backend llvm-native` rejection, has `make verify-native-ir-llvm-workflow` recording per-gate timings while avoiding the redundant trailing `make test` replay of `test-native-quick`, has `make verify-native-ir-llvm-smoke` as the fast iteration gate using two integrated object-emission fixtures instead of broad runtime replay, has `make verify-native-ir-llvm-runtime` proving linked execution of a semantic-subset CFG/arithmetic fixture against the native backend oracle on host ARM64 macOS, and has runtime-helper calls lowered with `argc` plus four argument slots, has `print` lowered to generated `oren_llvm_helper_print` semantics with string-token globals and libc `puts`, and has `exit` lowered to generated `oren_llvm_helper_exit` semantics plus libc `exit`, and lowers remaining unresolved `oren_*` helpers to deterministic named wrapper symbols such as `oren_llvm_helper_oren_string_eq(argc,arg0,arg1,arg2,arg3)` instead of the previous token-id dispatcher, while `oren_string_len` now lowers to generated `oren_llvm_helper_oren_string_len` semantics over string-token byte lengths, `oren_string_eq` lowers to generated `oren_llvm_helper_oren_string_eq` semantics for equal/unequal constant string-token branches, `oren_string_slice` lowers to generated `oren_llvm_helper_oren_string_slice` constant-token materialization that composes with length/equality helpers, unchecked `oren_string_byte_at_unchecked` plus checked/unchecked `oren_string_char_at` lower to generated constant-token access helpers, and generated string helper tables now use conservative single-assignment local-constant propagation for source/index/range arguments; `make verify-native-ir-llvm-helper-runtime` now proves print output parity without a test-only helper shim, `make verify-native-ir-llvm-exit-runtime` proves process exit-code parity against the native oracle, `make verify-native-ir-llvm-string-runtime` proves linked string-length helper execution parity, `make verify-native-ir-llvm-string-eq-runtime` proves linked string-equality helper execution parity, `make verify-native-ir-llvm-string-slice-runtime` proves linked string-slice materialization parity, and `make verify-native-ir-llvm-string-access-runtime` proves linked unchecked-byte and checked/unchecked-char access parity. This host currently has Apple `clang` on `PATH` plus Homebrew LLVM 19.1.6 under `/opt/homebrew/opt/llvm`, and the detector finds `llvm-config`/`llc` there even when Homebrew LLVM is not on `PATH`; strict LLVM mode now emits `build/native_ir/llvm_object/probe.o`, the backend path emits `build/native_ir/llvm_backend/hello_x64_linux.o` plus `div_mod_arm64_macos.o`, the runtime path emits/runs `build/native_ir/llvm_runtime/llvm_probe`, helper runtime emits/runs `build/native_ir/llvm_helper_runtime/llvm_helper_probe`, string runtime emits/runs `build/native_ir/llvm_string_runtime/llvm_string_probe`, string-equality runtime emits/runs `build/native_ir/llvm_string_eq_runtime/llvm_string_eq_probe`, string-slice runtime emits/runs `build/native_ir/llvm_string_slice_runtime/llvm_string_slice_probe`, and string-access runtime emits/runs `build/native_ir/llvm_string_access_runtime/llvm_string_access_probe`. | Extend string helper coverage beyond constant tokens, including allocator-backed dynamic string materialization and true runtime-varying helper arguments, while keeping x64/ARM64 as the oracle |
| NATIVE-X64-ARCH-STAGE2-CMD | P1 | W4 | done | Fixed the dedicated Arch x64 host stage2 command crash on `bruce@192.168.0.102`: x64 call-depth hook emission now uses serialized named `call_rel32` fixups, and the x64 runtime-object backend signature is bumped to `x64_v0_36` so stale `v0_35` rtobj seeds with unpatched `call +0` placeholders are not reused. | Remote Arch `./oren_stage2 test examples/hello.oren --backend bytecode --platform x64-linux --no-cache` exits `0` |
...
| MATH-STDLIB-EXPAND | P2 | W3 | todo | Continue deterministic `std:math` expansion toward C/C++ mathlib breadth with native and AVM fixtures per function family. Current release-gated families include core predicates/rounding/fmod/remainder/remquo/modf/sign helpers, integer rounding aliases `lround`/`llround`/`lrint`/`llrint`, sign-bit `copysign`, NaN-aware `fabs`/`fmin`/`fmax`, tolerance predicates `isclose`/`isclose_abs`, finite `lerp`, range mapping helpers `inverse_lerp`/`remap`/`remap_clamped`, graphics scalar/interpolation helpers `sign`/`step`/`fract`/`clamp`/`saturate`/`smoothstep`/`smootherstep`, angle wrapping/interpolation helpers `wrap`/`wrap_degrees`/`wrap_radians`/`angle_delta_degrees`/`angle_delta_radians`/`lerp_angle_degrees`/`lerp_angle_radians`, pure `std:math/vec2` length/length_sq/length_squared/distance/distance_sq/distance_squared/dot/cross/normalize/normalize_or_zero/angle/rotate plus add/sub/scale/lerp/midpoint/isclose_abs/isclose/abs/sign/is_nan/isnan/is_inf/isinf/is_finite/isfinite/is_subnormal/is__degrees/wrap_radians/angle_delta_degrees/angle_delta_radians/lerp_angle_degrees/lerp_angle_radians/floor/ceil/round/round_even/nearbyint/rint/lround/llround/lrint/llrint/trunc/fract/saturate/step/smoothstep/smootherstep/inverse_lerp/remap/remap_clamped/exp/exp2/exp10/expm1/log/ln/log2/log10/log1p/sin/cos/tan/sin_cos/sincos/sinh/cosh/tanh/asinh/acosh/atanh/asin/acos/atan/atan2/min/max/clamp/project/reject/reflect/faceforward/refract/closest_point_on_segment/distance_to_segment helpers, pure `std:math/vec3` length/length_sq/length_squared/distance/distance_sq/distance_squared/dot/cross/normalize/normalize_or_zero/angle-between plus add/sub/scale/lerp/midpoint/isclose_abs/isclose/abs/sign/is_nan/isnan/is_inf/isinf/is_finite/isfinite/is_subnormal/is__degrees/wrap_radians/angle_delta_degrees/angle_delta_radians/lerp_angle_degrees/lerp_angle_radians/floor/ceil/round/round_even/nearbyint/rint/lround/llround/lrint/llrint/trunc/fract/saturate/step/smoothstep/smootherstep/inverse_lerp/remap/remap_clamped/exp/exp2/exp10/expm1/log/ln/log2/log10/log1p/sin/cos/tan/sin_cos/sincos/sinh/cosh/tanh/asinh/acosh/atanh/asin/acos/atan/atan2/min/max/clamp/project/reject/reflect/faceforward/refract/closest_point_on_segment/distance_to_segment helpers, pure `std:math/vec4` length/length_sq/length_squared/distance/distance_sq/distance_squared/dot/normalize/normalize_or_zero/angle-between plus add/sub/scale/lerp/midpoint/isclose_abs/isclose/abs/sign/is_nan/isnan/is_inf/isinf/is_finite/isfinite/is_subnormal/is__degrees/wrap_radians/angle_delta_degrees/angle_delta_radians/lerp_angle_degrees/lerp_angle_radians/floor/ceil/round/round_even/nearbyint/rint/lround/llround/lrint/llrint/trunc/fract/saturate/step/smoothstep/smootherstep/inverse_lerp/remap/remap_clamped/exp/exp2/exp10/expm1/log/ln/log2/log10/log1p/sin/cos/tan/sin_cos/sincos/sinh/cosh/tanh/asinh/acosh/atanh/asin/acos/atan/atan2/min/max/clamp/project/reject/reflect/faceforward/refract/closest_point_on_segment/distance_to_segment helpers, pure finite `std:math/quat` identity/length/length_sq/length_squared/normalize/normalize_or_identity/conjugate/dot/inverse/mul/multiply/from_axis_angle/axis_angle/to_axis_angle/axis/angle/from_euler_xyz/euler_xyz/from_euler/to_euler_xyz/to_euler/euler_angles/angle_between/angular_distance/exp/log/ln/pow/power/rotation_power/scale_angle/squad/from_to_rotation/rotation_between/from_vectors/relative_rotation/difference/look_rotation/from_forward_up/look_at_rotation/from_basis_rows/from_rotation_matrix/from_mat4/rotate_vec3/rotate_vector/inverse_rotate_vec3/inverse_rotate_vector/right/up/forward/nlerp/slerp/rotate_toward helpers, pure finite row-major `std:math/mat4` identity/translation/from_translation/translate/scale/from_scale/scaling/rotation_x/rotation_y/rotation_z/from_axis_angle/rotation_axis/from_quat/from_quaternion rotation constructors, compose_trs/compose_transform/trs TRS composition, translation_of/scale_of/rotation_quat/decompose_trs/decompose_transform/decompose TRS decomposition, ortho/orthographic/frustum/perspective projection, look-at/model_view/view_projection/model-view-projection composition, mul/multiply, transpose/transposed, determinant/det, Frobenius/one/infinity norm and condition-number helpers, cofactor/adjugate/adjoint/minor helpers, inverse/invert/inverted, transform_point3/transform_point, transform_vec3/transform_vector, transform_normal3/transform_normal, project_point3/project_point, unproject_point3/unproject_point, viewport_matrix/inverse_viewport_matrix, and ndc_to_window/window_to_ndc transforms plus normal-matrix extraction, fixed and generic row/column extractors, checked cell/row/column/diagonal access and editing helpers, `col*`/`diag` aliases, diagonal reconstruction plus `from_diag`/`diag_matrix` aliases, basis/axes editing helpers, and exact/default plus tolerance-aware identity/symmetric/upper-lower-triangular/diagonal/translation/scale/uniform-scale/orthonormal/rigid-transform/invertible/singular predicates, plus overflow-aware `midpoint`, public ties-to-even rounding aliases, degree/radian angle conversion, normal/subnormal/`fpclassify` classification, component-wise ordered comparison predicates plus `signbit`/`copysign`, vector NaN-aware `fabs`/`fmin`/`fmax`, vector `fmod`/`remainder`/`remquo`, vector squared-length aliases, zero-normalize, vector `faceforward`/`refract`, vector segment projection/distance helpers, quaternion public aliases, quaternion look-rotation constructors, quaternion axis-angle decomposition, quaternion Euler decomposition, quaternion angular distance/relative rotation helpers, quaternion exp/log maps, power aliases, squad interpolation, rotation scaling/bounded rotate-toward/basis-vector helpers, quaternion basis/matrix reconstruction, mat4 TRS decomposition plus constructor/decomposition aliases and translation/basis/cell-row-column editors, ties-to-even and integer rounding aliases, `ilogb`/`logb`, `fdim`, `nextafter`/`nexttoward`/`nextup`/`nextdown`, pow/power, frexp/ldexp/scalbn decomposition and scaling, cbrt, scaled two- and three-argument hypotenuse, exp/log/ln/log2/log10/exp2/exp10 plus cancellation-aware expm1/log1p, hyperbolic sinh/cosh/tanh and inverse hyperbolic asinh/acosh/atanh, finite midpoint/isclose_abs/isclose/sqrt/cbrt/powi/pow2i/ldexp/scalbn/scalbln/frexp/ilogb/logb/modf/fdim/nextafter/nexttoward/nextup/nextdown/pow/power/hypot/hypot3/erf/erfc/degrees/radians/wrap/wrap_degrees/wrap_radians/angle_delta_degrees/angle_delta_radians/lerp_angle_degrees/lerp_angle_radians/exp/exp2/exp10/expm1/log/ln/log2/log10/log1p/sin/cos/tan/sin_cos/sincos/sinh/cosh/tanh/asinh/acosh/atanh/atan/atan2/asin/acos vector aliases for natural log and sincos, approximate real-valued erf/erfc, and a meaningful large-input Payne-Hanek trig vector at 2^40 now included in native quick; 2^53 periodicity is intentionally not asserted because `x + tau` rounds to `x + 6`, not `x + 2pi`. | focused math fixtures plus `make test-avm` |
| OREN-LSP-VSCODE | P4 | W2 | todo | Build a Go Oren language server plus VS Code extension for syntax highlighting, diagnostics, completion, navigation, and code intelligence. Current slices add a dependency-free `oren-lsp` JSON-RPC stdio server with `initialize`/`shutdown`, full-document sync, deterministic bracket/string diagnostics plus parser-backed diagnostics on open/change, clean close handling, keyword/local-symbol completion, direct imported-module alias completion, typed struct/class field completion after proven receivers, lexer-backed document symbols for imports, functions, variables, structs, and classes, same-file, opened-document cross-file, and import-resolved unopened-module definition navigation for relative and `std:` modules, lexer-backed hover and references over the same current/open/imported workspace graph, bounded transitive import traversal with cycle guards, scoped function-parameter and local-variable definition/hover/reference navigation with nested shadowing, constructor-inferred same-file and direct-import-alias struct/class field definition/hover/reference navigation with direct variable-alias propagation, direct named factory-return inference including `return if/else` expression branches plus factory-return field-chain evidence, direct typed constructor/factory expression receiver field navigation/completion plus constructor-field and constructor-bound variable/member-path alias field-chain inference/completion, homogeneous list/hash indexed receiver evidence for field navigation and direct literal completions, list `for-in` loop element receiver evidence for field navigation/references, nested element field-chain completion/navigation, call-site parameter propagation, loop-element return inference, returned-list element field-chain inference, returned-map value field-chain inference, conditional returned-list/map field-chain inference, conditional assigned returned-container field-chain inference, imported returned-container and imported parameter-returned-container field-chain evidence, returned and imported parameter-returned nested list-of-map and map-of-list container field-chain inference, nested list-of-map, map-of-list, list-of-list, and map-of-map indexed container field-chain inference, aliased nested indexed container field-chain coverage, constructor-field nested container field-chain inference, returned/imported nested container selection field-chain coverage, call-site constructor-held nested container selection coverage, indexed map value field-chain inference, and straight-line, branch-merged, returned, and imported-returned typed dotted member-path assignment propagation with conflicting, unknown, and branch-local member reassignment invalidation, recursive nested constructor-bound call-site parameter and parameter-return field-chain inference including imported identity-return call sites, source-brace scoped local/list-for-in receiver completions inside function blocks with member-completion regressions split below the source-line guard, consistent direct call-site parameter inference with conflict invalidation, direct parameter-return propagation, consistent conditional branch return and assignment inference, and unknown-assignment invalidation, conservative prepare/rename workspace edits for import aliases, exact scoped parameters/local variables, same-document and import-resolved top-level functions/types, plus same-document and import-resolved typed fields with rename server regressions split below the source-line guard, a single-parse imported-document return-fact environment builder and inferred-scope helper split below the source-line guard, full-document semantic-token classification for declarations/references/literals/operators plus parser-derived parameter and property classes, plus a VS Code extension package with `.oren` language contribution, TextMate syntax grammar, language configuration, `vscode-languageclient` activation, workspace-binary-or-PATH server fallback, and smoke verification; defer broader bulk LSP inference/provider work until Oren language features stabilize so one later pass can align tooling to the settled language. | `make verify-oren-lsp` |
| NATIVE-X64-ARCH-QI | P1 | W4 | done | Fixed the Linux x64 quick-integration failures on the dedicated Arch host now reachable at `bruce@192.168.0.102`. Root causes were x64 unchecked list-int reduction intrinsics clobbering allocator-sensitive registers, x64 intrinsic tracked-node lookup ignoring arena nodes, and unsafe x64 LIST_INT sum/dot fast-loop matchers around safepoints. | Remote `tests/native/test_quick_integration_native.oren` x64-linux run on the Arch host exits `0` |
...
| LANG-ANON-IMPORT | P3 | W2 | done | Added anonymous import syntax `import . "path"` for bootstrap and self-hosted parsers/linkers. Dot imports load dependency edges without alias-table entries, import public names unqualified under the same legacy-open/pub visibility rule, and reject collisions with local or prior anonymous-imported top-level names. | `tests/fixtures/anonymous_import/*`, native/bytecode focused smokes |
...
- Native ops plus ARM64 Mach-O/ELF, ARM64 expression/program/global/root
  ...
  literal-slot helper, while fast-path, FFI, data-table, toplevel, and
  program-entry orchestration scans now cache immutable argument, expression, module,
  ...
  matcher-term, literal-op, fresh-list, while dot/get-sum, FFI attr/import, DLL,
  runtime-offset, symbol, line, debug-table, runtime-global, top-level statement,
  wrapper, lambda, and debug-root counts before iteration, with
...
- Compiler loop-list analysis, arena, reset, and list-int lowering optimizer
  ...
  sink/rewrite, hoist/reset, reserve, candidate, touch, and rewrite traversals,
  with the byte-hotpath guard rejecting direct `oren_list_len` loop bounds in
  those passes.
- Shared optimizer helper list-touch, assignment, for-detection, and
...
- C transpiler constructor, lambda/capture analysis, list-get analysis,
  fast-while matching/emission, block, branch, call, array, function body, and
  entry-unit emission now caches immutable AST list counts before traversal, with
  byte-hotpath guard coverage rejecting direct `oren_list_len` loop bounds in
  those backend helper paths.
...
- Compiler build-cache scan-cache load/save, include-closure hashing, runtime C
...
- Compiler frontend/support help rendering, JSON key-field formatting,
  yield-lowering diagnostics/tracing, syscall policy scans, include-stack copies,
...
- Global DCE attr, expression, statement, root, and filter scans now cache
  immutable list counts before traversal, and its reachable-function worklist
  refreshes a cached queue count after each step, with byte-hotpath guard
...
- Shared native callable lowering now caches immutable statement, named-function
  ...
  byte-hotpath guard coverage rejecting direct `oren_list_len` loop bounds in
  that x64/ARM64 callable helper.
...
- X64/ARM64 ELF emitters now share `elf_artifact.oren` for common dynamic-link
  ...
  assembly, imported dynsym function entries, exported dynsym offset/row/append helpers,
  ELF64 RELA row packing, dynamic import GOT bookkeeping and relocation row
  construction, dynamic SysV hash sizing/tables,
...
- `std:math/mat4` now includes checked cofactor/adjugate/minor helpers and
  checked Frobenius/one/infinity matrix norm and condition-number helpers
  (`frobenius_norm`, `frobenius_norm_sq`, `norm_frobenius`, `norm`,
  `norm_one`, `one_norm`, `norm_inf`, `infinity_norm`,
  `condition_number_one`, `cond_one`, `condition_number_inf`, and `cond_inf`) with
...
- Oren LSP now recognizes anonymous `import . "path"` imports for workspace
  ...
  import aliases, exact scoped parameters/local variables, same-document and
  import-resolved top-level functions/types, and exact typed-member workspace
  edits for import-resolved and anonymous-imported fields.
...
- LSP/VS Code is intentionally last-priority while Oren syntax and semantics are
  still rolling; prioritize language completeness, runtime performance, AVM
  byte-hotpath work, and GUI/runtime polish before bulk LSP expansion.
...
- Scene3D flat generated-shape AVM coverage now lives in
  ...
  animation, override, group, binary, and invalid-case coverage.
...
- Legacy C runtime `oren_read_bytes` still returns the compatibility boxed byte list, but it now fills that list from bounded 64 KiB read chunks and avoids the former extra full-file temporary buffer; AVM host `oren_read_bytes` also fills its returned `LIST_INT` from bounded chunks without a full-file byte mirror; native, C runtime, and AVM host list-input `oren_write_bytes` compatibility paths validate before opening/truncating destinations and then write bounded chunks instead of allocating full-size byte mirrors, with C runtime and AVM host writes now routed through shared byte-span carrier validation/streaming helpers; AVM host and VFS `write_bytes` now accept `LIST_INT` carriers from `read_bytes` directly, VFS list/list-int writes validate byte ranges before IO charging or storage mutation and fill final VFS storage through a shared owned-data path instead of building a duplicate byte mirror, AVM and native byte/endian setters mutate optimized `LIST_INT` carriers through direct byte-span write paths, and AVM `oren_string_from_bytes` accepts optimized `LIST_INT` byte carriers directly instead of requiring boxed-list reconstruction.
- `std:fs.read_bytes` and `read_bytes_under` are now guarded as byte-native `u8_buf` aliases over the explicit `read_u8_buf` facades, while `read_byte_list` remains the named legacy list ABI for compatibility callers.
- AVM byte/string slice conversion now shares checked bytes/list/`LIST_INT` helpers for length checks, byte/endian reads, full-buffer string conversion, `bytes_to_hex`, `oren_string_from_bytes_slice`, `oren_u8_buf_from_bytes_slice`, `bytes_pack`, and `bytes_unpack`, and public `std:bytes` unsigned plus signed 16/32-bit endian getters now read `u8_buf` carriers directly after one stdlib span validation instead of bouncing through a second kernel validation. The native and legacy C runtimes now route the same byte/endian read-write, full-buffer string, `to_hex`, slice, pack, unpack, and SHA range family through shared byte-span helpers, keeping byte carriers on the optimized path without duplicated boxed-list copy loops; native whole-buffer and slice byte-to-string/u8-buffer conversions now copy `u8_buf` carriers directly after one span check, and `std:buffer` string-slice u8 construction plus exact and offset-aware contiguous string-slice/dense-row-string copies now use dedicated byte-native runtime slice conversion instead of stdlib per-byte loops. AVM HTTP/WS text facades now use whole-buffer byte-to-string conversion instead of spelling text decode as a repeated length-plus-slice conversion. The legacy C runtime byte access/endian entrypoints now live in `lib/runtime/044_byte_access.inc` instead of the list/map core shard. Native, legacy C runtime, and AVM `oren_sha256_range` now batch boxed-list/list-int inputs through bounded stack chunks filled by shared byte copy-span helpers instead of calling the SHA update routine once per byte or rejecting optimized `LIST_INT` carriers; pure Oren SHA-1/SHA-256 string hashing now shares the same internal byte-source compression path as list/u8_buf hashing instead of maintaining duplicated string-only block loops.
...
- Native WebSocket masked client sends now write the header/mask prefix with
...
- TLS ALPN protocol-list encoders now bulk-copy contiguous native string
  protocol IDs into OpenSSL/Schannel wire buffers instead of looping per byte;
  decoded negotiated ALPN strings still use byte-slice conversion.
...
- `std:buffer` u8 matrix pack helpers now write validated row-list payloads directly into fresh dense `u8_buf` storage and bulk-copy validated string rows into the same fresh dense storage; strided u8 view copies from byte carriers, `u8_buf`, and strings now validate the source span once, then write directly into strided `u8_buf` storage before falling back to checked per-element view stores; strided `u8_buf` exports and non-dense u8 matrix exports now gather directly into exact-size `u8_buf` output before string conversion.
- `std:buffer` contiguous u8 slice and dense u8 matrix copies from `u8_buf`, string, flat-list, or row-list sources now route through shared byte-span copy/direct byte-write paths, including offset-aware string-slice bulk copies into `u8_buf` slice and dense matrix views, before falling back to checked per-element view stores for strided or non-u8 carriers.
- `std:math/mat4` now exposes finite/affine/perspective predicates including tolerance-aware affine/perspective variants, exact equality, allclose tolerance predicates, exact/tolerance-aware invertible/singular predicates, basis_x/basis_y/basis_z and right/up/forward basis extractors, checked basis/axes reconstruction with optional translation vectors, checked translation editing helpers (`with_translation`, `set_translation`, `with_translation_vec`, `set_translation_vec`, `without_translation`), checked basis editing helpers (`with_basis`, `set_basis`, `with_axes`, `set_axes`, `without_basis`), fixed row0/row1/row2/row3 and column0/column1/column2/column3 extractors, checked cell/row/column/diagonal access and editing helpers (`at`, `element`, `with_cell`, `set_cell`, `with_row`, `set_row`, `with_column`, `set_column`, `with_col`, `set_col`, `diag`, `with_diagonal`, `set_diagonal`, `without_diagonal`), checked from_rows/from_columns reconstruction, from_diagonal/from_diag and diagonal_matrix/diag_matrix constructors, diagonal/trace inspection, transpose/determinant/inverse aliases, checked cofactor/adjugate/minor helpers, checked affine and rigid-transform inverse helpers, and validation, predicate, projection/look-at, transform, plus viewport/NDC conversion helpers split through focused modules to keep `mat4.oren` below the source-line guardrail, with typed bottom-row and trace checks so generic list elements compare/accumulate deterministically across native, C, bytecode, AVM, and OBC bundle probes.
...
- Scene3D generated grid-shape AVM coverage now lives in `tests/avm/test_ui_scene3d_grid_shapes_v0.oren`, reducing `test_ui_scene3d_v0.oren` to 1333 lines while keeping heightfield/surface raster checks in the default AVM and release-manifest gates.
- Scene3D generated architectural-shape AVM coverage now lives in `tests/avm/test_ui_scene3d_arch_shapes_v0.oren`, reducing `test_ui_scene3d_v0.oren` to 1027 lines while keeping wall/room/ramp/curb/post/fence/stair/roof/pyramid checks in the default AVM and release-manifest gates.
...
- Oren LSP member assignment facts now propagate straight-line, branch-merged, returned, and imported-returned typed dotted member-path writes, including direct struct fields and list/list-of-list container member paths, while conflicting, unknown, or branch-local member reassignments clear or avoid stale completion/navigation evidence.
...
- X64 named-call intrinsic/spread routing now separates statement-only
  rejection, split index/list/list-int core intrinsic dispatch,
  `oren_call_obj_list`, and spread-call lowering while preserving dispatch
  order and list/list-int runtime safety gates. The capped profile now shows
...
- Native HTTP/2 client response header-block and unknown-length DATA payload
  ...
  writes stream raw header-block spans across all CONTINUATION frames instead of
  copied split buffers,
  single-CONTINUATION inbound header blocks use the shared exact-capacity
  accumulator path without an overcapacity finish copy, `content-length` DATA
  bodies use exact-capacity `u8_buf` accumulation with mismatch checks, and header-only responses with `END_STREAM`
...
- X64 fast list/list_int dot while lowering now uses positional helper records
  ...
  short-lived keyed maps; LIST/LIST_INT dot prep, label, and checked-path state
  ...
  shape, and slot records instead of short-lived keyed maps. X64 fast LIST
  get-sum while prep, validation, dynamic-add label, loop-label, and print
  `sys_write` temp-state helpers now use compact positional records as well.
  X64 index-get operand prep plus call-object ABI/direct-call, spread,
  indirect, spawn, and varargs helpers, LCG sum while, lambda literal env, and
  native allocation kind helpers now also use positional helper records.
...
- `std:cbor` byte-string encoding now hoists shared byte-view backing pointers
  before byte emission; recursive and sequence decoding now carry one cached
  byte view data/pointer/length tuple per operation.
- `std:bytes` now exposes shared big-endian backing-aware byte-view readers, and
  CBOR decode uses them for header/additional-info reads instead of per-read view
  metadata lookups or local endian wrappers.
...
- `std:ui/commands` validation now hoists shared byte-view backing pointers for
  geometry, indexed-mesh, text-position, and image-rect fixed-width byte fields
  while retaining list-compatible byte input fallback.
- `std:ui/raster` now hoists shared byte-view backing pointers for polyline,
...
- Compiler loop-list reset and list-int lowering optimizer traversals now cache
...
- Global DCE attr, expression, statement, root, and filter scans now cache
  immutable list counts before traversal, and its reachable-function worklist
  refreshes a cached queue count after each step, with byte-hotpath guard
  coverage for direct `oren_list_len` loop conditions in that pass.
- Impl lowering trait/statement, receiver-arg insertion, function parameter,
  array/hash/call, block, and top-level traversals now cache immutable list
  counts before iteration, with byte-hotpath guard coverage for direct
  `oren_list_len` loop conditions in that semantic lowering pass.
- Optimizer fold block scans, C transpiler signature assembly, and top-level
  renamer scans now cache immutable list counts before traversal, with
...
- Type-annotation lowering module-graph, function parameter normalization,
  ...
  byte-hotpath guard coverage for direct `oren_list_len` loop conditions in
  that semantic lowering pass.
- C transpiler entry-unit collection, metadata preload, global/root registration,
  lambda prototype/body, function/type emission, and module/root top-level
...
- Repeated libavm iOS package-manifest JSON emission now routes through
...
- Legacy AVM host `oren_write_bytes` now keeps list-input compatibility but
  routes boxed-list, `LIST_INT`, and `u8_buf` carrier checks through shared AVM
  byte-span helpers before opening the destination, then streams bounded
  64 KiB stack chunks through the same helper path instead of allocating a
  full-size AVM heap byte mirror.
...
- `std:fs` now exposes explicit `read_u8_buf`/`read_u8_buf_under` facades for
  new byte-buffer callers, keeps `read_bytes`/`read_bytes_under` as guarded
  byte-native aliases over those facades, and keeps `read_byte_list` as the
  legacy list ABI.
...
- `std:math/mat4` determinant, adjugate/cofactor, inverse, condition-number,
  affine-inverse, normal-matrix, projection/look-at, point/vector/normal
  transform, and project/unproject implementations now live in focused helper
  modules while the public `mat4` facade keeps stable wrappers below the
