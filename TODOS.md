...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles and now has native-IR schema/validator/dump/parity/toolchain gates, opt-in `oren build --backend llvm-native`, fast `make verify-native-ir-llvm-smoke`, and textual LLVM lowering over the 7-fixture x 3-platform matrix. LLVM string values now use `%oren_llvm_string { len, data, owner_kind }` descriptor handles instead of token-switch helper tables: static descriptors carry `owner_kind=0`, `print`/`len`/`eq`/byte/char helpers load descriptor fields, equality uses length plus `memcmp`, `oren_string_slice_unchecked` reuses the descriptor slice body, `oren_string_char_code_at` reuses descriptor byte loads, and `slice` plus proven descriptor `+` call generated `oren_llvm_runtime_alloc_string(len)` to create heap descriptors with `owner_kind=1`. Descriptor facts now flow through a conservative native-IR CFG must-analysis, so non-constant values such as `slice_result + suffix` and `picked + suffix` after an `if` join lower through descriptor concat, and lowered safepoints push proven descriptor locals as roots so forced GC cannot reclaim a live local descriptor before later use. LLVM-native now also has a list descriptor ABI: `%oren_llvm_list { len, data, owner_kind, capacity }` backs array literals, optimized `oren_new_list_int(capacity)`, descriptor-backed `index_get`/`index_set`, and proven `oren_list*_push`, `oren_list*_len`, and `oren_list*_get` runtime helpers. The real C runtime validates list metadata and roots proven list locals across helper safepoints via `oren_llvm_runtime_roots_push_list`; `make verify-native-ir-llvm-list-runtime` forces GC between list allocation and later reads, verifies mutation through an alias, proves push growth from initial capacity, recursively marks nested list/string descriptor contents from a parent list root, carries lowerer element descriptor provenance through constant-index nested reads such as `parent[0][0] + "ed"`, and rejects opaque array/index get/set/list-int helper fallbacks. Lowered LLVM objects no longer embed libc `malloc` for string materialization; dynamic string allocation crosses real C runtime hooks in `lib/runtime/070_llvm_native_hooks.inc`, which register string bytes as GC-tracked string storage, register descriptors as tracked runtime structs, validate descriptor metadata at registration time, and now expose a v0 descriptor-root stack plus opt-in forced-GC safepoint poll that lowered helper calls use while roots are active. LLVM-native `%oren_llvm_bytes { len, data, owner_kind }` descriptors now cover hex construction, length, u8/u16/u32/u64 endian reads, signed i16/i32/i64 endian reads, u8 plus wider unsigned/signed endian writes, runtime-varying slice-copy via `oren_u8_buf_from_bytes_slice`, to-hex, bytes-slice, string-slice, pack, and unpack under forced-GC linked parity; byte helper bodies live in `scripts/native_ir_llvm_bytes_helpers.py` so the core lowerer stays below the source-line guardrail. Explicit helper roots are routed by proven descriptor kind instead of treating all tagged roots as strings. Homebrew LLVM 19.1.6 is detected under `/opt/homebrew/opt/llvm` even when not on `PATH`; strict mode emits LLVM objects on this host. | Add maps, runtime-shaped records, and broader runtime-varying descriptor inputs |
...
- LLVM-native now has a typed `%oren_llvm_bytes { len, data, owner_kind }`
  ...
  `oren_bytes_from_hex`/`oren_bytes_len`/`oren_bytes_get_u8` plus
  u16/u32/u64 endian reads, signed i16/i32/i64 endian reads,
  u8 plus wider unsigned/signed endian writes, `oren_bytes_to_hex`,
  `oren_u8_buf_from_bytes_slice`, `oren_string_from_bytes_slice`,
  `oren_bytes_pack`/`oren_bytes_unpack` helpers, type-specific bytes roots,
  and a forced-GC linked roundtrip proof via
  `make verify-native-ir-llvm-bytes-runtime`.
- The public LLVM lowerer script is now a small stable wrapper around
  `scripts/native_ir_llvm_lower_impl.py`; all LLVM-native byte helper emitters
  now live in `scripts/native_ir_llvm_bytes_helpers.py`, preserving source-line
  headroom for copy/write helper expansion.
