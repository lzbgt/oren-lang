...
- `std:math/mat4` now exposes finite/affine/perspective predicates, exact equality, allclose tolerance predicates, basis_x/basis_y/basis_z and right/up/forward basis extractors, checked basis/axes reconstruction with optional translation vectors, fixed row0/row1/row2/row3 and column0/column1/column2/column3 extractors, checked from_rows/from_columns reconstruction, from_diagonal/diagonal_matrix constructors, diagonal/trace inspection, transpose/determinant/inverse aliases, and checked rigid-transform inverse helpers, with typed bottom-row and trace checks so generic list elements compare/accumulate deterministically across native, C, bytecode, AVM, and OBC bundle probes.
