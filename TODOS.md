...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  forces safepoints after `record.name`, `record.payload`, and semantic
  slice-derived key reads, and the verifier requires LLVM roots for the
  member/dynamic-key-derived string/map locals. The lowerer also propagates map
  descriptor facts through constant-index list reads and known or derived-key
  map reads, so nested list/map descriptors avoid opaque fallback after forced
  GC. Remaining map work is broader descriptor inputs beyond known/derived keys
  to unknown runtime keys and values.
