...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  owner_kind, capacity }` descriptor ABI for empty and non-empty hash literals,
  descriptor-backed exact keys, semantic string descriptor keys using length plus
  `memcmp`, and proven `oren_map_len` calls. The real runtime registers map
  ...
  execution parity plus forced GC-at-safepoint. Native IR now lowers non-empty
  hash literals as map allocation plus explicit pair `index_set` operations, so
  LLVM sees the same map-write surface as statement-level mutation. The lowerer
  also propagates map descriptor facts through constant-index list reads and
  known-key map reads, so nested list/map descriptors avoid opaque fallback after
  forced GC. Remaining map work is broader runtime-varying descriptor inputs and
