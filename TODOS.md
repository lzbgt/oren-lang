...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  runtime dynamic string-key `index_get` dispatch when the container value kind
  is only known at runtime, so nested list/map descriptors avoid opaque read
  fallback after forced GC. Remaining map work is dynamic container
  mutation/growth for unknown descriptor values.
