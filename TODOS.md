...
| AVM-IOS-GFX-METAL | P1 | W5 | doing | Add game-grade Metal/`MTKView` GUI path. Current slice adds an SDK `OrenAVMMetalView` draw-loop/input/screen-state adapter for the full current `OGF0` drawing subset (`fill_rect`, `stroke_line`, `circle`, `text`); remaining work is retained resources, atlas/sprites, richer drawing/input records, measured frame timing budgets, and 2D/3D conformance gates. | `make verify-libavm-ios && make test-avm` |
