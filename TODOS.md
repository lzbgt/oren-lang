...
- Continue the iOS host-adapter SDK (`OrenAVMKit`) after the retained TIME/NET/GFX slices. Current SDK package builds `OrenAVMKit.xcframework`, exposes deterministic and interactive app defaults, runs OBC on virtual FS/NET/PROC with stdout capture, verifies wall-clock `time.sleep_ms`, verifies allowlisted `URLSession` prefetch into VirtualNET so OBC can read the fetched body through `oren_net_get(url)`, verifies binary GFX frame retrieval plus host-injected binary pointer/resize/key/text events through the GFX mailbox, and ships a default UIKit/CoreGraphics `OrenAVMGraphicsView` for the current `fill_rect`/`text`/`stroke_line`/`circle` subset. Next: explicit app-sandbox FileProvider, live/asynchronous network sessions if needed, compiler helper package, package store helper, and the game-grade GUI path: frame sequence/native drawable metadata, high-refresh/high-resolution pacing gates, retained resource handles, Metal/`MTKView`, richer 2D/3D rendering, and richer text input/IME helpers. Design notes: `project-doc/ios_avm_sdk_design_20260531.md`, `project-doc/avm_ui_render_performance_design_20260531.md`.
