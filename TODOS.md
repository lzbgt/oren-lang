...
- Native HTTP/2 client response header-block and unknown-length DATA payload
  accumulation now uses amortized `u8_buf` builders, `content-length` DATA
  bodies use exact-capacity `u8_buf` accumulation with mismatch checks, and
  header-only responses with `END_STREAM` terminate without waiting for a DATA frame.
