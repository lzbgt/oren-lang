...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  forces safepoints after `record.name` and `record.payload` reads, and the
  verifier requires LLVM roots for the member-derived string/map locals. The
  lowerer also propagates map descriptor facts through constant-index list reads
  and known-key map reads, so nested list/map descriptors avoid opaque fallback
  after forced GC. Remaining map work is broader runtime-varying descriptor
  inputs beyond known constant keys.
