...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  execution parity plus forced GC-at-safepoint. The lowerer also propagates map
  descriptor facts through constant-index list reads and known-key map reads, so
  nested list/map descriptors avoid opaque fallback after forced GC. Remaining
  map work is broader runtime-varying descriptor inputs and record/member
  provenance.
