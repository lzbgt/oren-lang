...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  runtime dynamic string-key `index_get`/`index_set` dispatch when the container
  value kind is only known at runtime, including runtime-backed map growth, so
  nested list/map descriptors avoid opaque read/write fallback after forced GC.
  Remaining map work is dynamic exact-key/list mutation dispatch and broader
