...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles and now has native-IR schema/validator/dump/parity/toolchain gates, opt-in `oren build --backend llvm-native`, fast `make verify-native-ir-llvm-smoke`, and textual LLVM lowering over the 7-fixture x 3-platform matrix. LLVM string values now use `%oren_llvm_string { len, data, owner_kind }` descriptor handles instead of token-switch helper tables: static descriptors carry `owner_kind=0`, `print`/`len`/`eq`/byte/char helpers load descriptor fields, equality uses length plus `memcmp`, and `slice` plus known-string `+` call generated `oren_llvm_runtime_alloc_string(len)` to create heap descriptors with `owner_kind=1`. The existing slice runtime gate compares a runtime-owned slice against a runtime-owned concat result, so fast smoke covers true runtime helper arguments without adding another broad test. Homebrew LLVM 19.1.6 is detected under `/opt/homebrew/opt/llvm` even when not on `PATH`; strict mode emits LLVM objects on this host. | Replace the generated allocator's temporary libc backing with Oren runtime-owned allocation/GC registration, then expand the descriptor ABI to non-constant program input and broader runtime helpers |
