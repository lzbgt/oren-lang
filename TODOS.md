...
- Native WebSocket binary sends/receives now expose opcode-2 payloads as validated byte-backed `u8_buf` values, with >4096-byte masked binary plus fragmented text/binary loopback fixtures guarding the byte-native path.
