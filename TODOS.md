...
- LLVM-native now has a v0 `%oren_llvm_map { len, keys, values, owner_kind,
  capacity }` descriptor ABI for empty hash literals, exact-key descriptor-backed
  `index_set`/`index_get`, and proven `oren_map_len` calls. The real runtime
  registers map descriptors/key-value storage, roots proven map locals across
  safepoints, and `make verify-native-ir-llvm-map-runtime` proves linked
  execution parity plus forced GC-at-safepoint. Remaining map work is broader
  semantic key equality, nested map descriptor provenance, runtime-shaped
