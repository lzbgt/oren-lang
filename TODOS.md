...
| NATIVE-X64-ARCH-QI | P1 | W4 | todo | Investigate the newly exposed Linux x64 quick-integration runtime crash on the dedicated Arch host `bruce@192.168.0.102`. Current evidence: simple stage1/stage2 `tests/native/print.oren` binaries and stage1 `tests/fixtures/tier1_native_result_smoke_main.oren` execute remotely with exit `0`, while stage1 `tests/native/test_quick_integration_native.oren` exits `139`; `gdb` reports SIGSEGV at `0x504f31` in the stripped static binary. | Reproduce on `bruce@192.168.0.102`, add a narrowed fixture, then include it in the x64 Linux runtime gate |
