...
| OBC-STORE-SERVICE | P1 | W4 | doing | Complete the `store.hubstack.cn` Go registry/API service. Current slice has publisher/package/release publish, signed public index, search/download, yanking, iOS SDK install/run smoke, deploy-safe admin bearer auth, and publisher-scoped bearer auth; remaining work is live deployment and token rotation/revocation hardening. | `make verify-obc-store-service && make verify-libavm-ios` |
