...
| MATH-STDLIB-EXPAND | P2 | W3 | todo | Continue deterministic `std:math` expansion toward C/C++ mathlib breadth with native and AVM fixtures per function family. Current release-gated families include pow/power plus finite trig/atan vectors; compiled C/AVM still need the huge Payne-Hanek 2^53 periodicity gap resolved. | focused math fixtures plus `make test-avm` |
