...
- `std:buffer` contiguous u8 slice and dense u8 matrix copies from `u8_buf`, string, flat-list, or row-list sources now route through shared byte-span copy/direct byte-write paths, including offset-aware string-slice bulk copies into `u8_buf` slice views, before falling back to checked per-element view stores for strided or non-u8 carriers.
