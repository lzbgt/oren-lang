...
- Continue the iOS host-adapter SDK (`OrenAVMKit`) after the retained TIME/NET slices. Current SDK package builds `OrenAVMKit.xcframework`, exposes deterministic and interactive app defaults, runs OBC on virtual FS/NET/PROC with stdout capture, verifies wall-clock `time.sleep_ms`, and verifies allowlisted `URLSession` prefetch into VirtualNET so OBC can read the fetched body through `oren_net_get(url)`. Next: explicit app-sandbox FileProvider, live/asynchronous network sessions if needed, compiler helper package, and GFX last after the mailbox exists. Design note: `project-doc/ios_avm_sdk_design_20260531.md`.
