...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  ...
  constant-index list reads and known or derived-key map reads, so nested
  list/map descriptors avoid opaque fallback after forced GC. Remaining map work
  is dynamic container operation dispatch for unknown descriptor values.
