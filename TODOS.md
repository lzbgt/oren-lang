...
| AVM-IOS-PERM-UX | P1 | W4 | doing | Package permission prompt UX handoff for host apps. Current SDK has persisted grants, runtime NET grant/revocation application, decoded OBC permission requests, host-facing `OrenAVMPermissionPrompt` metadata for native UI, prompt-based decision recording, explicit package `permission_defaults` application, and a Note-side bridge/Swift DTO handoff; remaining work is wiring the DTO into live package-run permission UI. | SDK verifier plus Note handoff verifier |
