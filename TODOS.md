...
| AVM-DESKTOP-SDK | P1 | W4 | done | Add a platform-neutral desktop `LibAVM` SDK gate. Current slice builds `build/libavm/desktop/LibAVM.xcframework` from the C embedder API for macOS arm64 and x86_64, exports public C headers/module map, symbol-checks both slices, and runs a host C embedder smoke on the local macOS architecture. | `make verify-libavm-desktop` |
