...
| OBC-STORE-SERVICE | P1 | W4 | doing | Complete the `store.hubstack.cn` Go registry/API service. Current slice has publisher/package/release publish, signed public index, search/download, and yanking; remaining work is deployment, auth hardening, and SDK end-to-end install smoke. | `make verify-obc-store-service` plus SDK end-to-end install smoke |
