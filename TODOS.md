...
- AVM byte/string slice conversion now shares one checked bytes/list/`LIST_INT` copy-span helper for full-buffer string conversion, `oren_string_from_bytes_slice`, and `oren_u8_buf_from_bytes_slice`, keeping LIST_INT byte carriers on the optimized path without duplicated boxed-list slice loops.
