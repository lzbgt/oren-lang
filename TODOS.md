...
- `std:math/mat4` now exposes finite/affine/perspective predicates plus basis_x/basis_y/basis_z and right/up/forward basis extractors, with typed bottom-row checks inside the predicate path so generic list elements compare deterministically across native, C, bytecode, AVM, and OBC bundle probes.
