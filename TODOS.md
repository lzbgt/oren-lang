...
- `std:bytes`, `std:buffer`, `std:strings`, and `std:list` now expose canonical
  fallible public verbs: `bytes.pack`, `bytes.get_u32_le`,
  `bytes.put_u32_le`, `buffer.load_i32`, `buffer.store_i32`,
  `strings.slice`, `strings.trim`, `list.get`, `list.slice_copy`, and
