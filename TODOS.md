...
| NATIVE-X64-BOXED-DOT-FAST-SAFE | P1 | W3 | todo | Rewrite and re-enable the x64 boxed-list dot-product fast loop safely. Default x64 boxed-dot matching is disabled because `benchmarks/dot_product/dot_product.oren` segfaulted on remote Arch x64 while typed `dot_product_int`, boxed/typed sum, and `fill_list_int` passed. | Remote Arch x64 boxed `dot_product` fast path exits `0` with an explicit opt-in trace gate |
