...
| AVM-IOS-PERM-UX | P1 | W4 | doing | Package permission prompt UX handoff for host apps. Current SDK has persisted grants, runtime NET grant/revocation application, decoded OBC permission requests, host-facing `OrenAVMPermissionPrompt` metadata for native UI, prompt-based decision recording, explicit package `permission_defaults` application, and a Note-side bridge/Swift DTO plus live source/OBC prompt presentation; remaining work is applying explicit user decisions back into SDK grant store/runtime policy. | SDK verifier plus Note handoff verifier |
