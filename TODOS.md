...
- `std:bytes`, `std:buffer`, `std:strings`, `std:list`, `std:linalg`,
  ...
  `std:buffer/raw`, `std:buffer/mat_core`, `std:buffer/mat_numeric`, and
  `std:buffer/mat_u8` also use canonical names at the importable helper
  boundary.
  Remaining buffer view helper `try_*` names are importable helper debt to
  collapse into canonical checked verbs.
