...
- `std:math/mat4` now exposes finite/affine/perspective predicates, exact equality, and allclose tolerance predicates, basis_x/basis_y/basis_z and right/up/forward basis extractors, fixed row0/row1/row2/row3 and column0/column1/column2/column3 extractors, checked from_rows/from_columns reconstruction, plus diagonal/trace inspection and transpose/determinant/inverse aliases, with typed bottom-row and trace checks so generic list elements compare/accumulate deterministically across native, C, bytecode, AVM, and OBC bundle probes.
