...
| MATH-STDLIB-EXPAND | P2 | W3 | todo | Continue deterministic `std:math` expansion toward C/C++ mathlib breadth with native and AVM fixtures per function family. Current release-gated families include pow/power plus finite trig/atan vectors and a meaningful large-input Payne-Hanek trig vector at 2^40; 2^53 periodicity is intentionally not asserted because `x + tau` rounds to `x + 6`, not `x + 2pi`. | focused math fixtures plus `make test-avm` |
