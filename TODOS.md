...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles and now has native-IR schema/validator/dump/parity/toolchain gates, opt-in `oren build --backend llvm-native`, fast `make verify-native-ir-llvm-smoke`, textual LLVM lowering over the 7-fixture x 3-platform matrix, real LLVM object emission through Homebrew LLVM 19.1.6, generated helper semantics for print/exit, descriptor-backed string/list/bytes values with runtime-owned allocation, descriptor-kind-specific safepoint roots, forced-GC linked parity, and a map descriptor ABI for empty/non-empty hash literals, map-shaped struct constructor records, member get/set lowering, member-derived descriptor root provenance, exact keys, semantic string descriptor keys including constant-derived slice keys, generic runtime-validated descriptor roots for unknown helper inputs, dynamic string-key index-get/index-set dispatch, dynamic exact-key map index-set dispatch, dynamic integer list index-set dispatch, runtime-backed map growth, get/set/len helpers, key-kind sidecar metadata, and nested map provenance through lists/maps. Detailed rolling evidence is kept in the current-done bullets below and the native-IR LLVM plan. | Add broader runtime-shaped descriptor inputs and dynamic container helper calls beyond index ops |
...
- LLVM-native now has a `%oren_llvm_map { len, keys, values, key_kinds,
  owner_kind, capacity }` descriptor ABI for empty/non-empty hash literals and
  map-shaped struct constructor records. Native IR lowers non-empty hash literal
  pairs and struct constructor fields as explicit `index_set` operations, lowers
  member get/set as string-key map operations, and keeps descriptor-backed exact
  keys, semantic string descriptor keys using length plus `memcmp`, and proven
  `oren_map_len` calls on generated helpers. The real runtime registers map
  descriptors/key-value/key-kind storage, roots proven map locals across
  safepoints, and `make verify-native-ir-llvm-map-runtime` proves linked
  execution parity plus forced GC-at-safepoint. The focused map fixture now
  forces safepoints after `record.name`, `record.payload`, semantic
  slice-derived key reads, and an unknown descriptor helper input read through a
  branch-merged string key; the verifier requires LLVM roots for the
  member/dynamic-key-derived string/map locals plus a runtime-validated generic
  value root. The lowerer also propagates map descriptor facts through
  constant-index list reads and known or derived-key map reads, and falls back to
  runtime dynamic string-key `index_get`/`index_set`, exact-key map
  `index_set`, and integer list `index_set` dispatch when the container value
  kind is only known at runtime, including runtime-backed map growth, so nested
  list/map descriptors avoid opaque read/write fallback after forced GC.
  Remaining work is broader runtime-shaped descriptor inputs and dynamic
  container helper calls beyond direct index ops.
