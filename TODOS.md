...
- `std:buffer` contiguous u8 slice copies from `u8_buf` now route through the shared byte-span `bytes.copy_into` path before falling back to checked per-element view stores for strided or non-u8 carriers.
