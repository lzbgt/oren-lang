...
| OBC-STORE-SERVICE | P1 | W4 | doing | Complete the `store.hubstack.cn` Go registry/API service. Current slice has publisher/package/release publish, signed public index, search/download, yanking, and iOS SDK install/run smoke against the service; remaining work is deployment and auth hardening. | `make verify-obc-store-service && make verify-libavm-ios` |
