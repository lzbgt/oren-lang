...
| NATIVE-IR-LLVM | P1 | W4 | doing | Formalize a backend-neutral native IR before introducing LLVM as an optional native backend. Current plan in `project-doc/native_ir_llvm_backend_plan_20260730.md` keeps x64/ARM64 emitters as correctness oracles, requires explicit safepoint/GC-root/runtime-helper ABI records, and now has `make verify-native-ir-toolchain` writing `build/native_ir/toolchain.txt`; this host currently has Apple `clang` but no `llvm-config`/`llc` on `PATH`, so full LLVM backend work remains gated on an explicit toolchain. | Native IR validator/dump gate, then backend parity fixtures before any default-backend migration |
