...
- `std:math` now includes deterministic tolerance, graphics scalar/interpolation, and range-mapping
  ...
  helpers for common 2D graphics/vector operations including component rounding, interpolation, and range-mapping helpers,
  pure finite `std:math/vec3` helpers for common 3D vector operations including component rounding, interpolation, and range-mapping helpers, and pure finite
  `std:math/vec4` helpers for homogeneous/vector algebra including component rounding, interpolation, and range-mapping helpers, backed by native/C/
