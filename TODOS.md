...
- Native WebSocket binary sends/receives now expose opcode-2 payloads as validated byte-backed `u8_buf` values, while `recv_bytes` handles ping/pong/close control payloads through scratch storage instead of managed byte-return buffers; >4096-byte masked binary plus fragmented text/binary loopback fixtures guard the byte-native path.
