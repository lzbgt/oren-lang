...
- Latest packed-bridge probe baseline leg (2026-03-20, optimized script path) reconfirmed the current steady baseline at `array_sum_int` ~2.43× C and `dot_product_int` ~3.11× C before entering the hidden full-runtime packed warm build. The remaining iteration cost is now concentrated in that first full-runtime native build, not in repeated rebuilds across probe cases.
