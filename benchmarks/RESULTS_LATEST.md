...
- New guardrail: `make perf-smoke-list-int` now builds the native `array_sum_int` / `dot_product_int` binaries and checks their tiny known-good outputs (`205` and `6590`) before heavier timing sweeps. This was added after a 2026-03-20 arm64 exact-dot dual-accumulator experiment returned the wrong `dot_product_int` smoke output (`4621`), so future dot-core work should pass this smoke before trusting any perf numbers.
