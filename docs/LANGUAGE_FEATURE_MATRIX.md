...
| FFI symbols (`ffi name`) | Rolling (native: macOS+Windows; linux x64+arm64 w/ `--link`; linux w/out `--link` panics-on-call) | Parser surface: `lib/compiler/parser_parse/**`; Native emit: arm64 Mach‑O stubs (`lib/compiler/arm64_macho.oren`), x64 Windows lazy stubs (`lib/compiler/x64_native_program/072_ffi.oren` + `lib/compiler/x64_pe.oren`); Linux dynamic ELF (`lib/compiler/x64_elf.oren`, `lib/compiler/arm64_elf.oren`) + `dlsym`-based resolver (`lib/compiler/x64_native_program/072_ffi.oren`, `lib/compiler/arm64_elf.oren`) | Examples: `examples/ffi_test.oren`; Windows smoke: `tests/native/ffi_windows_kernel32.oren` (remote Win11 gate via `scripts/verify_native_matrix.sh --targets x64-win`; compile-only gate also checks `--link msvcrt.dll` propagation via `scripts/verify_native_x64_compile_only.sh`); Linux OK contract (`--link`): `tests/native/ffi_linux_strlen_ok.oren` (linux/arm64 docker via `scripts/verify_native_matrix.sh --targets arm64-linux`; linux/x64 remote WSL2 via `scripts/verify_native_matrix.sh --targets x64-wsl`) (stage1 + stage2); Linux panic contract (arm64 + x64 w/out `--link`): `tests/native/ffi_linux_unresolved_panics.oren` |
...
| HTTP/1.1 GET stdlib (Content-Length + chunked, IPv4 + hostname via DNS A) | Rolling | Stdlib: `lib/std/net/http.oren` (built on `std:net/tcp`); hostname support via explicit resolver injection (`http.get_text_resolver` / `http.get_response_resolver`) or system `dns.default_resolver`; structured response API returns `{status, headers, body}`; no TLS/keep-alive pooling yet (parser recognizes `https://` but errors until TLS lands; see `docs/NET_TLS.md`) | Loopback regression: `tests/native/test_http_get_loopback.oren` (includes hostname path + status/header assertions); Tier‑1 cross‑arch gate: `scripts/verify_native_net_matrix.sh` |
| WebSocket v0 stdlib (ws:// handshake + masked text frames; ping/pong; hostname via DNS A) | Rolling | Stdlib: `lib/std/net/ws.oren` (built on `std:net/tcp`, `std:crypto/sha1`, `std:encoding/base64`, `std:crypto/rand`); hostname support via explicit resolver injection (`ws.connect_resolver`) or POSIX `dns.default_resolver`; no TLS/streaming/fragmentation yet (parser recognizes `wss://` but errors until TLS lands; see `docs/NET_TLS.md`) | Loopback regression: `tests/native/test_ws_echo_loopback.oren` (includes hostname path + ping/pong); Tier‑1 cross‑arch gate: `scripts/verify_native_net_matrix.sh` |
