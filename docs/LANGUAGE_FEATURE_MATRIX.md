...
| Deterministic map iteration | Rolling | C runtime keeps keys sorted; native runtime sorts lazily on iteration; x86_64 native maintains entries sorted on insert (Tier‑1 bring-up) | Tests: `tests/native/test_integration_suite.oren` (map iteration); Tier‑1 x86_64: `tests/fixtures/tier1_native_map_dynamic_keykind_main.oren` (`OREN_REMOTE_RUN=1`); Runtime: `lib/runtime/040_lists_maps.inc`, `lib/runtime_native/130_printing.oren` |
