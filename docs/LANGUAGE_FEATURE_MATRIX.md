...
| FFI symbols (`ffi name`) | Rolling (native: macOS+Windows; Linux stubbed) | Parser surface: `lib/compiler/parser_parse/**`; Native emit: arm64 Mach‑O stubs (`lib/compiler/arm64_macho.oren`), x64 Windows lazy stubs (`lib/compiler/x64_native_program/072_ffi.oren` + `lib/compiler/x64_pe.oren`); Linux native stubs unresolved imports (`lib/compiler/arm64_elf.oren`) | Examples: `examples/ffi_test.oren`; Native test: `tests/native/ffi.oren`; Windows smoke: `tests/native/ffi_windows_kernel32.oren` (remote Win11 gate via `scripts/verify_native_matrix.sh --targets x64-win`; compile-only gate also checks `--link msvcrt.dll` propagation via `scripts/verify_native_x64_compile_only.sh`) |
