...
| HTTP/1.1 GET stdlib (Content-Length + chunked, IP-only) | Rolling | Stdlib: `lib/std/net/http.oren` (built on `std:net/tcp`); no TLS/DNS/keep-alive pooling yet | Loopback regression: `tests/native/test_http_get_loopback.oren` (uses `spawn` for cross-OS) |
