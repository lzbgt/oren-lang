...
| WebSocket v0 stdlib (ws:// handshake + masked text frames) | Rolling | Stdlib: `lib/std/net/ws.oren` (built on `std:net/tcp`, `std:crypto/sha1`, `std:encoding/base64`); no TLS/DNS/streaming/ping-pong yet | Loopback regression: `tests/native/test_ws_echo_loopback.oren`; Tier‑1 cross‑arch gate: `scripts/verify_native_net_matrix.sh` |
