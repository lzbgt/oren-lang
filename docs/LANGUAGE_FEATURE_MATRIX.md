...
| FFI symbols (`ffi name`) | Rolling (native: macOS+Windows; x64-linux w/ `--link`; arm64-linux panics-on-call) | Parser surface: `lib/compiler/parser_parse/**`; Native emit: arm64 Mach‑O stubs (`lib/compiler/arm64_macho.oren`), x64 Windows lazy stubs (`lib/compiler/x64_native_program/072_ffi.oren` + `lib/compiler/x64_pe.oren`); x64 Linux dynamic ELF (`lib/compiler/x64_elf.oren`) + `dlsym`-based resolver (`lib/compiler/x64_native_program/072_ffi.oren`); arm64 Linux unresolved-import stubs still panic (`lib/compiler/arm64_elf.oren`) | Examples: `examples/ffi_test.oren`; Windows smoke: `tests/native/ffi_windows_kernel32.oren` (remote Win11 gate via `scripts/verify_native_matrix.sh --targets x64-win`; compile-only gate also checks `--link msvcrt.dll` propagation via `scripts/verify_native_x64_compile_only.sh`); Linux x64 gate: `tests/native/ffi_linux_strlen_ok.oren` (remote WSL2 via `scripts/verify_native_matrix.sh --targets x64-wsl`); Linux panic contract (arm64 + x64 w/out `--link`): `tests/native/ffi_linux_unresolved_panics.oren` |
