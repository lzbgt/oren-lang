...
| DNS v0 stdlib (UDP A query; explicit server, no system resolver yet) | Rolling | Stdlib: `lib/std/net/dns.oren` (built on `std:net/udp`, OS entropy via `std:crypto/rand` for txid); no TCP fallback, no AAAA, no /etc/resolv.conf yet | Loopback regression: `tests/native/test_dns_loopback.oren`; Tier‑1 cross‑arch gate: `scripts/verify_native_net_matrix.sh` |
| HTTP/1.1 GET stdlib (Content-Length + chunked, IP-only) | Rolling | Stdlib: `lib/std/net/http.oren` (built on `std:net/tcp`); no TLS/DNS/keep-alive pooling yet | Loopback regression: `tests/native/test_http_get_loopback.oren`; Tier‑1 cross‑arch gate: `scripts/verify_native_net_matrix.sh` |
