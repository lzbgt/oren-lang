...
- Symptom (native Win11 bring-up; stage2 compiler built by stage1):
  - `oren_stage2.exe build examples\\myapp.oren --backend native --platform x64-windows` failed with:
    - `x64 pe: failed to write: build/targets/x64-windows/native/myapp`
    - `write_bytes: sys_open failed`
  - Root observation: `build/targets/...` did not exist after the build attempt, even though the compiler calls `ensure_dir(dirname(out_path))`.

- Root causes:
  - Windows `oren_system("mkdir ... >nul 2>nul")` did not actually create directories under the native runtime because `cmd.exe` needs *raw* shell semantics.
    - The old implementation routed Windows `system()` through argv → CRT quoting → CreateProcessA, which produced `\"` in the raw command line.
    - `cmd.exe` can re-parse the raw command line, and CRT-style escaping breaks quoting/redirection in practice.
  - x64-windows native backend FS intrinsics historically treated '/' paths inconsistently across Win32 APIs during bring-up.

- Fixes:
  - Native runtime (`lib/runtime_native/260_threads.oren`):
    - `oren_system_timeout` (Windows branch) now invokes `cmd.exe /C <cmd>` by constructing a raw command line string and calling `sys_win_createprocess` directly (no argv re-escaping).
  - x64-windows backend (`lib/compiler/x64_native_program/046_emit_sys_intrinsics_windows*.oren`):
    - Normalize `'/'` → `'\\'` in Win32 FS intrinsic boundaries (CreateFileA/DeleteFileA/MoveFileExA/CreateDirectoryA) via stack scratch buffers.
  - C backend (Windows host):
