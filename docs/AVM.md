# AVM + OBC (Spec + Roadmap + Distribution)

**Last updated:** 2026-02-19

This file merges the AVM spec, the next‑gen AVM roadmap, and OBC distribution guidance into a
single reference for bytecode semantics, VM evolution, and portability/signing details.

---

# AVM Specification (Rolling)

This document defines the bootstrap AVM specification and instruction set.

For the next-gen plan and design direction, see `docs/AVM.md`.
For OBC portability/linking and signing, see `docs/AVM.md`.
...
- See `docs/AVM.md#avm-neon-mapping-plan-arm64-no-jit-first` for the concrete mapping plan and determinism constraints.
...
- swarm consensus (`docs/AVM.md#avm-swarm-consensus-agent-mobility-design-validation`)
...
- **OBX**: linker metadata embedded as an unused `BYTES` constant inside `.obc` (exports + relocs). AVM ignores it at runtime. See `docs/AVM.md`.
...
- Compile-time linking exists via OBX (`docs/AVM.md`).
...
- `docs/AVM.md#avm-in-avm-multiverse-design-nested-virtual-universes` (compiler-in-AVM section)
- `docs/AVM.md` (precompiled stdlib bundle + relocations)
- `docs/RUNTIME.md` (stdlib specifiers + distribution models)
...
- Nested universes (“AVM in AVM”): `docs/AVM.md#avm-in-avm-multiverse-design-nested-virtual-universes`
...
1) Freeze the ABI nucleus list in `docs/AVM.md` (names + arg order + return convention).
...
The script uses the existing ssh proxy workflow (see `docs/TOOLCHAIN_PLATFORMS.md`) and runs AVM inside
