...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Next: varargs (`...rest`) lowering + spread semantics, including efficient list packing and tail-call-safe wrapper stubs (x64 now supports named varargs calls without spread; see `tests/fixtures/x64_varargs_main.oren`).
