...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Next: implement map index set `m[key]=v` on x64 (lowering to `oren_map_set`), then allow richer key/value types and deterministic iteration order rules.
