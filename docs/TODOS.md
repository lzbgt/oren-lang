...
- **Legacy Makefile runner deprecated**: `make test-legacy` now aliases `./oretest --full` (parallel, curated); the old sequential shell runner is kept only as `make test-legacy-old` for reference while rolling.
