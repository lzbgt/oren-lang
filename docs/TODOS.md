...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Status: x64 bring-up now includes syscall-first `malloc`/`malloc_raw` + `ptr_get`/`ptr_set` (qword + byte) intrinsics, guarded by `tests/fixtures/x64_malloc_ptr_main.oren` (remote-run on Win11+WSL2).
