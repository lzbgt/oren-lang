...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Status: x64 bring-up now includes syscall/WinAPI-backed `malloc`/`malloc_raw` + `ptr_get`/`ptr_set` (qword + byte) intrinsics, minimal list intrinsics (`oren_new_list`, `oren_list_len`, `oren_list_push`, `oren_list_get`, `oren_list_set`), list literal `[a,b,c]`, list index sugar (`xs[i]`, `xs[i]=v` via `oren_index_set`), stdlib-style namespace calls (`import list "std:list"; list.len/list.push`), `"string literal"` values as raw pointers (usable with `ptr_get_byte`/`iadd`), plus native-layout-compatible map intrinsics (`oren_new_map`, `oren_map_len`, `oren_map_get`, `oren_map_set`), map literal `{ "k": v }`, map indexing `m[key]`, and map index assignment `m[key]=v` (string keys; v0 best-effort string equality) guarded by `tests/fixtures/x64_malloc_ptr_main.oren`, `tests/fixtures/x64_list_ops_main.oren`, `tests/fixtures/x64_array_literal_main.oren`, `tests/fixtures/x64_std_list_namespace_main.oren`, `tests/fixtures/x64_string_literal_bytes_main.oren`, `tests/fixtures/x64_map_literal_main.oren`, `tests/fixtures/x64_map_index_get_main.oren`, and `tests/fixtures/x64_map_index_set_main.oren` (remote-run on Win11+WSL2).
   ...
   - Next: converge map semantics with canonical ordering rules (deterministic iteration) and expand key/value support beyond string keys in x64 v0 (align with AVM).
