...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Status: x64 bring-up now includes syscall/WinAPI-backed `malloc`/`malloc_raw` + `ptr_get`/`ptr_set` (qword + byte) intrinsics, minimal list intrinsics (`oren_new_list`, `oren_list_len`, `oren_list_push`, `oren_list_get`, `oren_list_set`), list literal `[a,b,c]` (lowered via direct buffer fill so nested literals like `[0,1,[0,0]]` are correct), list index sugar (`xs[i]`, `xs[i]=v` via `oren_index_set`), `for x in xs {}` via `oren_iter_next(container, idx, out_pair) -> [ok, value]`, stdlib-style namespace calls (`import list "std:list"; list.len/list.push`), `"string literal"` values as raw pointers (usable with `ptr_get_byte`/`iadd`), plus native-layout-compatible map intrinsics (`oren_new_map`, `oren_map_len`, `oren_map_get`, `oren_map_set`), map literal `{ "k": v }` (string or small-int keys), **nested map literal** `{ "a": {"b": 7} }` (requires re-entrant intrinsic arg spilling), map indexing `m[key]`, and map index assignment `m[key]=v` with a v0 key-kind heuristic (`key < 4096 => int`, else C-string pointer) guarded by `tests/fixtures/x64_malloc_ptr_main.oren`, `tests/fixtures/x64_list_ops_main.oren`, `tests/fixtures/x64_array_literal_main.oren`, `tests/fixtures/x64_forin_list_main.oren`, `tests/fixtures/x64_std_list_namespace_main.oren`, `tests/fixtures/x64_string_literal_bytes_main.oren`, `tests/fixtures/x64_map_literal_main.oren`, `tests/fixtures/x64_nested_map_literal_main.oren`, `tests/fixtures/x64_map_index_get_main.oren`, `tests/fixtures/x64_map_index_set_main.oren`, and `tests/fixtures/x64_map_int_key_main.oren` (remote-run on Win11+WSL2).
   ...
   - Next: finish x64 intrinsic arg evaluation re-entrancy beyond map ops (remove hidden dependence on `$tmp1..$tmp8` across nested expression evaluation; migrate remaining intrinsics like list ops to the temp allocator), so nested intrinsic calls are always correct.
