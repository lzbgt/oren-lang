...
1) **ARM64 instruction encoder audit** (S)
   - Status: added a native golden-encoding test that asserts key `arm64_core.oren` encoders produce the documented clang-verified constants (LDR/STR/LDRSW + a few SIMD ops).
