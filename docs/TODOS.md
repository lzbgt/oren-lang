...
1) **Native backend Tier‑1: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Next: implement x64 hash/map literal lowering (or runtime-backed map intrinsics) so iterable maps (`std:iter.range`, `std:list.slice_view`) can work end-to-end.
