...
1) **Tier‑1 native backend: x86_64 (Linux ELF + Windows PE)** (L)
   ...
   - Status (rolling):
     ...
     - ✅ x64 `STACK_TRACE` now symbolicates to best-effort function names via an embedded symtab (fixed-base emitters)
     - ⏭️ next: richer symbolication (addr→fn+offset, fn+line mapping), richer `OREN_DIAG` parity with `lib/runtime_native/110_mem_diag.oren`, and closure perf (avoid per-call `args_list` allocations for common cases)
