...
2) **Native backend x86_64 (Linux ELF + Windows PE)** (L) (Tier 1 target)
   ...
   - Status: x64 lowering supports truthy conditions (`if <expr> { ... }`, `while <expr> { ... }` where `<expr>` is an int expression) by lowering to `cmp rax, 0` + Jcc; oretest builds both targets (`x64_if_truthy_main.oren`, `x64_while_truthy_main.oren`).
