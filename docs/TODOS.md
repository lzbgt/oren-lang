...
3) **W4 — List push/len fast paths + reserve** (M)
   ...
   - 2026-02-19: optimizer inserts `oren_list_reserve(list, n)` before `while i < n` push loops when `n` is a known int literal and `list` was created via `[]`.
     - Conservative guards: skips when earlier control-flow in the same block assigns to `list` or `n` (avoid new runtime panics).
     - Remaining: broader lowering (non-literal `n`), list_int reserve wiring, and safe `oren_list_push_unchecked` routing.
...
1) **Keep native backend bounded + predictable (perf + stability)** (L)
				     ...
				     - Reuse-on now gets past map tracking but still fails quick integration with `list_len on non-list` (select path) — keep gated.
				       ...
				       - Still failing (reuse-on): list magic overwritten with list pointer in `oren_select` (`test_quick_integration_native`):
				         ...
				         - Next steps: verify GC pin consumers (compiler/serde/renamer), audit select + netpoll pointer roots, and validate stack/register spill discipline at safepoints.
                           ...
                           - 2026-02-19: arm64 fast LCG sum loop keeps i/x/sum in registers and only spills at loop exit:
                             - Compiler: `lib/compiler/arm64_native_stmt.oren`.
                             - Bench (loop_sum, M2 Pro, runs=5): C 0.066657s, Oren C 0.060621s (~0.91×), Oren native 0.422603s (~6.34×), OBC 0.098458s (~1.48×).
                             - Artifact: `benchmarks/results/loop_sum_darwin_arm64_20260219_162414.md`.
                           - 2026-02-19: x64 fast LCG sum loop keeps i/x/sum/i_mod in registers and only spills on safepoint or loop exit:
                           ...
                           - 2026-02-19: AVM `INT_LCG_SUM_LOOP` unrolled by 4 in the fast-sum path to reduce branch overhead:
                           ...
                           - 2026-02-19: AVM `INT_LCG_SUM_LOOP` adds a 32-bit modulus fast path when operands fit `uint32` (keeps exact semantics for non-negative inputs):
                             - AVM: `lib/avm/avm_vm.c`.
                             - Bench (loop_sum, M2 Pro, runs=5): C 0.065802s, Oren C 0.060236s (~0.92×), Oren native 0.418437s (~6.36×), OBC 0.097881s (~1.49×).
                             - Artifact: `benchmarks/results/loop_sum_darwin_arm64_20260219_165555.md`.
                           - 2026-02-19: AVM builds now default to `-O3` (`AVM_CFLAGS`) to tighten OBC hot loops:
  ...
  Perf gaps from latest M2 benchmarks (2026-02-19, runs=3, RSS on unless noted):
  ...
  - (P1/M) **Loop_sum native still ~6.36× C; Oren C ~0.91×; OBC ~1.40×**
    - Latest (runs=5, warmups=1): C 0.065980s, Oren C 0.060040s (~0.91×), native 0.419561s (~6.36×), OBC 0.092398s (~1.40×) (`benchmarks/results/loop_sum_darwin_arm64_20260219_170217.md`).
    ...
    - Target: native ≤0.25s (≤4× C) and OBC ≤0.08s (≤1.2× C) while keeping correctness gates.
  ...
  - (P2/M) **multi_list_sum boxed list reads now ~2.1× C (Oren C) / ~3.7× native; OBC ~1.89×**
    - Latest (runs=5): C 0.008349s, Oren C 0.017322s (~2.07×), Oren native 0.030751s (~3.68×), OBC 0.015799s (~1.89×).
      - Result: `benchmarks/results/multi_list_sum_darwin_arm64_20260219_130703.md`
...
2) **Close list<int> perf gap vs C (M)**
  ...
  - 2026-02-19: bytecode now emits list_int push loop opcodes for `list.push` when RHS is a provably non-negative int expression (boxed list fill loops).
    ...
    - Impact: multi_list_sum OBC ~1.89× C; array_sum OBC ~2.41× C; dot_product OBC ~2.36× C (latest artifacts).
