...
3) **Backend architecture unification (CoreIR + canonical runtime ABI)** (L)
   ...
   - Next: close parity gaps exposed by fixtures (e.g. bytecode backend still rejects varargs+spread inside `spawn`; see `tests/native/fixtures/bytecode_codegen_error.oren`).
