...
2) **Native backend x86_64 (Linux ELF + Windows PE)** (L) (Tier 1 target)
   ...
   - Status: x64 lowering supports integer division and modulo: `/` and `%` (`x64_div_mod_main.oren`).
   ...
   - Status: added an opt-in remote-run oretest smoke (Win11 cmd.exe + WSL2) behind `OREN_REMOTE_RUN=1` to validate actual stdout + exit code on real x86_64 (now includes `div/mod`, `bit/shift`, and `&&/||` short-circuit exitcode checks).
