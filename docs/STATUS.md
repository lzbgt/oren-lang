...
- `std:math/mat4` now includes tolerance-aware affine/perspective predicates
  ...
  by shared row-major matrix validation.
- `std:math/mat4` now includes checked Frobenius, one-norm, and infinity-norm
  matrix helpers (`frobenius_norm`, `frobenius_norm_sq`, `norm_frobenius`,
  `norm`, `norm_one`, `one_norm`, `norm_inf`, and `infinity_norm`) with native/C/bytecode,
  AVM, and stdlib OBC bundle coverage.
