...
- **list<int> hot-loop parity**: the latest focused one-shot rerun still sits around `array_sum_int` 2.07×, `dot_product_int` 2.59×, and `multi_list_push_int` 2.24× vs C, while the latest steady-state runner remains above target (`array_sum_int` ~2.43× steady, `dot_product_int` ~2.78× steady vs C on arm64, 2026-04-04). The 2026-04-08 fast dot-ceiling rerun narrowed the explicit helper/bridge picture materially: canonical `dot_product_int` now measured `~1.2169× C`, the hidden direct-slot helper measured `~1.1182× C`, and the packed bridge dropped to `~4.9387× C` SIMD / `~17.0948× C` scalar. The later reuse-work read-split rerun still leaves the best explicit packed path materially behind (`~4.4566× C` pack-once SIMD long-per-rep; `~7.2240× C` reuse-work long-per-rep; `~7.3906× C` fresh-pack SIMD long-per-rep vs canonical `~1.2915× C`), so the remaining blocker is repeated bridge materialization/copy itself, not just fresh allocation or the packed dot kernel. The new direct-slot read-split rerun (`build/logs/perf-probe-list-int-slot-direct-read-split-20260408_235243_30345.log`) also confirms the hidden helper can beat the shipped whole-operation path on the same split (`array_sum_int_slot_direct` `~1.0383× C` vs canonical `~1.3410× C`; `dot_product_int_slot_direct` `~1.1637× C` vs canonical `~1.2680× C`), while the delta side is still too noisy to rank. The 2026-04-09 shared slot-direct follow-up promoted that ceiling into a public cross-backend stdlib surface: `linalg.reduce_sum_i64_list_int_slots(...)` / `linalg.dot_i64_list_int_slots(...)` now fast-path through backend-native all-int list detection and direct-slot helpers on C/native/AVM, while falling back to a portable scalar list walk for generic list inputs. `make verify-backend-parity-list-int` now exercises that public surface in addition to the raw helper contracts. The earlier steady ceiling snapshot (`build/logs/perf-probe-list-int-dot-ceiling-20260409_113946_99659.log`) ranked canonical `dot_product_int` at `~1.3221× C`, hidden direct-slot helper at `~1.0920× C`, and public-slot `std:linalg` at `~1.2079× C`; `array_sum_int` came back as canonical `~1.4110× C`, hidden helper `~1.0019× C`, and public-slot `~1.0800× C`. Same-day follow-up compiler work then converged arm64 list fast-loop validation onto the same alloc-index lookup x64 already uses (`native_alloc_index_get` instead of the older `oren_find_node` path in `lib/compiler/arm64_native_stmt_loops_list_emit.oren`). Two serialized light ceiling reruns after that change both moved the canonical path materially in the right direction (`build/logs/perf-probe-list-int-dot-ceiling-20260409_115936_33151.log` and `build/logs/perf-probe-list-int-dot-ceiling-20260409_120228_38413.log`), but still flipped direct-slot/public ordering. The new order-balanced stability probe, `build/logs/perf-probe-list-int-dot-ceiling-stability-20260409_123207_90132.log`, is now the stronger decision surface: median `array_sum_int` ratios came back as canonical `~1.1887× C`, direct-slot `~1.2329× C`, and public-slot `~1.2729× C`, while median `dot_product_int` ratios came back as canonical `~1.2177× C`, public-slot `~1.2231× C`, and direct-slot `~1.3097× C`. Rank counts on that same 5-sweep probe were canonical `2/5` wins on `array_sum_int` and `3/5` wins on `dot_product_int`, public-slot `1/5` wins on `array_sum_int` and `2/5` wins on `dot_product_int`, and direct-slot `2/5` wins on `array_sum_int` but `0/5` wins on `dot_product_int`. The production-quality fact is now different from the older light-probe story: the alloc-index convergence did not regress parity, the stronger repeated surface currently ranks the shipped canonical arm64 path first by median on both whole-operation benchmarks, and neither the public-slot nor hidden-helper surfaces are stable whole-operation winners on current `master`. The packed bridge stays far behind, the direct public reroute through internal `oren_list_int_*_slots_try_fast(...)` remains a measured regression (`build/logs/perf-probe-list-int-dot-ceiling-20260409_113108_86448.log`: public-slot `dot_product_int` `~3.2619× C`, public-slot `array_sum_int` `~2.1178× C`), and the exact whole-list unchecked-helper shortcut is still opt-in only after regressing the whole-operation split. The new explicit `array_sum_int` single-list cursor-reg probe, `build/logs/perf-probe-arm64-fast-get-sum-single-list-cursor-regs-list-int-20260409_130055_41301.log`, now keeps `OREN_ARM64_FAST_LIST_INT_GET_SUM_SINGLE_LIST_CURSOR_REGS` enabled by default: default steady native median `0.133314s` stayed slightly ahead of disabled `0.134232s` while the disabled gate leg carried a high-variance C warning and both legs kept the same 16-instruction traced loop. The new explicit `array_sum_int` single-list push-cursor probe, `build/logs/perf-probe-arm64-fast-push-single-list-cursor-list-int-20260409_132214_76347.log`, keeps `OREN_ARM64_FAST_LIST_INT_PUSH_SINGLE_LIST_CURSOR` enabled by default: default steady native median improved from disabled `0.136291s` to `0.131530s` and gate native improved from disabled `0.010571s` to `0.010084s`, with the same 16-instruction traced loop on both legs. The new final-tree setup-vs-steady attribution probe, `build/logs/perf-probe-list-int-array-sum-c-breakdown-20260409_143718_76549.log`, keeps the repeated `array_sum_int` gap explicit on the same workload: Oren steady per-rep is still `~6.4228×` the slot64 C vector steady per-rep, while the short-run setup estimate remains noisy enough that it should not drive shipping decisions by itself. The new final-tree explicit get-sum tick-mask sweep, `build/logs/perf-probe-arm64-fast-get-sum-tick-mask-list-int-20260409_143632_74801.log`, keeps `OREN_ARM64_FAST_LIST_INT_GET_SUM_TICK_MASK=4095` shipped for now. That rerun is intentionally conservative: explicit `16383` improved steady native median from `0.141901s` to `0.137232s` and `65535` improved it further to `0.131635s`, but the gate view stayed too noisy to trust (`c_cov=0.6421` at default, `0.2631` at `16383`, `0.1270` at `65535`). The new final-tree whole-operation host-C ceiling rerun, `build/logs/perf-probe-list-int-c-ceiling-20260409_143734_77001.log`, keeps the broader blocker picture intact: `oren_array_sum_int / array_slot64_vector ~5.7976×` and `oren_dot_product_int / dot_slot64_vector ~1.8923×`. Reweight again: on current arm64 `master`, the remaining whole-operation blocker is no longer helper/public slot routing. `array_sum_int` still lacks a competitive slot64-vector repeated-read path, while `dot_product_int` remains materially above even the slot64 host-C ceiling within the current ABI.
...
1) **W5 - Native integer hot-loop parity (loop_sum, dot_product)** (L)
   ...
   - Shipped scalar exact-`madd` subpath after the unroll2 default flip (2026-04-09): the older
     ...
     - Read-split decomposition follow-up (2026-04-09): the new wrappers
       ...
       - generic `dot_product`: `CURSOR=0,SCALAR=1` improves every reported native component on
         this rerun (`short -3.49%`, `setup -3.27%`, `delta -5.72%`, `long_per_rep -4.47%`)
       - explicit `dot_product_int`: `SCALAR=1` improves short/setup (`-3.28%`, `-3.95%`) and is
         nearly flat on repeated `long_per_rep` (`-0.11%`), but still worsens the `delta`
         estimate (`+4.22%`); `CURSOR=0,SCALAR=1` improves short/setup more but regresses repeated
         `long_per_rep` (`+2.74%`)
       Use these read-split probes as decomposition tools, not as shipped-default verdicts by
       themselves.
     - Order-balanced gate-stability tie-breaker (2026-04-09): the new
       ...
       - `SCALAR=1` wins absolute native gate median in `3/4` sweeps and by median `-1.31%`, but
         loses normalized `native/C` in `3/4` sweeps with median `+5.74%`
       - `CURSOR=0,SCALAR=1` is flatter on absolute native median (median `-0.52%`) but still
         loses normalized `native/C` (median `+2.32%`)
     - Conclusion: keep the scalar-tail `madd` choice opt-in on the shipped baseline, keep cursor
       regs on by default, and judge future arm64 dot core changes with the matrix + read-split +
       order-balanced gate-stability probes rather than the older scalar-only promotion story.
