...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  output, RGBA input reads, software raster image/mesh RGBA sampling, and software raster clear/pixel writes now use raw exact-size buffer stores or direct u8-buffer access, and
