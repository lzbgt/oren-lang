...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  Contiguous u8 slice and dense u8 matrix copies from `u8_buf` or string
  sources now use shared byte-span copy/direct byte-write paths before falling
  back to checked per-element view stores.
