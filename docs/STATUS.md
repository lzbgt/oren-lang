...
- **Rejected wrapper probe**: direct ARM64 lowering for constant-index `oren_list_get(...)` calls reduced emitted wrapper bytes but did not reduce wall time; the source probe was reverted and should not be reopened as a byte-size-only optimization. Statement profiling now identifies the real wrapper shape as generic `oren_list_get` argument-binding/lowering time in `lambda_wrap` and `fnwrap`, so the next viable design must reduce compiler-side wrapper binding cost directly (2026-05-05).
