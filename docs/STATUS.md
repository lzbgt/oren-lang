...
- Native ops plus ARM64 Mach-O/ELF, ARM64 expression/program/global/root
  ...
  wrapper, lambda, and debug-root counts before iteration, with hotpath
...
- Module-linking prelude parse/import scans, parse-stage dependency/worklist/include
  ...
  aggregation now cache immutable list counts before traversal, with hotpath guard
  coverage rejecting direct `oren_list_len` loop bounds in those link stages.
...
- Optimizer DCE env copies, use/read collection, trivial-var probes, and rewrite
  walks now cache immutable list counts before traversal, with hotpath guard
...
- Type-name resolution expression/statement walks now cache immutable AST list
  counts before arg/element/pair/field/case/statement traversal, with hotpath
...
- Native runtime-object cache file metadata, runtime fragment hashing, and
  ...
  traversal, with hotpath guard coverage rejecting direct `oren_list_len` loop
  bounds in that runtime-cache path.
...
- Native runtime include expansion now caches immutable include-stack counts
  before cycle-check and stack-copy traversals, with hotpath guard coverage
...
- Native top-level partitioning, ARM64 wrapper-body emission, and ARM64 GC
  ...
  traversal, with hotpath guard coverage rejecting direct `oren_list_len` loop
  bounds in those native support paths.
...
- X64/ARM64 ELF dynamic section finalization now shares hash/dynstr/dynsym/RELA
  table append, `.dynamic` emission, and final offset/length metadata through
  `elf_artifact.oren`, leaving relocation selection in the arch emitters.
...
- X64 index/list lowering cleanup now separates map-vs-generic runtime-fast
  ...
  prep plus call-object ABI/direct-call spill records, LCG sum while, lambda
  literal env, and native allocation kind helpers also use positional records.
...
- X64 system-intrinsic cleanup now separates Linux `sys_write` label/body/finish
  ...
  socket, sockaddr, fd2, sockopt, sendto/recvfrom, and close records.
...
- X64 runtime-required call fast-path classification now separates
...
- ARM64 native panic string emission now reuses the native expression
...
- iOS CoreGraphics OGF0 clip/transform/opacity/camera frame state now uses one
  ...
  remains retained 3D depth-window state without CGContext save/restore;
...
- iOS `OrenAVMRuntime` now leaves the live-network `NSURLSession` and scalar
  ...
  small-read buffer plus bounded 64 KiB scratch reads before returning
  heap-owned payload bytes to AVM.
...
- `std:math/mat4` now includes checked viewport/NDC conversion helpers
  ...
  semantics; projection/look-at constructors now live in a focused projection
  helper module, point/vector/normal and project/unproject transform
  implementations live in a focused transform helper module, and viewport
  implementation lives in a focused viewport helper module so
  `mat4.oren` stays below the source-line guardrail with room for future math
  expansion alongside the focused predicate/check/norm/inverse/projection/
...
- `std:math/mat4` now includes checked basis editing helpers (`with_basis`,
  ...
  3x3 linear part.
- `std:math/mat4` now includes checked cell/row/column access and editing
  helpers (`at`, `element`, `with_cell`, `set_cell`, `with_row`, `set_row`,
  `with_column`, `set_column`, `with_col`, and `set_col`) that read or replace
  finite row-major cells, rows, or columns while preserving untouched cells.
- `std:math/mat4` now includes checked diagonal aliases and editors (`diag`,
  `from_diag`, `diag_matrix`, `with_diagonal`, `set_diagonal`,
  `without_diagonal`) that preserve off-diagonal cells while validating finite
  four-value diagonals.
...
| Native x64 Linux/Windows | Active bring-up with compile/runtime gates. Arch host moved to `bruce@192.168.0.102`; x64 stage2 self-host builds and `oren_stage2 test examples/hello.oren --backend bytecode --platform x64-linux --no-cache` now pass there after routing call-depth hooks through serialized x64 `call_rel32` rtobj fixups and bumping the x64 rtobj backend signature to `x64_v0_36`. Linux x64 syscall scratch states now use compact positional lists across stat/path, read/write fd-buffer setup, getrandom, futex wait, fd-control/dup/ioctl, and syscall-tail helpers; Windows x64 thread/filesystem/network syscalls now use positional helper records; x64 list/int length/get/set/push, index-get/index-set, tracking, fast LIST/LIST_INT dot prep/label, push-while, get-sum, LCG, lambda-env, native allocation-kind, and call-object/direct-call helper records have moved off transient keyed maps. | Not fully mature; keep using the Arch host for x64 runtime smokes while continuing x64 codegen bring-up. |
																																																																					  ...
																																																																					  as a thin arithmetic-dispatch/result-pack wrapper. The capped profile now shows
																																																																						  ...
																																																																						  loop body, and patch helpers. The capped profile now shows
																																																																							  ...
																																																																							  prehook, `socket` IAT dispatch, `INVALID_SOCKET` result mapping, and capsule
																																																																								  ...
																																																																								  at about 42.9s total / 31.3s parse, with
																																																																								  `_emit_intrinsic_sys_linux_dup23_x64` exposed at about 58ms as the next parser
																																																																								  body.
																																																																								  ...
																																																																								  prehook/capture, stat-pointer spilling, syscall setup, and capsule posthook
																																																																									  ...
																																																																									  reads and mirrors compile-option grouping with progress, slow-function, and
																																																																										  ...
																																																																										  `x64_native_program.oren` at about 43.2s total / 31.6s parse, with
																																																																										  `_x64_emit_sys_getdirentries64_x64` exposed at about 57ms as the next parser
																																																																										  body.
																																																																										  ...
																																																																										  prehook/capture, buffer/size/ignored-position evaluation, `getdents64`
																																																																											  ...
																																																																											  profile now shows `x64_native_program.oren` at about 43.5s total / 31.7s
																																																																											  parse, with `_x64_emit_program_entry_sequence` exposed at about 59ms as the
																																																																											  next parser body.
																																																																											  X64 program-entry sequence emission now delegates prologue/bootstrap/root
																																																																												  state, entry-option packing, register/top/main/return emission, and done
																																																																												  logging to focused helpers while preserving DLL/SO entry metadata. The capped
																																																																												  profile now shows `x64_native_program.oren` at about 43.7s total / 31.8s
																																																																												  parse, with `_emit_ops_in_fn` exposed at about 58ms as the next parser body.
																																																																												  ...
																																																																												  helper while preserving reserve-skip, tracing, gas patching, prebuilt batch,
																																																																													  ...
																																																																													  `x64_native_program.oren` at about 45.2s total / 33.3s parse, with
																																																																													  `_x64_emit_getentropy_windows_resolve_fp` exposed at about 61ms as the next
																																																																													  parser body.
																																																																													  Windows x64 `sys_getentropy` function-pointer resolution now delegates cached
																																																																														  ...
																																																																														  now shows `x64_native_program.oren` at about 44.3s total / 32.6s parse, with
																																																																														  `_x64_emit_function_body_ops` exposed at about 61ms as the next parser body.
																																																																															  ...
																																																																															  `x64_native_program.oren` at about 44.3s total / 32.7s parse, with
																																																																															  ...
																																																																															  loop-body finish, and slow-path emission to focused helpers while preserving
																																																																																  ...
																																																																																  X64 compile-summary emission now lives in a bounded program-entry helper
																																																																																	  ...
																																																																																	  string-entry gating, string-pointer checks, concat dispatch, and integer-add
																																																																																		  ...
																																																																																		  capped profile now shows `x64_native_program.oren` at about 45.0s total / 33.0s
																																																																																		  parse, with `_x64_fast_lcg_prepare` exposed at about 62ms as the next parser body.
																																																																																		  X64 fast LCG prepare now delegates input extraction, fastmod reciprocal
																																																																																			  ...
																																																																																			  now shows `x64_native_program.oren` at about 44.2s total / 32.4s parse, with
																																																																																			  `_x64_string_batch_append_table_and_emit` exposed at about 61ms as the next parser body.
...
- The first GUI bridge slices now exist as binary GFX mailboxes. Bytecode can
  ...
  view exposes SDK-side frame metrics for rendered frame count, CPU encode time,
              target frame budget, budget-usage permille, over-budget status, geometry vertex count, and text-run count. Retained image resources are now available for sprite-like upload/draw/destroy/sub-rect and packed batched-atlas lifetimes, retained 2D and first retained 3D mesh resources avoid resending repeated triangle geometry, Metal geometry vertex builders reserve a small op-count-bounded initial capacity only on first append and transfer raw buffers into vertex-run ownership, Metal geometry-run preparation coalesces adjacent same-scissor raw vertex spans with geometric growth, Metal batched image-rect commands build one raw vertex-backed run per command instead of one run per rect, Metal image-run preparation coalesces adjacent same-texture/scissor/opacity runs into one raw vertex span, retained Metal material override draws resolve material RGBA and convert constant material/mesh RGBA bytes once before triangle loops, and Oren-side image upload budgets plus SDK retained image count/pixel budgets bound sprite memory; CoreGraphics and Metal retained image, text, mesh, material, and model lookups use scalar IDs instead of boxed dictionary keys; retained text upload/draw/destroy and packed retained text batching now avoid resending repeated UTF-8 labels, CoreGraphics caches text attributes by RGBA for immediate and retained text paths with a scalar one-entry MRU before scalar-key cache lookup/storage, and Metal packs rendered labels into bounded atlas textures, coalesces adjacent same-atlas/scissor/opacity runs, and shares a typed text-attribute cache with scalar-key lookup/storage to reduce repeated text-color miss overhead. UIKit/CoreGraphics
  ...
  human-readable `vertices_xyz` or flat `vertices_xy` / `faces` or `quads` coordinate arrays with
...
dictionaries, and retained text IDs use scalar-key lookup; CoreGraphics text attribute lookup now keeps a scalar same-color
MRU before scalar-key cache lookup/storage; CoreGraphics retained image
...
retained/immediate text resource drawing helpers now live there too. CoreGraphics
retained 2D/3D mesh, material, and model upload/remove helpers also live there,
keeping typed resource construction and raw payload ownership with the resource
model types; retained 2D mesh draw payload expansion now lives there too,
matching the Metal retained 2D mesh draw boundary, and retained 3D
mesh/model/material draw payload expansion now lives there too. The
...
`oren_write_bytes` now keeps list-input compatibility but routes boxed-list,
`LIST_INT`, and `u8_buf` carrier checks through shared AVM byte-span helpers
before opening the destination, then streams bounded 64 KiB stack chunks through
the same helper path instead of allocating a full-size AVM heap byte mirror. Legacy AVM
...
`oren_bytes_len` now uses the shared byte-carrier length checker used by the
rest of the byte-span helper family, so boxed lists and `u8_buf` carriers avoid
another local branch copy in the public length intrinsic. Legacy C runtime
...
`make verify-libavm-ios` guards these byte ownership contracts.
  ...
  The AVM release manifest also includes a whole-frame 2D/3D-projection raster conformance hash
	  covering geometry including `stroke_rect`, `round_rect`, `ellipse`, `polyline`, clip and translation stacks, retained text, retained images, atlas sub-rects, batched
          sprites, retained 2D meshes, retained 3D triangle and indexed meshes through orthographic projection, painter-depth ordering, camera depth-window culling, and draw ordering in one scene. A dedicated 3D conformance fixture separately hashes retained 3D resource behavior across camera depth windows, model translation/scale/Z, per-triangle RGBA depth ordering, indexed shared-vertex meshes, material override draws, and retained model-resource draws. The Metal retained-3D painter path now uses one compact visible-triangle order buffer sorted by depth with original-index ties, avoiding per-triangle Objective-C set/number allocations, repeated full rescans, and repeated RGBA byte conversion for constant material/mesh-color draws; small retained 3D draws use stack order storage, and larger draws use raw heap order buffers freed after sorting/drawing. The CoreGraphics retained-3D fallback now uses the same compact sorted order-buffer path and stack-first order storage for indexed and packed 3D meshes, preserving painter semantics without per-triangle `NSNumber`/`NSMutableSet` churn, repeated full rescans, or Objective-C data wrappers for large order buffers; `make verify-libavm-ios` guards those fallbacks against regression.
...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  responses, and exact-sizes decoded chunked response bytes,
	  ...
	  routes validated strings directly through kernel `oren_bytes_from_string`
	  ...
	  JSON full decode, scalar parse, tag equality, and escape paths
				  use direct source-string byte reads or exact-size `u8_buf` output, `std:bytes`
					  ...
					  from sizing-pass `list_int` metadata during exact-size byte-buffer writes,
						  ...
						  single-CONTINUATION header blocks use the shared exact-capacity accumulator path,
				  full regex
  ...
  receive-call prefix scratch buffer, unmasked frame sends stream raw payload spans after a
	  ...
	  without string conversion, `recv_bytes` control frames use scratch storage
	  instead of managed byte-return buffers, and
  ...
  capsule NET IPv4 sockaddr reads/rewrites copy directly after validation;
				  Base64/Base64URL encode hoists shared byte-view backing pointers once per call and maps sextets arithmetically instead of indexing alphabet strings, argparse ASCII lowercasing writes exact-size byte buffers instead of scanning alphabet strings,
  ...
  include scanning, compiler manifest JSON escaping, byte-native bytecode metadata payloads,
		  ...
		  guarded by the byte-hotpath source guard, runtime-object cache metadata check
		  magic/endian reads use cached u8 pointers after upfront validation with
  ...
  Raw `u8_pack_into` validates the full input list before mutating the
  destination and writes valid bytes directly into `u8_buf` storage before
  falling back to compatibility stores.
  ...
  U8 matrix pack helpers now write validated row-list payloads directly into
  fresh dense `u8_buf` storage and bulk-copy validated string rows into the
  same fresh dense storage.
  ...
  The module renamer now preserves builtin annotation names such as `bytes`
	  ...
	  Pure Oren SHA-1/SHA-256 now validate bytes through shared byte views, expose canonical
	  `digest` / `hex` / receiver-method APIs, and process virtual padding via
	  byte-view reads instead of unpacking the whole message to a list, and
	  ...
	  literal-length metadata, or byte-slice conversion instead of building
		  intermediate Oren byte lists; TLS ALPN
			  ...
			  buffers, single-CONTINUATION inbound header blocks use the shared exact-capacity
				  accumulator path without an overcapacity finish copy, `content-length` DATA bodies use exact-capacity accumulation
          ...
          raw payload spans, masked sends stream fixed-size masked chunks, and header slices/frame payload reads plus DNS QNAME
...
- The iOS SDK implementation is split so `OrenAVMRuntimeConfig` and
  ...
  in one review boundary. Scene3D PLY/STL source lowering now lives in
  `scripts/scene3d_ply_stl.py`, reducing `make_scene3d_bin_v0.py` to 1523 lines
  while preserving byte-identical `.os3d` output for the package demo asset. The
  iOS SDK verifier reuses
  ...
  release-manifest heightfield/surface raster coverage. Scene3D generated
  architectural-shape assertions now live in
  `tests/avm/test_ui_scene3d_arch_shapes_v0.oren`, reducing
  `test_ui_scene3d_v0.oren` to 1027 lines while preserving default AVM and
  release-manifest wall/room/ramp/curb/post/fence/stair/roof/pyramid coverage. Scene3D byte-native `.os3d` binary package loading now lives in
...
- Current core includes integer/float abs/min/max/clamp, IEEE-ish predicates and
  ...
  pure finite `std:math/quat` identity/length/length_sq/length_squared/normalize/normalize_or_identity/conjugate/dot/inverse/mul/multiply/from_axis_angle/axis_angle/to_axis_angle/axis/angle/from_euler_xyz/euler_xyz/from_euler/to_euler_xyz/to_euler/euler_angles/angle_between/angular_distance/exp/log/ln/pow/power/rotation_power/scale_angle/squad/from_to_rotation/rotation_between/from_vectors/relative_rotation/difference/look_rotation/from_forward_up/look_at_rotation/from_basis_rows/from_rotation_matrix/from_mat4/rotate_vec3/rotate_vector/inverse_rotate_vec3/inverse_rotate_vector/right/up/forward/nlerp/slerp/rotate_toward helpers,
  pure finite row-major `std:math/mat4` identity/translation/from_translation/translate/scale/from_scale/scaling/rotation_x/rotation_y/rotation_z/from_axis_angle/rotation_axis/from_quat/from_quaternion rotation constructors, compose_trs/compose_transform/trs TRS composition, translation_of/scale_of/rotation_quat/decompose_trs/decompose_transform/decompose TRS decomposition, checked with_translation/set_translation/with_translation_vec/set_translation_vec/without_translation editing helpers, is_finite/isfinite/is_affine/has_perspective predicates, equals exact comparison, allclose_abs/allclose tolerance predicates, is_identity/is_identity_abs identity checks, exact/tolerance-aware is_symmetric/is_symmetric_abs, is_upper_triangular/is_upper_triangular_abs, is_lower_triangular/is_lower_triangular_abs, is_diagonal/is_diagonal_abs, is_translation/is_translation_abs, is_scale/is_scale_abs, is_uniform_scale/is_uniform_scale_abs, is_invertible/is_invertible_abs, and is_singular/is_singular_abs predicates, exact/default plus tolerance-aware is_orthonormal/is_orthonormal_abs and is_rigid_transform/is_rigid_transform_abs transform predicates with is_rigid/is_rigid_abs aliases, rigid-transform inverse helpers and aliases, basis_x/basis_y/basis_z plus right/up/forward basis extractors, checked from_basis/from_axes reconstruction with optional translation vectors, row0/row1/row2/row3, indexed row, column0/column1/column2/column3, col0/col1/col2/col3, indexed column/col extractors plus checked cell/row/column/diagonal editors and from_rows/from_columns reconstruction and from_diagonal/from_diag/diagonal_matrix/diag_matrix constructors, diagonal/diag/trace/one-norm/infinity-norm/condition-number inspection, checked cofactor/adjugate/minor helpers, ortho/orthographic/frustum/perspective projection, look-at/model_view/view_projection/model-view-projection composition, mul/multiply, transpose/transposed, determinant/det, inverse/invert/inverted, normal_matrix3/normal_matrix extraction, transform_point3/transform_point, transform_vec3/transform_vector, transform_normal3/transform_normal, project_point3/project_point, unproject_point3/unproject_point, viewport_matrix/inverse_viewport_matrix, and ndc_to_window/window_to_ndc transforms,
...
- `tests/avm/test_std_math_core.oren`, `tests/avm/test_std_math_pow.oren`,
  ...
  normal/subnormal/`fpclassify` classification, component-wise ordered comparison predicates plus `signbit`/`copysign`, vector NaN-aware `fabs`/`fmin`/`fmax`, vector `fmod`/`remainder`/`remquo`, ties-to-even and integer rounding aliases,
  ...
  field-chain inference, returned-map value field-chain inference,
	  ...
	  and imported constructor-field nested container field-chain inference, returned/imported nested container
		  selection field-chain coverage, call-site constructor-held nested container selection coverage,
					  ...
					  branch-merged, returned, imported-returned, parameter-returned,
					  and imported parameter-returned typed dotted member-path
...
- X64 index expression lowering now separates dynamic label state, nil-container
  ...
  path emission while preserving temp cleanup and patch order. The capped
...
- X64 fast LIST_INT dot-product entry lowering now separates prepare-state setup,
  checked fast-path emission, and generic fallback routing while preserving temp
  cleanup and slow-path label behavior. The capped profile now shows
