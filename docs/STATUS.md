...
- **Rejected task-group task-validator factoring**: replacing `_task_group_validate_policy_shape(...)` with a public `task.validate_stop_policy(...)` wrapper deleted the duplicated source shape, but it repeated the known bytecode-only mixed task-group failure at `191096` while task-group, task, C, and native surfaces stayed green. Source was reverted; do not route task-group runtime preflight through `std:task` policy helpers unless the AVM bytecode mismatch is understood and fixed first (2026-05-06).
