...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  and `from_hex` plus direct lowercase `to_hex` emission and `std:strings`
  ...
  emission, the native runtime hex intrinsic reads list/u8-buffer backing
  storage directly after one validation pass, and Windows Schannel passphrase
  cache keys use the direct string hash path instead of materializing a byte
  list, SHA-1/SHA-256 digest buffers finalize through direct unchecked u8 stores
  after exact-size allocation, and native SHA-256 contiguous input remainders
  copy with `oren_memcpy`. Compiler source-policy scans, scan-cache line/number parsing and delimiter writes, C-runtime
