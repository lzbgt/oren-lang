...
- Optimizer fold update (2026-05-05): `optimizer.summary` now includes gated fold-shape counts, and the retained fold cleanup hoists stable lengths in fold-local loops, returns early for leaf expressions, skips recursive folding for leaf children, skips child loops for calls whose callee/args/spread are all leaves, avoids clone-of-clone environments for isolated `if`/`for` folding, and avoids a redundant empty environment push for function-expression bodies. The measured profile moved optimizer total from about `1.98s` to about `1.55s`, `fold_ms` from about `833ms` to about `449ms`, `fold_env_clone` from `2592` to `1317`, and `fold_env_push` from `3754` to `3192`; the current fold count table shows `8398` non-leaf folded expressions after child-leaf skipping, led by `2644` calls, `2489` infix expressions, `1271` expression-level `if` nodes, and only `22` remaining other expressions. The older reverted fold hoist above refers to the broader neutral stable-list-length probe before list-pass and fixture-shape reductions; do not repeat that probe without new evidence.
