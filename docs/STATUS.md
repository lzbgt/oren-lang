...
| Effect ledger contract | Rolling schema | `docs/EFFECT_LEDGER_CONTRACT.md`, `lib/avm/main.c`, `lib/runtime_native/000_prelude_sys.oren`, `lib/runtime_native/010_channels_globals_consts.oren`, `scripts/run_backend_semantic_diff.sh` | `make verify-effect-ledger-contract`, `make verify-avm-effect-ledger-json`, `make verify-backend-semantic-diff`, `make verify-native-capsule-resource-checks`; AVM run JSON emits `effect_ledger_summary` with gas/heap/wall/log/trace budget fields; native executables can emit `oren.native-run.v0` with native `effect_ledger_summary` wall timing, scanned live tracked-heap bytes, `oren.native-capsule-effect-gates.v0` domain-gate counters, and `oren.native-capsule-resource-checks.v0` resource-check counters through `OREN_NATIVE_RUN_JSON=1`; semantic diff emits `oren.semantic-diff.v0` with native/OBC ledger summaries, normalized `budget_deltas`, and explicit C ledger-unavailable markers; native package policy can emit `oren.native-package-policy-run.v0` runner-observed wall/heap/CPU-budget JSON plus captured native ledger summaries and now enforces native `budget_heap_bytes` from the live-heap scan and `budget_cpu_ms` from child process resource usage where available; native gas accounting remains future work |
