...
| Effect ledger contract | Rolling schema | `docs/EFFECT_LEDGER_CONTRACT.md`, `lib/avm/main.c` | `make verify-effect-ledger-contract`, `make verify-avm-effect-ledger-json`; AVM run JSON emits `effect_ledger_summary` with gas/heap/wall/log/trace budget fields; full native/AVM ledger parity fixtures are future work |
