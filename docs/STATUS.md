...
- **Rejected wrapper probe**: direct ARM64 lowering for constant-index `oren_list_get(...)` calls reduced emitted wrapper bytes but did not reduce wall time; the source probe was reverted and should not be reopened as a byte-size-only optimization (2026-05-05).
