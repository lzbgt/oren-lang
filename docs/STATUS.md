...
- **Native build profiling**: ARM64 self-host profiling now has enough phase detail to rank link, parse, optimizer, wrapper, and user-declaration codegen buckets without manual log spelunking. The latest report path also prints phase-specific top function bodies for `user_decls`, `fnwrap`, and `lambda_wrap`; the current generator-surface fixture shape was trimmed by splitting the remaining large timeout policy wait, nil/collect iteration, and declaration counter/collect clusters while preserving fixture semantics (2026-05-05).
- **Rejected generator-surface fixture split**: a follow-up split of defer-finalize, collect/for-in, declaration collect, and local-generator fixture clusters passed direct bytecode/native fixture builds and the native build profile, but did not improve the aggregate target. The exact profile moved labels into smaller for-in/local rows while `user_decls` stayed roughly neutral/slightly worse (`~2.96s` prior band vs raw `~2.99s`) and optimizer total rose (`~404ms -> 456ms`) from the added function surface. Source was reverted; future fixture shape work must improve aggregate wall time, not just top-row labels (2026-05-05).
- **Rejected wrapper probes**: direct ARM64 lowering for constant-index `oren_list_get(...)` calls reduced emitted wrapper bytes but did not reduce wall time; a later direct lambda-wrapper local-binding probe removed the visible `lambda_wrap` generic `oren_list_get` statement bucket but moved cost into uninstrumented wrapper body work and regressed default wrapper timing (`lambda_wrap` about `2.03s -> 2.28s`, `fnwrap` about `1.10s -> 1.31s`). Both source probes were reverted; the next viable wrapper design must reduce compiler-side binding/lowering time in the measured wall-clock phase, not just hide or relabel the statement bucket (2026-05-05).
- **Rejected task-group parse probe**: caching repeated primitive tag results inside `_task_group_validate_policy_shape(...)` preserved semantics and passed focused task-group surfaces, but widened the parsed/body shape and regressed the default profile (`lib/std/task_group.oren` about `471ms -> 616ms`, `user_decls` about `3.38s -> 4.22s`). Keep residual task/task-group parser work focused on structural simplifications with measured parse/codegen wins, not local tag temporaries (2026-05-05).
- **Rejected task-group validated-map probe**: removing the redundant policy-map normalization inside private `_task_group_validate_policy_shape(...)` passed `make stage2` plus focused task/task-group surfaces, but the exact profile did not improve the hot body and moved `lib/std/task_group.oren` to about `477ms / 65 stmts` with the same `_task_group_validate_policy_shape` top row. Source was reverted; this confirms residual task-group parser work needs larger shape deletion, not tiny private precondition tightening (2026-05-05).
- **Task-group runtime stop cleanup**: `std:task_group` now shares the runtime preflight/validate/take-snapshot/default-policy merge path used by `stop_policy(...)` and `stop_policy_wait(...)`, while preserving the empty-preflight early return and the preflight-before-take ordering. Focused task/task-group/generator/coroutine gates pass, and the exact native profile moves `lib/std/task_group.oren` from `486ms / 64 stmts / parse_body_ms=296` to `471ms / 65 stmts / parse_body_ms=289`; the former `stop_policy` and `stop_policy_wait` hot parse rows drop out of the top three (2026-05-05).
- **Task-group runtime stop snapshot field-shape cleanup**: the private `_task_group_runtime_stop_snapshot(...)` carrier now returns `[members, member_kinds, policy]` instead of a string-keyed map consumed immediately by `stop_policy(...)` / `stop_policy_wait(...)`. The public `task_group.snapshot(...)` surface remains map-shaped. Focused task/task-group/generator/coroutine gates pass; the exact profile moves `lib/std/task_group.oren` to about `433ms / 62 stmts` with `parse_body_ms=265`, while task and optimizer/codegen remain in-band (2026-05-06).
- **Rejected task-group runtime snapshot carrier probe**: widening the compact private carrier to all runtime snapshots preserved public `task_group.snapshot(...)` as a map and passed stage2 plus focused task/task-group/generator/coroutine gates, including the mixed bytecode task-group/task surface. Measurement did not justify retaining it: the helper-heavy version moved `lib/std/task_group.oren` to about `445ms / 63 stmts`, and the tightened public-wrapper version still moved it to about `444ms / 62 stmts` while leaving `_task_group_validate_policy_shape` unchanged at about `17ms`. Source was reverted; keep the compact carrier scoped to runtime stop snapshots only (2026-05-06).
- **Rejected task-group direct merge probe**: replacing the private `_task_group_merge_policy(...)` key-list loop with six direct field merges passed `make stage2` plus focused task/task-group/generator/coroutine gates, but widened the measured module shape. The exact profile moved `lib/std/task_group.oren` from the retained `~433ms / 62 stmts` band to about `448ms / 62 stmts`, with `_task_group_merge_policy` becoming the second hot parse row at about `12ms`; source was reverted. Keep future validation/merge work focused on larger shape deletion, not unrolling this loop (2026-05-06).
- **Rejected task-group merge+validate helper probe**: adding a `_task_group_merge_policy_validated(...)` wrapper for the runtime stop preflight preserved behavior and passed `make stage2` plus focused task/task-group/generator/coroutine gates, but it added a statement/function and did not remove the validation hot row. The exact profile moved `lib/std/task_group.oren` to about `447ms / 63 stmts` with `_task_group_validate_policy_shape` still at about `17ms`; source was reverted. Do not wrap merge+validate as another helper unless the design deletes the validation data shape itself (2026-05-06).
- **Rejected task-group task-validator factoring**: replacing `_task_group_validate_policy_shape(...)` with a `task.validate_stop_policy(...)` wrapper deleted the duplicated source shape, but it repeated the bytecode-only mixed task-group failure at `191096` while task-group, task, C, and native surfaces stayed green. A narrowed bytecode probe showed the mechanism: after prior runtime stop/cancel cases, helper-routed invalid-policy preflight took about `278ms`, then `cancel_wait` took about `321ms`, letting the observer task naturally return `1800` instead of observing cancel reason `912` and returning `2712`. Source was reverted; do not route live runtime-group preflight through `std:task` policy helpers unless the bytecode preflight latency is removed first (2026-05-06).
- **Task stop-policy delay cleanup**: `std:task.stop_policy(...)` and `stop_policy_wait(...)` now route request/cancel modes through the normalized `delay_ms` computed by `_task_stop_fields(...)`, matching the stop-mode path and removing redundant deadline wrapper branches plus the unused returned `deadline_ns` field. The task-focused profile keeps `lib/std/task.oren` in-band but slightly lower (`~321ms -> 318ms`) and moves `stop_policy_wait` to about `24ms`; an adjacent task-group `extend` loop unification plus watcher-join helper probe was reverted because it removed one hot row but worsened the exact task-group module parse profile (2026-05-05).
- **Task stop-policy field-shape cleanup**: `_task_stop_fields(...)` now returns its private normalized fields as a compact positional list instead of building a temporary string-keyed map that the two callers immediately indexed. The public policy map API is unchanged. Focused task/task-group/generator/coroutine gates pass; the exact profile moves `lib/std/task.oren` to about `286ms / 49 stmts`, `stop_policy_wait` to about `19ms`, and `_task_stop_fields` to about `14ms`, while `task_group.oren` and optimizer/codegen stay in-band (2026-05-06).
- **Task/task-group list intrinsic cleanup**: `std:task` and `std:task_group` now call kernel `oren_list_len(...)` / `oren_list_push(...)` directly for hot internal loops instead of routing simple length/push operations through `std:list` wrappers; `std:task` also drops its now-dead `std:list` import. Focused task/task-group/generator/coroutine gates pass, and the exact profile moves `lib/std/task_group.oren` from `476ms / 65 stmts / parse_body_ms=299` to `454ms / 65 stmts / parse_body_ms=274`, while `lib/std/task.oren` moves from `310ms / 51 stmts` to `300ms / 50 stmts` (2026-05-05).
- **Std generator/task predicate wrapper cleanup**: the generator/task/task-group stdlib surfaces now call primitive runtime predicates/tags directly where the local wrappers only returned the primitive result. This removes `_generator_is_int`, `_generator_is_string`, `_task_handle_ok`, `_task_group_is_runtime`, and `_task_group_target_ok` without adding local temporaries. Focused task/task-group/generator/coroutine gates pass; the exact profile keeps parser-body timings stable while reducing source shape to `lib/std/generator.oren` `135` stmts, `lib/std/task.oren` `49` stmts, and `lib/std/task_group.oren` `63` stmts, with `user_decls` in-band at about `2.68s / 328 funcs` (2026-05-05).
- **Task direct runtime predicate cleanup**: remaining hot internal task/task-group paths now call `oren_task_is_handle(...)` / `oren_task_is_done(...)` directly instead of routing through exported `std:task` predicate wrappers. This preserves the public predicates while deleting internal wrapper-call shape in `_task_timeout_detach_result(...)`, `_task_request_then_stop_handle(...)`, and `_task_group_target_kind(...)`. Focused task/task-group/generator/coroutine gates pass; the exact profile keeps statement counts stable and moves `lib/std/task.oren` to about `285ms / 49 stmts` and `lib/std/task_group.oren` to about `444ms / 63 stmts`, with `user_decls` in-band at about `2.67s / 328 funcs` (2026-05-06).
- **Rejected runtime native dead-helper probe**: a source token scan plus direct `rg` confirmation made several private runtime helpers look unused, but deleting them was not safe. The probe removed stale-looking green scheduler wrappers/accessors, Darwin/Linux OS-thread metadata-size accessors, and `_netpoll_iocp_post_recvfrom(...)`; `make stage2` initially completed, then task-surface fixture builds segfaulted immediately, and a cold runtime-seed rerun narrowed the remaining IOCP wrapper deletion to a stage2 seed segfault. Source was reverted; runtime-native helper reachability must be proven by build/fixture execution, not source tokens alone (2026-05-06).
- **Optimizer reserve safe-maintenance guard**: the list-reserve pass now skips reserve safe-int maintenance for statement shapes that cannot assign tracked locals, instead of always running `_opt_safe_ints_after_stmt(...)` plus assigned-name collection after every non-top statement. The optimizer/list-reserve/list-int/generator/coroutine gates pass; the exact native profile moves `optimizer.summary total_ms` from about `1175ms` to `1140ms`, `reserve_ms` from `227ms` to `219ms`, and `reserve_safe_ms` from `198ms` to `190ms` while keeping `user_decls` in-band (2026-05-05).
- **Optimizer residual hot-pass profiling**: `optimizer.summary` now reports top function bodies for residual `tco_mod`, `tco`, `for`, `sink`, and `dce` passes in addition to `fold`, `list_int`, and `reserve`, with a 1ms threshold so profiling does not rank sub-millisecond bodies on every function. The profile report also prints an `optimizer pass totals` row. The current profile shows residual `tco`/`for`/`dce` cost is distributed with no >=1ms function body, while only one `sink` body reaches 1ms; next optimizer work should target structural pass-level overhead rather than fixture splitting for those buckets (2026-05-05).
- **Optimizer residual structural reductions**: direct self-recursion detection now runs once in the optimizer driver and skips both TCO passes for functions with no direct self-call, while native `tail_recursion_ok` and `non_tail_modconst_ok` still pass. DCE now returns after recursively handling children when the current block has no trivially-removable local `Var`, avoiding read collection for blocks that cannot delete anything. The retained profile moves `tco_mod+tco` from about `71ms` to `tco_scan=19ms` plus zero rewrite work on the non-recursive generator build, drops `dce_ms` from about `55ms` to `11ms`, and moves optimizer total to about `1052ms` (2026-05-05).
- **Optimizer reserve precision update**: `stmt_may_assign_name(...)` now recursively checks control-flow bodies instead of treating every `Block`/`If`/`While`/`For`/`Switch` as assignment-capable. This keeps `Var`/`Assign`/`Set` and real nested assignments on the original reserve-safe path, but skips safe-int/list-fact invalidation for control-flow subtrees that cannot assign tracked names. The measured generator profile moves optimizer total from about `1052ms` to `857ms`, `reserve_ms` from about `226ms` to `46ms`, and `reserve_safe_ms` from about `196ms` to `15ms` (2026-05-05).
- **Optimizer loop/list residual guards**: the optimizer driver now skips simple-`for` lowering unless a function body actually contains a `For`, skips list-literal sinking unless the body both touches list shapes and contains an `if`, and reports `for_scan_ms` / `sink_scan_ms` in `optimizer.summary`. List-int touch marking also reuses the shared conservative list-touch detector before cloning safe-int state. The measured generator profile keeps optimizer total in-band at about `852ms`, while `for_ms` moves to about `1ms`, `sink_ms` to about `19ms`, `list_int_ms` to about `177ms`, and `list_int_touch_ms` to about `28ms` (2026-05-05).
- **Optimizer fold block-frame guard**: fold traversal now pushes a non-nil fact frame for a `Block` only when the block has direct `Var` or `Assign` statements that can update that frame; assignment-free blocks reuse the current environment while nested functions and branch/loop isolation keep their existing fresh environments. The measured profile moves optimizer total to about `760ms`, `fold_ms` to about `203ms`, and `fold_env_push` from `3204` to `1667` while `fold_env_clone` stays at `1319` (2026-05-05).
- **Optimizer list-int active-candidate guard**: list-int lowering now distinguishes live candidates from names already proven unsafe. Use scanning keeps the cheap any-candidate guard, while touch marking and final rewrite loops are skipped once no active candidate remains; later statement flow can still discover new candidates. The measured profile moves optimizer total to about `694ms`, `list_int_ms` to about `133ms`, `list_int_use_scan_ms` to about `36ms`, and `list_int_touch_ms` plus rewrite buckets to `0ms` (2026-05-05).
- **Optimizer fold control-body guard**: branch and loop body folding now reuses the current non-nil fact environment when the body is already a `Block`, relying on `fold_stmt(Block)` to create a local fact frame only when direct `Var`/`Assign` statements can update facts. Non-block control bodies keep the old clone/push isolation path. The measured profile moves optimizer total to about `514ms`, `fold_ms` to about `60ms`, `fold_env_push` from `1667` to `352`, and `fold_env_clone` from `1319` to `4` (2026-05-05).
- **Optimizer list-int lower candidate guard**: nested list-int lowering now uses a candidate-start detector instead of the broader conservative list-touch detector before descending into branch/loop/switch/block bodies. The detector mirrors the lowerer surface and only recurses when a nested `Var`/`Assign` can introduce a list/list-int constructor or potentially-safe list literal, while leaving use scanning and final rewrites unchanged. The measured profile moves optimizer total from the prior `~514ms` band to about `422ms`, `list_int_ms` from about `141ms` to `80ms`, and `list_int_lower_ms` from about `67ms` to `4ms` (2026-05-05).
- **Optimizer list-int use-scan clone guard**: list-int use scanning now clones the safe-int map only for statement/expression shapes that can mutate branch-local safe-int state during the scan, and it recomputes post-statement safe-int state only for assignment-capable statement shapes using the shared `stmt_may_assign_name(...)` detector. The measured profile moves optimizer total to about `404ms`, `list_int_ms` to about `68ms`, `list_int_scan_ms2` to about `45ms`, and `list_int_use_scan_ms` to about `24ms` while keeping lower/touch/rewrite buckets near zero (2026-05-05).
- **Rejected list-int trace probe**: gating the remaining disabled `OREN_TRACE_LIST_INT` string construction in `optimizer_list_int.oren` preserved trace behavior and passed list-int/reserve/generator gates, but added hot-path trace-flag checks and regressed the default profile (`reserve_safe_ms` about `201ms -> 280ms`, `user_decls` about `3.38s -> 5.01s`). Keep the shipped simpler trace helper calls unless a future change removes trace construction without adding per-site hot checks (2026-05-05).
- **Mach-O local resolver update**: the ARM64 Mach-O local BL/ADR-code target cache now keeps metadata keys sorted and uses binary search plus key-range string comparison instead of a full linear target-list scan. A 4-entry scalar MRU probe first proved repeated local names exist, but did not materially reduce wall time and was not retained. The retained sorted-cache profile preserves the non-generic-map constraint and cuts stats-enabled local resolver work from about `875080` scan steps to about `107739`; the default profile trims the later BL resolve tail (`~97ms + ~126ms` to `~28ms + ~57ms`) while the first 4096 local BL fixups remain the main unresolved bucket at about `1.9s` (2026-05-05).
- **Rejected Mach-O preseed probe**: prebuilding the sorted local target table from `ctx["functions"]` plus `runtime_functions_enc` did eliminate the first-touch map lookup behavior (`appended=0`, first 4096 resolve around `37ms`), but the insertion-sorted preseed itself cost about `96-97s` for `1891` targets and regressed the default profile catastrophically. Do not reopen whole-table preseed unless the table construction is linear or otherwise proven below the current `~1.9s` first-bucket boundary (2026-05-05).
- **Mach-O hot-target profiling**: stats-enabled native build profiles now report the top local BL/ADR-code targets by reference count in both phase rows and the profile report. The exact retained tree shows the first `4096` local BL fixups still cost about `1.95s`; the dominant local targets are `oren_panic` and `oren_ensure_tracked` (`2305` refs each), `oren_call_depth_exit` (`1631`), `native_list_len_panic` (`474`), and `oren_find_node` (`438`). A per-call direct-known-BL probe that consulted `arm64_lookup_function(...)` during hot user codegen passed `make stage2` but made default and stats native profiles time out at `180s` before `arm64.codegen.user_decls.done`, so the next viable design needs precomputed integer target ids/offsets or linear-time materialization, not hot map lookups from emit sites (2026-05-05).
- **Rejected Mach-O BL sid probe**: a sparse BL symbol-id lane annotated only the five measured hot targets and tried to resolve those ids once in Mach-O instead of calling `arm64_lookup_function(...)` at emit sites. It passed `make stage2`, but the stats-enabled native profile still timed out at `180s` before `arm64.codegen.user_decls.done`; even carrying extra fixup-side sid arrays from hot emit sites is too expensive in the current Oren data-shape. Keep the first-touch resolver follow-up below the generic fixup/list representation: compact offsets or target ids must be carried as low-level codegen metadata or materialized in one linear post-codegen pass (2026-05-05).
- **Rejected Mach-O hot-name probe**: a Mach-O-loop-only shortcut precomputed the five hot target offsets once and matched the existing BL names before the sorted resolver. This avoided emit-site overhead and cut stats lookup counters (`11131 -> 3978`, scan steps `106353 -> 35716`), but the final stats wall-time target did not improve: reaching `i=4096` effectively stayed at the `~1.95s` boundary after adding the one-time hot-target precompute, and the exact final stats run measured about `213ms + 1945ms`. Do not reopen string-name hot-target bypasses; the remaining resolver work needs compact ids/offsets that avoid repeated string comparison and generic lookup entirely (2026-05-05).
- **Dead helper cleanup**: removed the stale Mach-O `_macho_str_eq_bytes(...)` helper left behind by the sorted local-target resolver work and the unused `std:task_group` `_task_group_runtime_surface(...)` sentinel. The retained resolver still uses `_macho_str_eq_bytes_known_len(...)` on same-key candidate ranges, and task groups still use `_task_group_surface(...)` for real surface validation, so this is source-shape cleanup rather than a behavior/performance-path change; the next real targets remain compact Mach-O target-id/offset materialization and larger stdlib parser-body deletion (2026-05-06).
- **Mach-O BL single-pass fixups**: local BL fixups now resolve and apply in one pass when no valid prefilled target lane exists, avoiding the old nil-filled `bl_target` materialization and second apply loop. This keeps existing prefilled-target support but does not add generic maps, hot-name bypasses, or emit-side sid fields. The default profile moves the first local BL bucket from the prior `~1.9s` boundary to about `1.41s`, and the stats profile shows the same resolver counters (`11123` BL lookups, about `106069` scan steps) with lower wall time, proving the win is from removing fixup-list materialization/application overhead rather than changing lookup semantics (2026-05-06).
- **Mach-O local resolver insertion stats**: stats-enabled native build profiles now report `insert_shifts` for the sorted local target cache, and `scripts/profile_native_build_phases.sh` prints a `Mach-O local resolve stats` table alongside hot targets. The retained stats profile reports `21120` shifts by the first `4096` BL fixups and `37056` shifts across all BL lookups, versus `106069` scan steps and `16163` byte compares. Insertion maintenance is real but not the whole `~1.4s` bucket, so the next compact target-id/offset design should eliminate both lazy sorted-cache maintenance and repeated string/name lookup (2026-05-06).
- **Rejected Mach-O same-key append probe**: inserting new sorted-cache entries at the end of their already-scanned metadata-key range preserved behavior and reduced same-key candidate scans (`16163 -> 12294` byte compares in the stats profile), but it barely changed insertion shifts (`37056 -> 36925`) and left the default first-touch BL wall time effectively unchanged at about `1.40s`. Source was reverted; do not spend more turns on local same-key insertion-order tweaks (2026-05-06).
- **Rejected Mach-O sparse `bl_target` probe**: reusing the existing compact `bl_target` lane as sparse `(fixup_index, target)` pairs for hot runtime BL targets avoided new sid arrays and passed `make stage2`, but the stats-enabled native profile timed out at `180s` before `arm64.codegen.user_decls.done`. Source was reverted; even existing-lane sparse direct-offset metadata on hot emit paths is too expensive. A viable target-id/direct-offset path must sit below Oren map/list writes in hot codegen or materialize direct target arrays after codegen in linear time (2026-05-06).
- **Mach-O hot BL id lane**: selected hot runtime BL fixups now store compact negative ids in the existing `fixups_local_compact["bl_name"]` lane while preserving the legacy string fixup maps. The Mach-O pass precomputes six runtime offsets once and resolves those ids before falling back to the sorted string resolver; compact-fixup verification treats ids as equivalent to their string names. The stats profile reports `hot_id=7531`, reducing BL string lookups from `11123` to `3592`, scan steps from about `106069` to `32627`, and byte compares from `16163` to `4077`. The default first `4096` BL span moves from the recent `~1.40s` band to about `1.29s`. Insertion shifts remain about `36598`, so the next Mach-O work should target direct target arrays or insertion-free materialization for the remaining non-hot names (2026-05-06).
- **Extended Mach-O hot BL id lane**: the compact negative-id path now also covers the next measured runtime BL targets (`oren_is_err`, `oren_map_get_str`, `oren_new_list`, `oren_list_get`, `oren_list_push_unchecked`, `_list_find_node_known_kind`, `oren_func`, `oren_list_push`, `oren_add`, `oren_map_get_int`) without adding new fixup-side lists or maps. The stats profile moves `hot_id` to `9412`, BL string lookups to `1711`, scan steps to `14478`, byte compares to `1565`, and the default first `4096` BL span to about `1.05s`. Remaining work is now insertion-free/direct-target materialization for the long tail, not more Oren-level side metadata (2026-05-06).
- **Mach-O append-only local target cache**: after the compact-id reduction made the remaining unique-target set small, the local resolver now appends newly resolved target offsets instead of maintaining a sorted cache with tail shifts. This removes insertion maintenance (`insert_shifts=0`) while keeping metadata-key filtering before string compares; the stats profile raises raw scan steps (`14478 -> 268198`) but keeps byte compares low (`1691`) and preserves the default first `4096` BL span at about `1.03s`. The next real target is scan-free direct target arrays for the long tail, not sorted-cache insertion tweaks (2026-05-06).
- **Mach-O long-tail BL id extension**: the existing compact negative-id `bl_name` lane now covers the measured long-tail leaders `STD_generator_next`, `STD_generator_start`, `strcmp`, `STD_generator_send`, and `STD_generator_is_done` without adding new side lists or maps. The stats profile moves `hot_id` to `9727`, string lookups to `1396`, scan steps to `205608`, and byte compares to `1381`; the default first `4096` BL span remains in-band/slightly lower at about `1.02s`. Further Mach-O work should now require a true scan-free direct target array/materialization path, not more one-off name-cache tweaks (2026-05-06).
- **Mach-O residual BL id extension**: a final compact-id batch covers the next residual leaders (`oren_map_set_str`, `oren_type_name`, `STD_generator_cancel_reason`, `_oren_generator_context_exchange`, `STD_generator_is_cancel_requested`) and direct `oren_map_set_str` emit sites. Stats move `hot_id` to `9923`, string lookups to `1200`, scan steps to `177442`, and byte compares to `1122`; the default first `4096` BL span moves to about `0.97s`. The remaining top names are now scattered sub-32-reference targets, so further one-off ids are low leverage unless a new profile exposes a larger cluster (2026-05-06).
- **Mach-O hot-id dispatch array**: the compact BL id path now resolves negative ids through a precomputed target-offset array instead of a 26-branch ladder inside the local BL loop. Lookup counters are unchanged, but the first `4096` BL span nudges lower in default profiling (`~0.97s -> ~0.96s`) by reducing per-hot-id dispatch overhead across about `9923` compact-id fixups. This is the end of local id-dispatch cleanup; the next meaningful Mach-O step still needs scan-free target materialization for the remaining string long tail (2026-05-06).
- **Mach-O first-byte target buckets**: the append-only local target cache now also keeps fixed 256-entry first-byte buckets, so remaining string BL/ADR-code lookups scan only the matching bucket before applying the existing metadata-key and byte-equality checks. This avoids generic maps, global sorting, emit-side sid fields, and future-name rewrites. The retained profile moves BL scan steps from the prior `177442` band to `52381` and default first-4096 BL timing from about `0.96s` to about `0.60s`; remaining names are scattered sub-32-reference targets, so the next Mach-O work should be a true direct target array only if it can be built linearly (2026-05-06).
- **Rejected Mach-O meta-bucket probe**: grouping each first-byte resolver bucket by metadata key reduced stats scan steps (`52381 -> 39725`) while preserving candidates/byte compares, but the extra nested list shape regressed the default first-4096 BL span from the retained `~0.60s` band to about `1.01s`. Source was reverted; do not replace the flat first-byte bucket with Oren-level nested meta buckets unless construction and lookup overhead are proven below the flat bucket path (2026-05-06).
- **Mach-O resolver helper split**: moved the local resolver decode/lookup/stat helper cluster into `arm64_macho_resolve.oren`. `arm64_macho.oren` now focuses on object emission and drops from the near-limit `1921` lines to `1694`, while the new resolver module is `227` lines. Default and stats profiles preserve the retained first-byte bucket counters (`BL lookups=1193`, `scan_steps=52381`, `insert_shifts=0`), and focused generator/coroutine gates plus full `make test` pass (2026-05-06).
- **ARM64 expression lowering split**: split `native_expr_chunk_noncall(...)` out of `arm64_native_expr/010_lowering_a.oren` into `arm64_native_expr/015_lowering_a_noncall.oren` and added the include to the `arm64_native_expr.oren` facade. A concatenation check proved the initial split preserved source order before normalizing moved-block whitespace for `git diff --check`; the near-limit file moves from `1938` lines to `470` plus a `1468`-line non-call module. `make stage2`, generator/coroutine surfaces, native profiling, and full `make test` pass (2026-05-06).
- **ARM64 call lowering split**: split `native_call_chunk_030_lowering_c(...)` out of `arm64_native_expr/030_lowering_c.oren` into `arm64_native_expr/035_lowering_c_calls.oren`, preserving facade include order between `030` helpers and `040` call lowering. The split was verified by concatenation before normalizing moved-block whitespace; `030_lowering_c.oren` moves from `1904` lines to `59`, while the extracted call module is `1844` lines. `make stage2`, generator/coroutine surfaces, native profiling, and full `make test` pass (2026-05-06).
- **ARM64 call-lowering core split**: split the extracted `035` dispatcher into an ordered wrapper plus `native_call_chunk_035_list_ops(...)` and `native_call_chunk_036_list_simd_ops(...)`. Whole `fn_name` blocks stay intact: list/list-int len/get/set/push cases remain in `035`, while reduce/dot/call-object/SIMD/GEMM cases move to `036`. This keeps both files below the guardrail (`927` and `925` lines), with stage2, generator/coroutine surfaces, native profiling, and full `make test` passing (2026-05-06).
- **C transpiler entry split**: split the C backend entry/part-collection tail out of `transpiler.oren` into `transpiler/090_entry.oren` through the existing textual include mechanism. A concatenation check proved byte-for-byte source-order equivalence before build verification, and the near-limit facade drops from `1999` lines to `1725` while the extracted entry module is `276` lines (2026-05-06).
- **Green runtime/parser prelude guardrail splits**: split the green scheduler steal/pop-any tail from `runtime_native/263_green/010_green_core.oren` into `010_green_core_steal.oren`, and split parser impl-declaration lowering from `parser_parse/000_prelude.oren` into `000_prelude_impl.oren`. Both use existing textual include expansion, and both were checked by expanding back to the original source order before verification; the near-limit files move to `1880` and `1742` lines respectively (2026-05-06).
- **x64/runtime guardrail split batch**: split three more near-limit source-order tails without changing behavior: Windows x64 NET IOCP intrinsics move from `046_emit_sys_intrinsics_windows_net.oren` into `046_emit_sys_intrinsics_windows_net_iocp.oren`, x64 while-loop dot fast paths move from `057_emit_ops_while_emit.oren` into `057_emit_ops_while_dot.oren`, and runtime task-group helpers move from `260_threads.oren` into `260_threads_task_group.oren`. Each split was checked by expanding the include back to the original source order; parent files move to `1095`, `1318`, and `1399` lines respectively (2026-05-06).
- **x64 core SIMD-tail split**: split the packed-SIMD and late memory-helper encoder tail from `x64_core.oren` into `x64_core_simd_tail.oren`. The split keeps scalar/core x64 encoders in the parent and uses the same textual include mechanism; expanding the include matches the original source order, moving `x64_core.oren` from `1830` to `1662` lines (2026-05-06).
- **Guardrail split batch**: split four more source-order clusters at whole-function boundaries. Green scheduler netpoll/runq/sleep/zombie/switch helpers now live in `runtime_native/263_green/010_green_core_sched.oren`, parser generator-lowering helpers live in `parser_parse/000_prelude_generator.oren`, list-int lowering/touch/rewrite helpers live in `optimizer_list_int_lower.oren`, and list-loop reset/reserve helpers live in `optimizer_loops_list_reset.oren`. Expansion checks match the original source order, moving parent files to `1252`, `896`, `927`, and `864` lines respectively (2026-05-06).
- **Backend/runtime guardrail split batch**: split five more expansion-equivalent source-order clusters. `arm64_native_stmt_inner.oren` owns the ARM64 statement inner dispatcher, `x64_native_program/040_emit_expr_eval.oren` owns the x64 integer expression evaluator, `transpiler/050_fast_while.oren` owns the C fast-while emitters, `codegen_bytecode/030_tail_loops.oren` owns bytecode list-loop specializations, and `runtime_native/100_time_core_crash_gc.oren` owns crash-footer plus GC-mode initialization helpers. Parent files move to `529`, `293`, `802`, `655`, and `911` lines respectively (2026-05-06).
- **Rejected Mach-O user-function preseed probe**: pre-seeding the append-only first-byte bucket cache from `ctx["function_names"]` passed `make stage2`, but regressed the default first-4096 BL span from the retained `~598ms` to about `1039ms`. The probe seeded `422` targets, spent about `518ms` before `i=0`, and raised BL scan/candidate counts to `56888` / `1430`, while only `40` targets would have been lazily appended. Source was reverted; do not preseed user functions unless construction is proven cheaper than the current lazy bucketed path (2026-05-06).
- **Rejected Mach-O BL name materialization probe**: rewriting later identical compact `bl_name` entries to a positive resolved target offset after the first string lookup passed `make stage2`, but the stats-enabled native profile timed out at `180s` before the first `i=4096` local BL progress mark. The phase log reached `macho.fixups.local.bl.resolve.progress i=0` and then stayed in the future-name rewrite work, proving this O(unique-names * fixups) post-codegen materialization is too expensive. Do not repeat whole-fixup-list future-name rewrites; direct target arrays must be built without scanning the full BL list per unique name (2026-05-06).
- **Rejected Mach-O ADR-code single-pass probe**: applying the same single-pass target-list deletion to the local ADR-code fixups passed `make stage2`, default profile, and stats profile, but did not improve the small ADR-code bucket. The exact logs kept ADR-code in the same noisy `~70-95ms` range while changing phase ordering, so the source probe was reverted. Keep the retained single-pass change BL-only unless ADR-code fixups become a measured large bucket (2026-05-06).
- **Rejected Mach-O local-BL previous-target probe**: a one-entry previous-target reuse inside the local BL resolver reused `1055` adjacent target names, but the measured wall-time target stayed effectively unchanged (`~1.42s` baseline to `~1.41s` for the first 4096 local BL fixups). Source was reverted; do not reopen small MRU/previous-target caches around the existing name path. The remaining fixup work still needs compact target ids/offsets or linear post-codegen materialization below repeated string/name lookup (2026-05-06).
- **Rejected Mach-O shell-preseed materialization probe**: a whole-table preseed rebuilt from `ctx["functions"]` plus `runtime_functions_enc` with an Oren-level shell sort eliminated lazy target appends (`appended=0`) and moved the BL loop itself to roughly `100ms`, but preseed/sort cost about `71.7s` for `1889` targets before the first BL fixup. Source was reverted. This rules out Oren-level whole-table sorting as the compact-materialization path; a viable design must carry target offsets/ids from lower-level codegen metadata or build direct target arrays without generic maps and without Oren-level global table sorting (2026-05-06).
- **Mach-O stale wrapper cleanup**: removed unused local forwarding wrappers from `arm64_macho.oren` (`arm64_ctx_fixup_adr_data`, `push_u32_be`, `ceil_div`, `insn_adr`, `insn_ret`). These wrappers had no in-file users and no repo-qualified external callers; the Mach-O emitter still calls the retained helpers it actually uses (2026-05-06).
- **Optimizer stale wrapper cleanup**: removed the unused `_opt_lower_int_only_lists_in_block(...)` wrapper from `optimizer_list_int.oren`. The optimizer driver already calls `_opt_lower_int_only_lists_in_block_opts(...)` directly, and recursive list-int lowering still uses the opts form through `_opt_lower_int_only_lists_in_stmt(...)` (2026-05-06).
- **Optimizer loop stale helper cleanup**: removed the unused `_opt_block_has_unsafe_list_use(...)` helper from `optimizer_loops_list.oren`. The reset-aware block unsafe-use helper remains active, and this drops the file below the 2000-line guardrail again (2026-05-06).
- **Optimizer split-invariant loop pass split**: moved the split-invariant list-push optimizer tail from `optimizer_loops_list.oren` into `optimizer_loops_split.oren`, with `optimizer_loops.oren` remaining the facade. This preserves the pass behavior and backend gating while reducing `optimizer_loops_list.oren` from the near-limit `1988`-line band to `1759` lines; focused optimizer/generator/coroutine gates and full `make test` pass (2026-05-06).
- **Compiler stale private-helper cleanup**: removed another repo-proven dead-helper batch from active compiler surfaces: `_dbg_is_ident(...)`, `_json_obj_str(...)`, `_rt_bundle_shell_quote(...)`, `_path_is_sep_byte(...)`, `_ml_parse_program_for_module_serial(...)`, and `_arm64_rtobj_clone_map_with_cap(...)`. Each helper had only its own definition as a repo hit, so this is source-shape reduction rather than behavior change (2026-05-06).
- **x64 stale private-helper cleanup**: removed repo-proven dead x64 private helpers `_elf_hash_sysv_u32(...)`, `_x64_env_enabled_default_true(...)`, `_emit_write_newline(...)`, and `_emit_parse_u64_decimal_from_cstr_ptr_x64(...)`. These definitions had no repo callers; the retained x64 paths still use their live neighboring helpers (2026-05-06).
- **Cross-module stale private-helper cleanup**: removed another repo-proven dead-helper batch across compiler and stdlib modules: `_arm64_cmp_may_use_strcmp(...)`, `_attr_list(...)`, `_arm64_rtobj_encode_fixups(...)`, `_astbin_g_read_string(...)`, `_astbin_read_string_ref_v2(...)`, `_is_meta(...)`, `_u32be_set(...)`, `_table_len(...)`, `_store_u8_buf_direct(...)`, `_load_u8_buf_direct(...)`, `_pack_list_like_with(...)`, `_new_i32_buf(...)`, `_new_i64_buf(...)`, `_new_f32_buf(...)`, and `_new_f64_buf(...)`. Each name had exactly one repo occurrence before removal, so this is source/codegen surface reduction without changing active paths (2026-05-06).
- **ARM64 stmt stale-import cleanup**: removed unused non-prelude ARM64 statement imports after narrowing the alias scan to files without textual includes: `abi` / `labi` from `arm64_native_stmt.oren` and `loopm` from `arm64_native_stmt_set.oren`. Prelude and imports-only false positives remain intentionally untouched because their aliases feed included files or bundle linking (2026-05-06).
...
- Update (2026-04-23): the next true four-stream recurrence follow-up under that same shipped four-wide body is now closed negative too. A temporary multi-stream rerun kept four carried lanes live in preserved regs and advanced each by a precomputed `4*step mod` delta across main iterations. The emitted loop still improved: `build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_061011_87026.log` and `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_061011_87005.log` show the wide main iteration dropping to `32` hot instructions for `4` outputs (`8.00` per element). But that same emitted-code pass also shows the cold GC-tick side blocks growing from `16` to `24` instructions because the helper had to spill and restore four preserved pairs instead of two. The same-tree shipped-vs-enabled decision surface (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-multi-stream-decision-20260423_060858_85589.log`) therefore still rejected promotion cleanly: fill/share preferred the shipped default (`default_fill_vs_c_vector ~2.1459×` vs enabled `~2.1862×`), exact `array_sum_int` median also preferred default (`default_array_ratio_median ~2.1979×` vs enabled `~2.2283×`), and exact `dot_product_int` median preferred default too (`default_dot_ratio_median ~1.5145×` vs enabled `~1.6533×`, `decision_surface_alignment: agree`). Reweight again: the remaining blocker is no longer just the serial recurrence in isolation; any future multi-stream retry has to avoid paying a wider safepoint spill/reset tax at the same time.
...
- Update (2026-04-23): a narrower precise-safepoint-spill follow-up under that same shipped four-wide body is now closed negative too. A temporary rerun changed only the inline GC tick helper, replacing the conservative two-pair cursor spill with the exact live-pointer spill `[x19]` through `stp x19, xzr` / `ldp x19, xzr`. The emitted hot loop did not change: `build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_062415_90653.log` and `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_062415_90632.log` still show `35` hot instructions for `4` outputs (`8.75` per element), but the cold GC-tick side blocks shrink from `16` instructions on the clean shipped baseline (`build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_061450_87903.log`, `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_061450_87882.log`) to `12`. That cleaner side path still does not hold on the actual shipped surface: compared with the clean baseline decision probe (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-decision-20260423_045547_59235.log`), the precise-spill rerun (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-decision-20260423_062307_89324.log`) improved fill/share (`default_fill_vs_c_vector ~2.2247× -> ~2.1355×`) but regressed both exact same-tree medians (`default_array_ratio_median ~2.1889× -> ~2.2115×`, `default_dot_ratio_median ~1.7420× -> ~1.7738×`). Reweight again: exact live-pointer conservative spills are cleaner, but not enough to keep as a standalone shipped perf move; any future retry must pair that narrower spill contract with a stronger recurrence/control win or a correctness requirement.
- Update (2026-04-23): a spill-neutral two-stream recurrence follow-up under that same shipped four-wide body is now closed negative too. A temporary rerun split the carried values into two live streams (`x24/x25`) plus a shared `2*step mod` delta in `x26`, but kept the shipped safepoint spill width unchanged (`[x19,x20]` and `[x21,x22]`). The emitted loop only improved slightly: `build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_063816_94224.log` and `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_063815_94203.log` show the wide main iteration dropping from `35` hot instructions for `4` outputs (`8.75` per element) to `34` (`8.50` per element), while the cold GC-tick side blocks stay at `16`. The actual same-tree decision surface (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-two-stream-decision-20260423_063427_92506.log`) still rejected promotion cleanly on all tracked metrics: fill/share preferred the shipped default (`default_fill_vs_c_vector ~2.1431×` vs enabled `~2.2392×`), exact `array_sum_int` median also preferred default (`default_array_ratio_median ~2.2033×` vs enabled `~2.2142×`), and exact `dot_product_int` median preferred default too (`default_dot_ratio_median ~1.5928×` vs enabled `~1.6949×`, `decision_surface_alignment: agree`). Reweight again: the remaining blocker is no longer just “multi-stream without a wider safepoint spill tax”; even that narrower recurrence split loses to the shipped serial four-wide body.
- Update (2026-04-23): a narrower remaining-count control follow-up under that same shipped four-wide body is now closed negative too. A temporary rerun kept the shipped safepoint spill width and store shape intact, but replaced the carried `i` + `n - i` recompute with a carried `remaining` count and reconstructed the final idx/count only at loop exit. The emitted loop still improved slightly: `build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_070103_440.log` and `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_070107_517.log` show the wide main iteration dropping from `35` hot instructions for `4` outputs (`8.75` per element) to `34` (`8.50` per element), and the per-output-element category mix only changes in arithmetic (`2.75× -> 2.50×`) while `stores 1.00`, `moves 1.25`, `compare/tick 1.75`, and `branches 2.00` stay flat. The actual same-tree decision surface (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-remaining-decision-20260423_065955_98963.log`) still does not justify keeping it: fill/share and exact `array_sum_int` improve slightly (`default_fill_vs_c_vector ~2.2233×` vs enabled `~2.1710×`, `default_array_ratio_median ~2.1913×` vs enabled `~2.1894×`), but exact `dot_product_int` regresses (`default_dot_ratio_median ~1.7179×` vs enabled `~1.7739×`, `decision_surface_alignment: agree`). Reweight again: the remaining gap is not an `n - i` bookkeeping problem by itself; future retries need to change the control/store mix more fundamentally or pair a similar arithmetic shave with a stronger whole-program dot win.
- Update (2026-04-23): a narrower peeled-tail follow-up under that same shipped four-wide body is now closed negative on correctness before it even reaches the ranking surface. The temporary rerun enabled `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR_UNROLL4_PEELED_TAIL=1` and peeled the scalar remainder out of the wide body, but the enabled fill-share wrapper log (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-peeled-tail-decision-20260423_071903_4101.enabled-fill.log`) fails because the generated `fill_list_int_oren_native` exits nonzero. The dedicated repro log (`build/logs/verify_fill_list_int_repeat_stability_20260423_peeled_tail_repro_v1.log`) shows the branch is repeated-invocation unsafe: `n=2000000 reps=1` still returns `11`, but `n=2000000 reps=2` panics with `Index out of bounds`. The shipped verifier now closes that gap too: `make verify-native-list-int-fast-lowering` honors `OREN_BENCH_ENV_BUILD_OREN` for its arm64 builds and runs `fill_list_int 2000000 2` as a repeat-stability runtime guard, so future fast-push experiments have to survive repeated invocation before they count as viable.
- Update (2026-04-23): a spill-budget-neutral unroll4 no-wrap split is now also closed negative. The temporary branch enabled `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR_UNROLL4_NOWRAP_SPLIT=1` and guarded the existing wrap-capable wide body behind a single `current < mod - 3*step` fast-path check, so the common no-wrap case stopped paying the per-lane wrap-control sequence. The real same-tree decision surface (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-nowrap-split-decision-20260423_074857_16234.log`) still does not justify shipping it: fill/share improves strongly (`default_fill_vs_c_vector ~2.1517×` vs enabled `~1.7698×`) and exact `array_sum_int` median also prefers enabled (`default_array_ratio_median ~2.1881×` vs enabled `~2.1519×`), but exact `dot_product_int` regresses across every sweep (`default_dot_ratio_median ~1.5254×` vs enabled `~1.7467×`). The branch was otherwise correct and repeat-stable (`build/logs/verify_native_list_int_fast_lowering_20260423_075050_17727.log`), so the rejection is performance-only, not another correctness gap. In the same slice, the fill hot-loop probe was tightened so split-path loops measure their dominant fast subpath instead of statically charging the rare fallback block too: the refreshed compare logs (`build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_075358_18627.log`, `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_075357_18606.log`) now report `main_iter_kind=split_fast_path` at `23` hot instructions for `4` outputs (`5.75` per element, categories `stores 1.00 / arith 2.00 / moves 0.00 / compare_tick 1.25 / branches 1.50`). Reweight again: even a common fill-side path that beats the host C vector body on static per-element count is not enough by itself; future retries must preserve that dominant-path win without paying enough rare-wrap/control-side cost to hurt exact `dot_product_int`.
- Update (2026-04-23): the exact-program fill-mix attribution gap for that same single-list unroll4 family is now closed more cleanly. `make perf-probe-arm64-fast-push-exact-fill-mix` now defaults to `array_sum_int,array_sum_int_step7,dot_product_int`, and the refreshed summary (`build/logs/perf-probe-arm64-fast-push-exact-fill-mix-20260423_081815_23618.log`) shows both single-list sum benchmarks are directly causal for this family (`fill_pushes_per_iter: 1`, `single_list_unroll4_applicable: yes`) while `dot_product_int` remains structurally ineligible (`fill_pushes_per_iter: 2`, `single_list_unroll4_applicable: no`, `ineligible_reason: pushes_per_iter!=1 blocks single_list_cursor/unroll4 gate`). Their isolated fill streams are all mostly no-wrap (`array_sum_int 98.8000%`, `array_sum_int_step7 98.0000%`, `dot_product_int` streams `99.2000%` and `98.0000%`), but only the two single-list programs are causal for this branch family.
- Update (2026-04-23): the first causal single-list family decision surface is now in-repo too, and it exposes a real disagreement on the current shipped tree. `make perf-probe-arm64-fast-push-nonneg-linear-unroll4-single-list-decision` (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-unroll4-single-list-decision-20260423_082026_25099.log`) keeps the existing fill/share surface but ranks exact programs as causal single-list benchmarks (`array_sum_int,array_sum_int_step7`) plus a separate non-causal control (`dot_product_int`). The result does not align with the earlier fill/share-only story: fill/share still prefers the shipped default (`default_fill_vs_c_vector ~2.0473×` vs disabled `~2.3812×`), but both causal exact programs slightly prefer `disabled` on steady native/C (`array_sum_int ~2.0855×` vs `~2.0782×`, `array_sum_int_step7 ~2.0855×` vs `~2.0782×`) and the non-causal control also slightly prefers `disabled` (`dot_product_int ~5.1433×` vs `~5.1370×`), so `decision_surface_alignment: disagree`. Reweight again: before more local single-list control-form tuning, either the current shipped unroll4 default needs a stronger causal steady-surface defense or it needs a direct re-check against this narrower family surface.
- Update (2026-04-23): the fill-vs-C disasm probe now emits per-category counts for the shipped four-wide body itself, which sharpens the remaining blocker again. `build/logs/perf-probe-arm64-list-int-fill-hot-loop-disasm-20260423_064855_96771.log` and `build/logs/perf-probe-arm64-fill-vs-c-loop-compare-20260423_064900_96827.log` still show the landed Oren main iteration at `35` hot instructions for `4` outputs (`8.75` per element), but now also quantify the category mix per output element: Oren spends `stores 1.00`, `arith 2.75`, `moves 1.25`, `compare/tick 1.75`, and `branches 2.00`, while the host C vector ceiling through `LBB0_15` spends `stores 0.50`, `arith 5.00`, `moves 0.25`, `compare/tick 0.50`, and `branches 0.50` at `6.75` instructions per element. Reweight again: the remaining gap is not “not enough arithmetic” by itself; it is the extra control/move/store overhead around Oren's serial recurrence. Future retries should only ship if they trade that control-state maintenance for more independent arithmetic work, not if they merely shave arithmetic ops in isolation.
...
- **Feature set maturity**: the remaining essential language backlog is now full
  ...
  coroutine-oriented alias facade `std:coroutine` (`reflect.is_coroutine(v)` /
  `reflect.is_coroutine_context(v)` currently match the same generator handle/context tags).
...
- **Native backend**: Tier‑1 intent only; tagged‑value convergence still rolling; GC/allocator correctness is improving but not yet stable at production gates; hot‑loop parity still above target. The ARM64 self-host generator surface is now much healthier after the 2026-05-05 generic-call/string-literal cleanup: repeated default profiles put `user_decls` around `4.26s / 292000 bytes / 284 funcs`, down from the post-ABI-cache `~10.06s / 292000 bytes` baseline, with the remaining visible call row centered on runtime-observable generator tracing rather than broad ABI marshalling. Top-level integer-global preinit now also removes side-effect-free literal integer global assignments from `__top_level__` on non-gas builds; gas-accounting modes keep the old statement path so native gas surfaces stay observable. The link optimizer is now subprofiled under the standard phase log, and a conservative list-touch prefilter limits list-int/reserve optimizer passes to 119/470 function bodies in the current generator profile, moving optimizer time from about `3.76s` to `3.31s` while keeping `user_decls` around `4.0s / 292000 bytes`. That list-touch prefilter now lives in shared optimizer helpers, so top-level candidate filtering and nested list-int lowering use the same conservative shape detector. The same `optimizer.summary` row now embeds top fold/list-int/reserve function bodies without adding extra phase-log append rows; the first retained profile showed generator surface fixture bodies dominated residual list-pass time. Focused generator-surface fixture resplits now keep the same assertions while splitting basic, close, finalize, timeout, iteration, declaration, and cancel-hook chunks; the latest profile moves optimizer total from about `3.16s` to about `1.98s`, `list_int_ms` from about `1.16s` to about `0.21s`, and the largest list-int body from about `135ms` to about `28ms`. The summary now also reports list-pass internals: the list-int pass now avoids an unnecessary recursive-lowering seed clone, skips no-op recursive lowering for statement forms with no lowerable body, uses the shared list-touch detector to avoid lowering nested bodies that cannot contain list work, only scans list uses once a current-block candidate exists, and only runs touch marking after an empty-list candidate exists; together these move `list_int_scan_ms2` from about `863ms` to about `173ms`, with `lower` about `68ms`, `use_scan` about `40ms`, and `touch` about `47ms`. The retained reserve cleanup skips a duplicate top-level `Var`/`Assign` safe-int update and keeps reserve around `0.25s` (`reserve_safe_ms` about `220-225ms`). A narrower reserve-state guard lowered `reserve_safe_ms` by only about `9ms` and worsened repeated optimizer totals (`~2.05-2.07s`), so it was reverted. A narrower list-int-only prefilter probe reduced the candidate count to `86/470`, but raised scan overhead to about `53ms`, left `list_int` around `1.44s`, and worsened optimizer total to about `3.42s`, so the source probe was reverted. A fold-pass list-length-hoist cleanup also passed stage2 and focused optimizer/generator gates, but repeated profiles left `fold_ms` around `0.97-1.02s` and optimizer total around `3.40-3.43s`, so that source probe was reverted too. A pass-order probe that moved global DCE before the optimizer passed focused surfaces but regressed the default profile (`optimizer` about `5.1s`, `user_decls` about `7.2s`), so it remains rejected.
- Optimizer fold update (2026-05-05): `optimizer.summary` now includes gated fold-shape counts, and the retained fold cleanup hoists stable lengths in fold-local loops, returns early for leaf expressions, skips recursive folding for leaf children, skips child loops for calls whose callee/args/spread are all leaves, avoids clone-of-clone environments for isolated `if`/`for` folding, avoids a redundant empty environment push for function-expression bodies, and gates fold trace string construction behind the already-computed `OREN_TRACE_OPTIMIZER` flag. The measured profile moved optimizer total from about `1.98s` to about `1.35s`, `fold_ms` from about `833ms` to about `252ms`, `fold_env_clone` from `2592` to `1317`, and `fold_env_push` from `3754` to `3192`; the current fold count table shows `8398` non-leaf folded expressions after child-leaf skipping, led by `2644` calls, `2489` infix expressions, `1271` expression-level `if` nodes, and only `22` remaining other expressions. A broader leaf-route skip for leaf-only prefix/index/member/infix expressions was tested next; early profiles showed `fold_ms` around `437-438ms`, but the final exact-tree profile regressed to `451ms` and optimizer total `1.58s`, so the source probe was reverted. The older reverted fold hoist above refers to the broader neutral stable-list-length probe before list-pass and fixture-shape reductions; do not repeat these fold probes without new evidence.
- Optimizer const-prop update (2026-05-05): `optimizer.summary` now includes gated const-prop shape counters, and `make profile-native-build-phases` prints an `optimizer const pass counts` table. The profile proved the current generator surface had no `% identifier` rewrites (`const_mod_rewrite=0`) while the old pass still walked `14482` expressions, `4126` statements, and cloned/intersected environments `3803` times. The retained conservative prefilter scans for the exact `% <identifier>` shape before running the env-mutating const-mod rewrite pass, moving `const_ms` from about `144ms` to about `20ms` and optimizer total from about `1.35s` to about `1.23s` on the refreshed native profile. The positive path is covered by `tests/fixtures/optimizer_const_mod_rewrite_main.oren`, now included in native quick integration.
- Seed fix (2026-05-05): `scripts/build_rtobj_seed.sh` now recognizes schema-versioned runtime-object cache keys (`s3_b_*` and future `sN_b_*`) instead of the stale `s2_b_*` prefix, and `make rtobj-seed` now warms both core and full native runtime profiles for no-debug/debug host seeds. It can also copy from a compatible persistent fallback seed into isolated active seed dirs, and it refuses cross-compiler cold rtobj seed builds by default because current keys validate runtime/backend signatures, not the compiler binary that emitted the runtime bytes. This closes the local `std:linalg` full-runtime no-debug seed miss that made `verify-backend-parity-list-int` fall into `rtobj.miss.build.start` despite a compatible cache entry, and prevents direct stage2 quick-autoseed from manufacturing a stage1-emitted runtime object that can crash the stage2-built smoke binary.
- Module parse profiling (2026-05-05): native build phase profiles now emit serial `link.parse_module.done` rows and `scripts/profile_native_build_phases.sh` prints a `module parse by path` table with read/cache/parse/lexer/parser/merge/prepare plus parser-body, hot-statement, and generator-core sub-buckets. The parser now caches the generated generator-core source string and parsed generator-core statement template on the normal non-forced-GC compiler path, cloning the cached AST before prepending it so later in-place compiler passes do not share mutable nodes across modules; both caches are disabled when `OREN_COMPILER_GC=1|true` because compiler globals are not yet guaranteed native GC roots under forced compiler GC. Repeated refreshed profiles show `lib/std/generator.oren` around `317-330ms / 137 stmts`, with `gen_core_ms=25-26` and `gen_core_parser_ms=0`; before the source-string cache it was about `1.40s` with `gen_core_ms=1112`, and the source-string-only cache boundary was about `481ms` with `gen_core_ms=188`. A semantics-preserving task/task-group body cleanup now shares task-group member stop/watch/wait/terminal-result loops, task-only join/detach loops, and the task timeout-detach block; the focused task/task-group/generator/coroutine surfaces pass, and a follow-up primitive-tag cleanup removed local `_task_*` / `_task_group_*` int/string wrappers in favor of direct `oren_type_tag(...)` checks. The current profile puts `lib/std/task_group.oren` around `444ms / 64 stmts` with `parse_body_ms=272` and `lib/std/task.oren` around `306ms / 51 stmts`, while the former `terminal_results` and `join_all` hot rows remain out of the top three. Remaining module-parse work should target `_task_group_validate_policy_shape`, residual `stop_policy_wait`, `_task_stop_fields`, and any deeper generator-core representation only if it beats the cloned-AST cache without importing ASTBIN codec codegen, not function headers, lexer setup, file reads, cache probing, preparation, or the smaller `reflect/result/list` tail. A task-group policy-shape helper extraction was rejected after it passed stage2 but caused a repeatable bytecode-only `verify-task-group-task-surface-v0` failure at `191096`, while C/native stayed green; the shipped tree keeps the original inline validation shape. A separate `std:task.stop_policy_wait` stop-mode helper split also passed stage2 and focused task/task-group/generator/coroutine gates, but moved `lib/std/task.oren` to about `339ms / 54 stmts` and merely shifted the hot row into `_task_stop_policy_wait_stop_mode`, so it was reverted. An opt-in serial module-cache write probe was rejected because writing only the first two ASTBIN cache entries pushed the profile to the 180s timeout, with `lib/std/time.oren` alone reporting about `114.5s` including encode/write cost. A separate parser-side generator-core ASTBIN template-cache probe was also rejected: it reduced visible `lib/std/generator.oren` `gen_core_ms` to about `41ms`, but importing ASTBIN machinery into the parser path made the native profile time out at 180s later in native emit/codegen. A parser trace-env cache probe for `OREN_TRACE_PARSE_FN*` was also reverted after repeated profiles moved task/task-group parse rows slightly worse instead of proving a win.
- **AVM backend**: deterministic VM; single‑threaded today; heap uses `malloc` (no GC yet); opcode/ABI stability is rolling; capability gating exists but maturity is below production. iOS readiness inspection (2026-05-07) found no Xcode/Swift/iOS app integration target in this repo, only a Cocoa UI source file; the shipped AVM build path emits a host `avm` executable rather than a static library/xcframework, and the public C API still carries fixed global/frame/stack limits, rolling capability/budget fields, global allocation-owner state, and abort-on-leak teardown. Curated `make avm && make test-avm` passes, but wildcard fixture coverage is not release-grade: `AVM_TESTS="tests/avm/*.oren"` fails at `test_budget_io_fs` because the Makefile lacks per-fixture env/expected-failure metadata, even though a direct `AVM_IO_BYTES=128` run correctly returns `AVM_ERR_BUDGET`. Verdict: libavm is not production-ready for an iOS app until an embeddable library packaging/API gate and full fixture manifest exist.
...
- **Compiler‑in‑AVM**: fixture-level bootstrap only; not production-ready. The repo has VFS harnesses that load `build/oren_compiler.obc` and optionally `build/stdlib_bundle.obc` to compile small bytecode programs in a nested AVM universe, but no stabilized iOS resource packaging, default release gate, compiler `.obc` production artifact target, or AVM-hosted compiler pipeline entrypoint is documented as ready.
...
5) **W4 - Feature set completeness (essential modern features)**
   ...
   - New (2026-05-04): `std:net/http` and `std:net/ws` now expose structured wrappers over their
   ...
   - New: native quick integration now guards both success and failure paths for module visibility:
     ...
     `pub`.
   - New (2026-05-04): `std:ui/color`, `std:ui/commands`, `std:ui/raster`, and `std:ui/ppm`
     now expose structured `try_parse_hex`, `try_validate`, `try_rasterize`, `try_encode_rgba`,
     and `try_write_rgba_ppm` aliases over their existing value-or-error surfaces, and the result
     smoke / AVM UI tests guard those names directly.
   ...
   - New (2026-04-22): `--strict-yield-lowering-v0` is now a real compiler policy gate for
     ...
     artifact-cache restore to avoid cached non-strict outputs bypassing the check.
	   ...
	   - New (2026-04-23): that parity verifier now also emits a backend-neutral
	     `[yield_lowering_v0]` summary for bytecode, C, and native, so the same prepared-plan shape is
	     visible in all three build paths instead of only through the older AVM-only lowering trace.
		     ...
		     - supported policy keys are:
			   ...
			   - New (2026-04-23): `std:task` now ships as the safe generic `spawn` facade:
			     ...
			     - `task.request_cancel_after(...)` / `request_cancel_after_wait(...)` and
				   ...
				   - zero-delay or already-expired `*_wait(...)` calls now apply the request synchronously
			     ...
			     - `task.join(...)`, `join_timeout(...)`, `detach(...)`, and `join_all(...)` wrap the raw
				   ...
				   - `join_timeout(...) == -60` now preserves the live handle for later join/cancel/detach
				     ...
				     - `task.cancel(...)`, `cancel_after(...)`, `cancel_after_wait(...)`, `cancel_at(...)`,
				       and `cancel_at_wait(...)` now ship as bounded task-cancel helpers: they record the
				       same cooperative sticky request first, then use the safe join/detach stop path instead
				       of an unsafe preemptive worker kill
				     ...
				     - that task stop surface now accepts `mode="request_cancel"`, `mode="cancel"`, and
					   ...
					   - `stop_policy(...)` always returns a watcher handle; `stop_policy_wait(...)` returns `nil`
					     for `mode="request_cancel"` and returns the `{status, result, reason, detach_result}`
					     map for `mode="cancel"` or `mode="stop"`, with `join_timeout_ms` as an explicit
					     synchronous wait-budget override
							      ...
							      - task members now use the same shared `std:task` contract, including cooperative
							        ...
							        cancel/stop execution is runtime-owned through `oren_task_cancel_now(...)`
									      - `std:task.stop_capabilities()` now exposes the current runtime stop boundary
									        as data: immediate cancel-now, cancel-request state, bounded cancel-wait,
									        and delayed synchronous cancel-wait are runtime/scheduler-backed across
									        C, native, and AVM; AVM uses dedicated scheduler opcodes for the wait paths
							    ...
							    - the remaining boundary is now narrower: runtime-backed groups are already unified and
							      runtime-owned for mixed membership, stored default policy, and atomic
									      ...
									      ships as cooperative request plus bounded stop/detach; immediate task stop
											      ...
											      `oren_task_cancel_wait(...)`, and `oren_task_cancel_after_wait(...)`
											      across C, native, and AVM. Generator/coroutine typed stop execution still
											      lives in stdlib instead of wholly in the runtime scheduler.
...
- Rejected newest-first local resolver scan (2026-05-05): reversing the local target-cache traversal
...
- ARM64 literal branch trait-probe hoist (2026-05-04): direct `if` comparisons against `true`,
  ...
  a follow-up also reuses already-known string-trait results inside the guarded `strcmp` decision
  instead of walking both operands again. Treat this as safe constant-factor guard cleanup, not the
  final wall-time fix.
...
5) **AVM iOS embeddability + compiler-in-AVM release gate** (L, W4)
   - Reweight (2026-05-07): libavm is not production-ready for an iOS app. This repo has no iOS app/Xcode target for libavm; `make avm` builds a host arm64 Mach-O executable, not a static library/xcframework or Swift/ObjC embedding surface.
   - Blockers before production iOS use: stable C embedder API with no CLI-owned lifecycle assumptions, reentrant allocator ownership, deterministic error returns instead of abort-on-leak teardown, release packaging for iOS simulator/device, a fixture manifest that captures per-test budget/env/expected-failure policy, and a default gate that exercises compiler-in-AVM with compiler and stdlib OBC resources.
   - Evidence: `make avm && make test-avm` passes on the curated list, `AVM_TESTS="tests/avm/*.oren"` fails at `test_budget_io_fs` under the current generic harness, and a direct `AVM_IO_BYTES=128` run of that fixture returns `AVM error: code=9 msg=budget exceeded (io)`.
6) **AVM full-suite manifest coverage** (M, W4)
   - Reweight (2026-05-07): wildcard `AVM_TESTS="tests/avm/*.oren"` is documented as full coverage but is not a valid release gate because fixture-specific budgets, expected failures, and backend policies are encoded only in selected Makefile cases. Add a manifest-driven runner before treating AVM/iOS maturity as production-grade.
7) **Local agent UI polling/backoff** (S, W3)
...
2) **AVM multiverse maturity (budgeted child universes + snapshot/restore)** (M, W2)
3) **Scientific/AI acceleration roadmap (SIMD kernel coverage + GPU/BLAS path)** (M, W2)
4) **Allow non-macOS hosts for partial targets** (S, W2)
5) **Package manager / signed module workflow** (M, W2)
6) **Refactor oversized native emitters (>2000 lines)** (M, W2)
7) **Keep large-file scans empty after the current refactor pass** (M, W2)
...
| NET substrate (TCP/UDP) | Rolling | `lib/runtime_native/240_tcp.oren`, `250_udp.oren`; `lib/std/net/tcp.oren` / `lib/std/net/udp.oren` (`try_*` adapters) | `tests/native/test_net_suite.oren` |
