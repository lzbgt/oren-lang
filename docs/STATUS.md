...
- Module parse profiling (2026-05-05): native build phase profiles now emit serial `link.parse_module.done` rows and `scripts/profile_native_build_phases.sh` prints a `module parse by path` table with read/cache/parse/lexer/parser/merge/prepare sub-buckets. The refreshed generator profile shows `lib/std/generator.oren` as the dominant parse module at about `1.43s / 137 stmts`, with `parser.parse_program` accounting for about `1.32s` while lexer setup rounds to `0ms`; `lib/std/task_group.oren` is about `502ms / 62 stmts` with `317ms` in `parse_program`, and `lib/std/task.oren` is about `321ms / 52 stmts` with `211ms` in `parse_program`. Future module-parse work should target parser-level work in those concrete files, especially generator-core/generator-policy parsing shape, not file reads, cache probing, preparation, or the smaller `reflect/result/list` tail. An opt-in serial module-cache write probe was rejected: writing only the first two ASTBIN cache entries pushed the profile to the 180s timeout, with `lib/std/time.oren` alone reporting about `114.5s` including encode/write cost, so serial cache seeding needs a different design.
