...
- The first GUI bridge slices now exist as binary GFX mailboxes. Bytecode can
  ...
  builder-side Wavefront OBJ `obj_source` / `obj_text`, binary-or-ASCII STL `stl_source`, inline ASCII STL `stl_text`, binary-or-ASCII PLY `ply_source`, inline ASCII PLY `ply_text`, and PLY face/vertex colors lowered to `mesh3d_rgba`,
