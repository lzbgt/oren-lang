...
- Native ops plus ARM64 Mach-O/ELF, ARM64 expression/program/global/root
  ...
  literal-slot helper, atomic scratch, infix state, while fast-path, FFI,
  data-table, toplevel, and
...
- X64 system-intrinsic cleanup now separates Linux `sys_write` label/body/finish
  ...
  CreateProcess temp, call-area layout, and process helper records now use
  compact positional state as well. Windows x64
  ...
  positional WSAStartup, select, socket, sockaddr, fd2, sockopt, sendto/recvfrom,
  close, overlapped WSA message, fcntl, create/post IOCP, GQCSX, and CancelIoEx
  records.
			  ...
			  prove both under real runtime allocation hooks, safepoint roots, and
				  ...
				  descriptor provenance through list origins, local aliases, and
					  ...
					  has `%oren_llvm_bytes { len, data, owner_kind }` descriptor handles
						  ...
						  u16/u32 endian reads, `oren_bytes_set_u8`, `oren_bytes_to_hex`,
