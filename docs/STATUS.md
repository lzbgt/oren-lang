...
- The first GUI bridge slices now exist as binary GFX mailboxes. Bytecode can
  ...
  target frame budget, budget-usage permille, over-budget status, geometry vertex count, and text-run count. Retained image resources are now available for sprite-like draws; retained image destruction/lifetime, atlas batching, and text atlas resources remain next. UIKit/CoreGraphics
