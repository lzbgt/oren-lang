...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  Contiguous u8 slice and dense u8 matrix copies from `u8_buf` now use the
  shared byte-span `bytes.copy_into` path before falling back to checked
  per-element view stores.
