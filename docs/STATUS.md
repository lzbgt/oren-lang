...
- Oren LSP typed member assignment now records dotted member-path writes for
  direct struct fields and list/list-of-list container member paths, then clears
  stale navigation/completion evidence on unknown member reassignment. Regression
  proof lives in `internal/orenlsp/member_symbols_test.go`.
...
- Oren language tooling: initial `oren-lsp` stdio JSON-RPC server now covers
  ...
  field-chain inference, returned-map value field-chain inference,
	  ...
	  and imported constructor-field nested container field-chain inference, returned/imported nested container
		  ...
		  indexed map value field-chain inference, and typed dotted member-path
		  assignment propagation with unknown member reassignment invalidation,
