...
- **list<int> hot-loop parity**: the canonical arm64 whole-operation path is materially healthier than it was earlier in the day, but still not at production parity. The exact current whole-operation ceiling rerun (`build/logs/perf-probe-list-int-c-ceiling-20260409_163202_21950.log`) puts `oren_array_sum_int / array_slot64_vector` at `~2.3939×` and `oren_dot_product_int / dot_slot64_vector` at `~1.8678×`. The important shipped change behind that improvement is the explicit get-sum unroll2 promotion in `lib/compiler/arm64_native_stmt_loops_list_emit.oren`: the earlier crashy candidate was root-caused to reserved heap-register clobber in the experimental unrolled bodies (`X27` / `X28` reused as loop temporaries), now fixed by using caller-saved `X12` / `X13` instead. `OREN_ARM64_FAST_LIST_INT_GET_SUM_UNROLL2` therefore now ships on by default for single-read-list shapes, and the broad integrated gates pass on that promoted tree (`build/logs/make_test_get_sum_unroll2_promote_20260409.log`, `build/logs/runtime_robustness_w5_20260409_163313.log`). The paired default-vs-disabled acceptance wrapper (`build/logs/perf-probe-arm64-fast-get-sum-unroll2-list-int-20260409_163132_20811.log`) stayed noisy and locally favored the disabled branch, so the shipped decision surface for this path is the exact whole-operation C ceiling plus integrated green lanes, not that micro-probe alone. Reweight accordingly: the remaining blocker is still the repeated `array_sum_int` get-sum path, but it is no longer the earlier `~5.8×` emergency; `dot_product_int` remains materially above the slot64 host-C ceiling within the current ABI.
	    ...
	    - Arm64 canonical steady tick-mask probe (`make perf-probe-arm64-fast-loop-tick-masks-steady`,
	      ...
	      default remains `4095`.
		    ...
		    - Arm64 explicit get-sum unroll2 promotion (`2026-04-09`): the earlier crashy candidate
		      ...
		      `build/logs/runtime_robustness_w5_20260409_163313.log`. The paired acceptance wrapper
		      (`build/logs/perf-probe-arm64-fast-get-sum-unroll2-list-int-20260409_163132_20811.log`)
		      stayed noisy and locally favored the disabled branch, so exact whole-operation ceiling
		      plus integrated green lanes are now the decision surface for this shipped path.
