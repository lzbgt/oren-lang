...
7) **W3 - Docs fidelity + regression gates**
   ...
   - Gas-surface calibration now includes default smoke, loop-heavy, and branch-heavy fixtures plus
     an `oren.gas-surface-conversion-decision.v0` blocker, keeping package-policy gas conversion
	     disabled until native instruction-equivalent gas evidence exists. The first block-weighted native
	     surface still spans `~1.1357x` to `~13.3761x` native ticks per AVM opcode gas across the calibration
	     set (`build/reports/backend_gas_surface_calibration_set_20260412_073423_25794.json`).
