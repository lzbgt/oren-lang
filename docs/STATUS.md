...
- `std:math/mat4` now includes checked basis editing helpers (`with_basis`,
  ...
  3x3 linear part.
- `std:math/mat4` now includes checked row/column editing helpers (`with_row`,
  `set_row`, `with_column`, `set_column`, `with_col`, and `set_col`) that
  replace one finite 4-cell row or column while preserving untouched cells.
