...
- `std:net/url` provides pure HTTP/WebSocket URL parsing shared by native
  ...
  the shared request-target composer, and native HTTP wire requests strip URL
  fragments through the shared `request_target`.
...
- AVM app-facing stdlib hot paths now prefer raw bytes: `std:ui/avm` has
  ...
  query builders while native HTTP strips fragments from wire request targets,
...
- Oren language tooling: initial `oren-lsp` stdio JSON-RPC server now covers
  ...
  constructor/alias/factory/call-site/parameter-return evidence remains.
