...
1) **W5 - Native integer hot-loop parity (loop_sum, dot_product)** (L)
   - Baseline (arm64 native, 2026-02-20): `loop_sum` 3.38× C, `dot_product` 10.13× C.
...
2) **W5 - Allocation/GC overhead reduction (alloc_churn, alloc_drop)** (L)
   - Baseline (arm64 native, 2026-02-20): `alloc_churn` 24.70× C, `alloc_drop` 31.56× C.
   ...
   - Heuristic: loops without a simple literal upper bound default to per‑iteration mode;
     ...
     - New: const‑int bounds (including prior locals assigned a literal) >= `OREN_ARENA_LONG_LIVED_BOUND`
       (default 1,000,000) are treated as long‑lived (per‑iteration).
...
3) **W4 - List reserve + unchecked push** (M)
   - Baseline (arm64 native, 2026-02-20): `array_sum` 6.63× C, `multi_list_push_int` 3.19× C.
...
5) **W3 - SIMD/typed-buffer parity on native (x64 + arm64)** (M)
    - Baseline (arm64 native, 2026-02-20): `dot_product_int` 4.17× C.
