...
- **Native backend**: Tier‑1 intent only; tagged‑value convergence still rolling; GC/allocator correctness is improving but not yet stable at production gates; hot‑loop parity still above target. The ARM64 self-host generator surface is now much healthier after the 2026-05-05 generic-call/string-literal cleanup: repeated default profiles put `user_decls` around `4.26s / 292000 bytes / 284 funcs`, down from the post-ABI-cache `~10.06s / 292000 bytes` baseline, with the remaining visible call row centered on runtime-observable generator tracing rather than broad ABI marshalling. Top-level integer-global preinit now also removes side-effect-free literal integer global assignments from `__top_level__` on non-gas builds; gas-accounting modes keep the old statement path so native gas surfaces stay observable. The link optimizer is now subprofiled under the standard phase log, and a conservative list-touch prefilter limits list-int/reserve optimizer passes to 119/470 function bodies in the current generator profile, moving optimizer time from about `3.76s` to `3.31s` while keeping `user_decls` around `4.0s / 292000 bytes`. The same `optimizer.summary` row now embeds top fold/list-int/reserve function bodies without adding extra phase-log append rows; the first retained profile showed generator surface fixture bodies dominated residual list-pass time. Focused generator-surface fixture resplits now keep the same assertions while splitting basic, close, finalize, timeout, iteration, declaration, and cancel-hook chunks; the latest profile moves optimizer total from about `3.16s` to about `2.02s`, `list_int_ms` from about `1.16s` to about `0.29s`, and the largest list-int body from about `135ms` to about `32ms`. The summary now also reports list-pass internals: the list-int pass now avoids an unnecessary recursive-lowering seed clone, skips no-op recursive lowering for statement forms with no lowerable body, only scans list uses once a current-block candidate exists, and only runs touch marking after an empty-list candidate exists; together these move `list_int_scan_ms2` from about `863ms` to about `269ms`, with `lower` about `162ms`, `use_scan` about `40ms`, and `touch` about `49ms`. The retained reserve cleanup skips a duplicate top-level `Var`/`Assign` safe-int update and keeps reserve around `0.25s` (`reserve_safe_ms` about `220-225ms`). A narrower list-int-only prefilter probe reduced the candidate count to `86/470`, but raised scan overhead to about `53ms`, left `list_int` around `1.44s`, and worsened optimizer total to about `3.42s`, so the source probe was reverted. A fold-pass list-length-hoist cleanup also passed stage2 and focused optimizer/generator gates, but repeated profiles left `fold_ms` around `0.97-1.02s` and optimizer total around `3.40-3.43s`, so that source probe was reverted too. A pass-order probe that moved global DCE before the optimizer passed focused surfaces but regressed the default profile (`optimizer` about `5.1s`, `user_decls` about `7.2s`), so it remains rejected.
