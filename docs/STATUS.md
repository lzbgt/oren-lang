...
- Optimizer fold update (2026-05-05): `optimizer.summary` now includes gated fold-shape counts, and the retained fold cleanup hoists stable lengths in fold-local loops, returns early for leaf expressions, and avoids clone-of-clone environments for isolated `if`/`for` folding. The measured profile moved optimizer total from about `1.98s` to about `1.79s`, `fold_ms` from about `833ms` to about `641ms`, and `fold_env_clone` from `2592` to `1317`; the current fold count table shows `19903` expressions, dominated by `11527` leaf/other expressions plus `2644` calls, `2489` infix expressions, and `1271` expression-level `if` nodes. The older reverted fold hoist above refers to the broader neutral stable-list-length probe before list-pass and fixture-shape reductions; do not repeat that probe without new evidence.
