...
- **Performance parity**: native hot loops remain >2× C (see perf tracker baselines: `loop_sum` 3.37×, `dot_product` 4.23×; `alloc_churn` 6.16×, `alloc_drop` 2.32× on arm64, 2026-02-25).
...
2) **W5 - Performance parity (hot loops + alloc/GC)**
   - Baselines: `loop_sum` 3.37× C, `dot_product` 4.23× C; `alloc_churn` 6.16× C, `alloc_drop` 2.32× C (arm64, 2026-02-25).
...
1) **W5 - Native integer hot-loop parity (loop_sum, dot_product)** (L)
   - Baseline (arm64 native, snapshot 2026-02-25): `loop_sum` 3.37× C, `dot_product` 4.23× C.
...
2) **W5 - Allocation/GC overhead reduction (alloc_churn, alloc_drop)** (L)
   - Baseline (arm64 native, 2026-02-25): `alloc_churn` 6.16× C, `alloc_drop` 2.32× C.
   ...
   - New: loop list reuse hoists safe, non-escaping list allocations out of loops and replaces per-iter
     ...
     - `alloc_churn` native improved to 6.16× C in the 2026-02-25 snapshot.
