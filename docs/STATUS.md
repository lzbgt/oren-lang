...
- The first GUI bridge slices now exist as binary GFX mailboxes. Bytecode can
  ...
  view exposes SDK-side frame metrics for rendered frame count, CPU encode time,
              target frame budget, budget-usage permille, over-budget status, geometry vertex count, and text-run count. Retained image resources are now available for sprite-like upload/draw/destroy/sub-rect and packed batched-atlas lifetimes, retained 2D and first retained 3D mesh resources avoid resending repeated triangle geometry, Metal geometry vertex builders reserve a small op-count-bounded initial capacity only on first append and transfer raw buffers into vertex-run ownership, Metal batched image-rect commands build one raw vertex-backed run per command instead of one run per rect, Metal image-run preparation coalesces adjacent same-texture/scissor/opacity runs into one raw vertex span, retained Metal material override draws resolve material RGBA and convert constant material/mesh RGBA bytes once before triangle loops, and Oren-side image upload budgets plus SDK retained image count/pixel budgets bound sprite memory; CoreGraphics and Metal retained image, text, mesh, material, and model lookups use scalar IDs instead of boxed dictionary keys; retained text upload/draw/destroy and packed retained text batching now avoid resending repeated UTF-8 labels, CoreGraphics caches text attributes by RGBA for immediate and retained text paths with a scalar one-entry MRU before scalar-key cache lookup/storage, and Metal packs rendered labels into bounded atlas textures, coalesces adjacent same-atlas/scissor/opacity runs, and shares a typed text-attribute cache with scalar-key lookup/storage to reduce repeated text-color miss overhead. UIKit/CoreGraphics
