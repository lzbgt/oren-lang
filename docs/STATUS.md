...
| Capsule model (capability gating) | Rolling | runtime + emit constraints | `docs/CAPABILITY_RUNTIME_CONTRACT.md`, `@oren.package(...)`, `policy.source_package_check`, `--enforce-package-policy`, `scripts/run_package_policy.sh`, `scripts/run_avm_package_policy.sh`, `scripts/run_native_package_policy.sh`, `OREN_NATIVE_PACKAGE_POLICY_RUN_JSON`, `make verify-capability-runtime-contract`, `make verify-capability-metadata`, `make verify-capability-manifest-policy`, `make verify-avm-package-policy-runner`, `make verify-native-package-policy-runner`, `tests/native/fixtures/capsule_*`, `tests/fixtures/meta_capabilities_src.oren` |
...
| Capability domains (CORE/FS/TIME/RNG/NET/PROC/ENV/AVM) | Rolling | `lib/avm/avm_native.inc`, `lib/compiler/metadata.oren` | `docs/CAPABILITY_RUNTIME_CONTRACT.md`, `@oren.package(...)`, `policy.source_package_check`, `--enforce-package-policy`, `scripts/run_package_policy.sh`, `scripts/run_avm_package_policy.sh`, `scripts/run_native_package_policy.sh`, `OREN_NATIVE_PACKAGE_POLICY_RUN_JSON`, `make verify-capability-runtime-contract`, `make verify-capability-metadata`, `make verify-capability-manifest-policy`, `make verify-avm-package-policy-runner`, `make verify-native-package-policy-runner`, `tests/avm/**` |
| Effect ledger contract | Rolling schema | `docs/EFFECT_LEDGER_CONTRACT.md`, `lib/avm/main.c`, `lib/runtime_native/000_prelude_sys.oren`, `lib/runtime_native/010_channels_globals_consts.oren`, `scripts/run_backend_semantic_diff.sh` | `make verify-effect-ledger-contract`, `make verify-avm-effect-ledger-json`, `make verify-backend-semantic-diff`; AVM run JSON emits `effect_ledger_summary` with gas/heap/wall/log/trace budget fields; native executables can emit `oren.native-run.v0` with native `effect_ledger_summary` wall timing and `oren.native-capsule-effect-gates.v0` domain-gate counters through `OREN_NATIVE_RUN_JSON=1`; semantic diff emits `oren.semantic-diff.v0` with native/OBC ledger summaries, normalized `budget_deltas`, and explicit C ledger-unavailable markers; native package policy can emit `oren.native-package-policy-run.v0` runner-observed wall-budget JSON; full native/AVM budget parity fixtures are future work |
