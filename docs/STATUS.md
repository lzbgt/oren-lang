...
- Module parse profiling (2026-05-05): native build phase profiles now emit serial `link.parse_module.done` rows and `scripts/profile_native_build_phases.sh` prints a `module parse by path` table. The first refreshed generator profile shows `lib/std/generator.oren` as the dominant parse module at about `1.42s / 137 stmts`, followed by `lib/std/task_group.oren` at about `520ms / 62 stmts` and `lib/std/task.oren` at about `322ms / 52 stmts`; future module-parse work should target those concrete files or cache hits, not the smaller `reflect/result/list` tail. An opt-in serial module-cache write probe was rejected: writing only the first two ASTBIN cache entries pushed the profile to the 180s timeout, with `lib/std/time.oren` alone reporting about `114.5s` including encode/write cost, so serial cache seeding needs a different design.
