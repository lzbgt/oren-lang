...
| Effect ledger contract | Rolling schema | `docs/EFFECT_LEDGER_CONTRACT.md`, `lib/avm/main.c`, `scripts/run_backend_semantic_diff.sh` | `make verify-effect-ledger-contract`, `make verify-avm-effect-ledger-json`, `make verify-backend-semantic-diff`; AVM run JSON emits `effect_ledger_summary` with gas/heap/wall/log/trace budget fields; semantic diff emits `oren.semantic-diff.v0` with OBC ledger summary, normalized `budget_deltas`, and explicit C/native ledger-unavailable markers; native package policy can emit `oren.native-package-policy-run.v0` runner-observed wall-budget JSON; full native/AVM ledger parity fixtures are future work |
