...
- Update (2026-04-23): the exact-program fill-mix attribution gap for that same single-list unroll4 family is now closed too. `make perf-probe-arm64-fast-push-exact-fill-mix` and its first summary log (`build/logs/perf-probe-arm64-fast-push-exact-fill-mix-20260423_080825_22120.log`) read the benchmark sources themselves and report whether each exact program can actually enter the shipped gate `single_list_cursor && pushes_per_iter==1 && nonnegative_linear && tick_period%4==0`. The result is unambiguous: exact `array_sum_int` is directly causal for this family (`fill_pushes_per_iter: 1`, `single_list_unroll4_applicable: yes`) and stays overwhelmingly no-wrap in isolation (`494000` no-wrap wide trips, `6000` wrap trips, `98.8000%` / `1.2000%`), while exact `dot_product_int` is structurally ineligible (`fill_pushes_per_iter: 2`, `single_list_unroll4_applicable: no`, `ineligible_reason: pushes_per_iter!=1 blocks single_list_cursor/unroll4 gate`) even though its two separate fill streams would also be mostly no-wrap in isolation (`99.2000%` for `a`, `98.0000%` for `b`). Reweight again: for this single-list fill branch family, treat exact `array_sum_int` as the causal same-tree benchmark and exact `dot_product_int` as a non-causal control/guardrail until a distinct multi-push specialization exists.
