...
- The first GUI bridge slices now exist as binary GFX mailboxes. Bytecode can
  ...
  `prisms_xy` extruded polygon solids, bounded `heightfields_xy` terrain grids,
	  bounded `surfaces_xyz` arbitrary surface grids, flat indexed `vertices_xy`, flat `triangles_xy`/`quads_xy`, flat per-triangle-color `triangles_xy_rgba`/`quads_xy_rgba`, rectangular `planes_xy`, filled `rounded_rects_xy`, flat `polygons_xy`,
