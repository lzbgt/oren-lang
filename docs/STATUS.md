...
- **Optimizer list-int active-candidate guard**: list-int lowering now distinguishes live candidates from names already proven unsafe. Use scanning, touch marking, and final rewrite loops are skipped once no active candidate remains, while later statement flow can still discover new candidates. The measured profile moves optimizer total to about `712ms`, `list_int_ms` to about `153ms`, `list_int_touch_ms` and rewrite buckets to `0ms`, with `list_int_use_scan_ms` rising to about `56ms` but net optimizer time still improving (2026-05-05).
