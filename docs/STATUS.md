		     ...
		     - supported policy keys are:
			   ...
			   - New (2026-04-23): `std:task` now ships as the safe generic `spawn` facade:
			     ...
			     - that task stop surface now accepts `mode="request_cancel"` and `mode="stop"`
				   - `mode="request_cancel"` records cooperative sticky state only; generic tasks still have
				     no hard cancellation primitive
				   ...
				   - `stop_policy(...)` always returns a watcher handle; `stop_policy_wait(...)` returns `nil`
				     ...
				     map for `mode="stop"`, with `join_timeout_ms` as an explicit synchronous wait-budget
				     override
