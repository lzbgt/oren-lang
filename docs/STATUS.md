...
| Capsule model (capability gating) | Rolling | runtime + emit constraints | `docs/CAPABILITY_RUNTIME_CONTRACT.md`, `@oren.package(...)`, `policy.source_package_check`, `--enforce-package-policy`, `scripts/run_package_policy.sh`, `scripts/run_avm_package_policy.sh`, `scripts/run_native_package_policy.sh`, `make verify-capability-runtime-contract`, `make verify-capability-metadata`, `make verify-capability-manifest-policy`, `make verify-avm-package-policy-runner`, `make verify-native-package-policy-runner`, `tests/native/fixtures/capsule_*`, `tests/fixtures/meta_capabilities_src.oren` |
