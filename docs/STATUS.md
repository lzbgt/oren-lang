...
- `tests/avm/test_std_math_core.oren`, `tests/avm/test_std_math_pow.oren`,
  ...
  pure `std:math/vec2` helpers including squared-length aliases, zero-normalize, direction/move-toward navigation, and vector angle conversion/wrapping/interpolation,
  pure `std:math/vec3` helpers including squared-length aliases, zero-normalize, direction/move-toward navigation, and vector angle conversion/wrapping/interpolation,
  pure `std:math/vec4` helpers including squared-length aliases, zero-normalize, direction/move-toward navigation, and vector angle conversion/wrapping/interpolation,
