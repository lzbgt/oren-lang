...
- **list<int> hot-loop parity**: the canonical arm64 whole-operation path is materially healthier than it was earlier in the day, but still not at production parity. The exact promoted-tree whole-operation ceiling rerun (`build/logs/perf-probe-list-int-c-ceiling-20260409_163202_21950.log`) put `oren_array_sum_int / array_slot64_vector` at `~2.3939×` and `oren_dot_product_int / dot_slot64_vector` at `~1.8678×`. The important shipped change behind that improvement is the explicit get-sum unroll2 promotion in `lib/compiler/arm64_native_stmt_loops_list_emit.oren`: the earlier crashy candidate was root-caused to reserved heap-register clobber in the experimental unrolled bodies (`X27` / `X28` reused as loop temporaries), now fixed by using caller-saved `X12` / `X13` instead. `OREN_ARM64_FAST_LIST_INT_GET_SUM_UNROLL2` therefore now ships on by default for single-read-list shapes, and the broad integrated gates pass on that promoted tree (`build/logs/make_test_get_sum_unroll2_promote_20260409.log`, `build/logs/runtime_robustness_w5_20260409_163313.log`). The new combined decision probe (`build/logs/perf-probe-arm64-fast-get-sum-unroll2-decision-20260409_170812_66742.log`) now makes the local-acceptance contradiction explicit instead of leaving it as chat-only reasoning: acceptance steady still preferred disabled (`-4.79%`), acceptance gate slightly preferred default (`+0.78%`), but the same-tree exact whole-operation `array_sum_int` sweeps strongly preferred the shipped default in all three runs (`default median ~2.3793×` vs disabled median `~5.3859×`, `array_default_wins: 3/3`). Exact `dot_product_int` stayed mixed (`default ~1.8343×`, disabled `~1.8138×`, disabled wins `2/3`), which is consistent with this being an `array_sum_int` get-sum decision rather than a general dot-path optimization. The next exact-path follow-up, `build/logs/perf-probe-arm64-fast-get-sum-dual-accum-decision-20260409_174904_22327.log`, closed another live branch: the opt-in `OREN_ARM64_FAST_LIST_INT_GET_SUM_DUAL_ACCUM=1` experiment looked good on the local acceptance wrapper (`steady -19.53%`, `gate -52.65%`) but still lost on the actual shipped decision surface, where the exact whole-operation reruns preferred the default in `4/5` sweeps (`default median ~2.2506×` vs enabled `~2.2797×`). The newest exact-path follow-up, `build/logs/perf-probe-arm64-fast-get-sum-pair-post-decision-20260409_180234_41790.log`, resolved the remaining get-sum-only pair-post branch the same way: the local acceptance wrapper strongly preferred `OREN_ARM64_FAST_LIST_INT_GET_SUM_PAIR_POST=1` (`steady -26.20%`, `gate -53.71%`), but the same-tree exact whole-operation reruns still preferred the shipped default in `3/5` sweeps (`default median ~2.3604×` vs enabled `~2.4015×`). Reweight accordingly: the shipped decision surface for this path remains the combined exact C-ceiling probe plus integrated green lanes, and both the dual-accum and get-sum pair-post branches stay opt-in only. The remaining blocker is still the repeated `array_sum_int` get-sum path, but it is no longer the earlier `~5.8×` emergency; `dot_product_int` remains materially above the slot64 host-C ceiling within the current ABI.
- **list<int> hot-loop parity**: the canonical arm64 whole-operation path is materially healthier than it was earlier in the day, but still not at production parity. The exact promoted-tree whole-operation ceiling rerun (`build/logs/perf-probe-list-int-c-ceiling-20260409_163202_21950.log`) put `oren_array_sum_int / array_slot64_vector` at `~2.3939×` and `oren_dot_product_int / dot_slot64_vector` at `~1.8678×`. The important shipped change behind that improvement is the explicit get-sum unroll2 promotion in `lib/compiler/arm64_native_stmt_loops_list_emit.oren`: the earlier crashy candidate was root-caused to reserved heap-register clobber in the experimental unrolled bodies (`X27` / `X28` reused as loop temporaries), now fixed by using caller-saved `X12` / `X13` instead. `OREN_ARM64_FAST_LIST_INT_GET_SUM_UNROLL2` therefore now ships on by default for single-read-list shapes, and the broad integrated gates pass on that promoted tree (`build/logs/make_test_get_sum_unroll2_promote_20260409.log`, `build/logs/runtime_robustness_w5_20260409_163313.log`). The new combined decision probe (`build/logs/perf-probe-arm64-fast-get-sum-unroll2-decision-20260409_170812_66742.log`) now makes the local-acceptance contradiction explicit instead of leaving it as chat-only reasoning: acceptance steady still preferred disabled (`-4.79%`), acceptance gate slightly preferred default (`+0.78%`), but the same-tree exact whole-operation `array_sum_int` sweeps strongly preferred the shipped default in all three runs (`default median ~2.3793×` vs disabled median `~5.3859×`, `array_default_wins: 3/3`). Exact `dot_product_int` stayed mixed (`default ~1.8343×`, disabled `~1.8138×`, disabled wins `2/3`), which is consistent with this being an `array_sum_int` get-sum decision rather than a general dot-path optimization. The next exact-path follow-up, `build/logs/perf-probe-arm64-fast-get-sum-dual-accum-decision-20260409_174904_22327.log`, closed another live branch: the opt-in `OREN_ARM64_FAST_LIST_INT_GET_SUM_DUAL_ACCUM=1` experiment looked good on the local acceptance wrapper (`steady -19.53%`, `gate -52.65%`) but still lost on the actual shipped decision surface, where the exact whole-operation reruns preferred the default in `4/5` sweeps (`default median ~2.2506×` vs enabled `~2.2797×`). The newest exact-path follow-up, `build/logs/perf-probe-arm64-fast-get-sum-pair-post-decision-20260409_180234_41790.log`, resolved the remaining get-sum-only pair-post branch the same way: the local acceptance wrapper strongly preferred `OREN_ARM64_FAST_LIST_INT_GET_SUM_PAIR_POST=1` (`steady -26.20%`, `gate -53.71%`), but the same-tree exact whole-operation reruns still preferred the shipped default in `3/5` sweeps (`default median ~2.3604×` vs enabled `~2.4015×`). The new fill-share decision probe (`build/logs/perf-probe-list-int-fill-share-decision-20260409_181428_58993.log`) reweights the adjacent setup side too: a single-list fill-only `list<int>` whole-operation pass is `~4.8260×` slot64 C vector and about `~10.2796×` the current shipped `array_sum_int` steady per-rep read cost, while still explaining only `~59.12%` of the current `array_sum_int` setup estimate. Reweight accordingly: the shipped decision surface for this path remains the combined exact C-ceiling probe plus integrated green lanes; the dual-accum and get-sum pair-post branches stay opt-in only; and the next high-leverage work should move back toward list build/fill lifetime/setup costs instead of another get-sum-local micro-branch. `dot_product_int` still remains materially above the slot64 host-C ceiling within the current ABI.
