...
- Update (2026-04-21): arm64 nonnegative-linear fill lowering now saturates `_arm64_nonneg_linear_safe_n_limit(...)` instead of overflowing the identity shape to `0x8000...` in the preheader compare, but this branch is no longer shipped by default. The same-day self-host check showed that a stage2 compiler built with `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR=0` compiles and runs native quick cleanly while the default-on self-hosted compiler stalls, so `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR` is back to opt-in until it is self-host-safe. The current guardrail is a reduced aggressive-GC `list<int>` churn smoke (`tests/native/test_gc_reuse_alloc_churn_min.oren`) in native quick plus the tighter W5 tracking smoke set.
- Update (2026-04-23): the broad nonnegative-linear family is still performance-positive on the current tree even though it remains opt-in. The refreshed decision surface (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-decision-20260423_002542_70401.log`) again preferred `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR=1` across both fill/share and exact same-tree reruns (`default_fill_vs_c_vector ~5.7432×` vs enabled `~2.3262×`, `default_array_ratio_median ~2.1543×` vs enabled `~2.0482×`, `default_dot_ratio_median ~1.8061×` vs enabled `~1.7963×`). The reduced W5 guardrail also passes under the opt-in branch now (`build/logs/verify_alloc_churn_tracking_smoke_nonneg_linear_20260423_002715.log`), so the remaining blocker is no longer the reduced alloc-churn/list-header smoke itself; it is still the broader self-host/native-quick safety surface. A narrower attempted default-on isolation for the exact fresh single-list shape did not close that gap cleanly and was reverted instead of shipped: the current rerun (`build/logs/perf-probe-arm64-fast-push-nonneg-linear-decision-20260423_003345_74555.log`) preferred default on exact `array_sum_int` only, while fill/share and exact `dot_product_int` still preferred the disabled branch (`default_fill_vs_c_vector ~5.8346×` vs disabled `~5.6761×`, `default_dot_ratio_median ~1.8944×` vs disabled `~1.8570×`). Reweight accordingly: keep the broad nonnegative-linear branch opt-in, do not promote fresh-single-list isolation as a new shipped default, and focus next on root-causing or fencing the remaining self-host/native-quick blocker on the performance-positive broad path.
...
3) **Runtime robustness W5: GC reuse + list header integrity** (L, W5)
		  ...
		  - Fix + verify (2026-04-21): reduced aggressive-GC `list<int>` churn now has a stable current
		    ...
		    heavier benchmark build. The same-day self-host decision surface also forced
		    `OREN_ARM64_FAST_LIST_INT_PUSH_NONNEG_LINEAR` back to opt-in only:
		    `./oren_stage2_nonneg0` (built with the flag off) runs native quick cleanly while the
		    default-on self-hosted build stalls. A same-day direct `oren_gc_collect()` arm64 call shortcut was
		    also removed instead of shipped: it regressed native quick by hanging
		    `test_gc_collect_does_not_deadlock_with_green_join_waiter`, so the current tree keeps the
		    normal direct-call lowering for `oren_gc_collect()`.
