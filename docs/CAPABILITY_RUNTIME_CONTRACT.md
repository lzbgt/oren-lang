...
process resource usage when the host exposes that accounting surface. It still fails closed for
declared `budget_gas` until native-equivalent gas accounting exists. When
`OREN_NATIVE_PACKAGE_POLICY_RUN_JSON=<path>` is set, it writes
`oren.native-package-policy-run.v0` JSON with runner-observed wall/heap/CPU-budget evidence, the
...
Gas remains reported as unavailable/null until backend-inserted instruction-equivalent gas exists;
package-policy JSON remains runner-observed wall/heap/CPU evidence with captured runtime summaries,
...
- Capability budgets are now representable in package metadata. The AVM package-policy
  ...
  live-heap scans, and CPU-budget checks from child process resource usage where available, while
  still failing closed for declared gas budgets. They are not yet a complete enforcement contract
  across native and AVM because native gas accounting is still absent. Native capsule runtime
  knobs are domain/resource allowlists first.
