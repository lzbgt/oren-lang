...
2) **Determinism is a first‑class constraint.**
  - For AVM/multiverse, we need deterministic execution under budgets and virtualized backends (see `docs/AVM_ROADMAP.md`).
...
4) **Tier‑1 means “real validation,” not “best effort.”**
   ...
   - See `docs/PLATFORMS.md` for x86_64 validation, and existing arm64 fixtures.

## The Unification Strategy: One Frontend, One Canonical IR, Thin Backends

Today the repo already has:

- A production‑oriented **C backend** (architecture neutral via the host C toolchain): `docs/BACKENDS.md#c-backend-design-and-abi`
- A high‑performance **native backend** (arm64 rich; x86_64 bring‑up): `docs/BACKENDS.md#native-backend-overview`
...
`NativeIR` here matches the direction in `docs/BACKENDS.md#native-backend-code-reuse-plan`.
...
- C backend: uniform callable ABI via `oren_call_obj(...)` / `oren_call_obj_list(...)` (`docs/BACKENDS.md#c-backend-design-and-abi`)
...
See `docs/TOOLCHAIN.md` for more details on the lowering rules.
...
For “do not regress” invariants (and the regression gates that enforce them), see `docs/BACKENDS.md#native-backend-guardrails`.
...
- **Language Features**:
  ...
  - **Lists / Maps (Tier‑1; runtime-defined semantics)**:
    ...
    - Indexing is polymorphic: `xs[i]` / `m[k]` dispatches based on **tracked allocation metadata** (see `docs/BACKENDS.md#native-runtime-layout`).
...
- **Shared Libraries**: Build `.dylib` (macOS) using `--lib`. Exports defined functions.
  ...
  - Linux notes:
    ...
    - **Linux executables** (arm64-linux + x64-linux) can export selected symbols for callback interop via `@ffi.export` (see `docs/LANGUAGE_APPENDICES.md`).
...
The bounded tracing knobs in `docs/TOOLCHAIN.md` are designed to tell you *which bucket* you’re in without spamming.
