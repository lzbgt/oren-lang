...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     output, Scene3D binary package magic checks, and PPM/software-raster RGBA sampling plus clear/pixel writes use raw exact-size buffer stores, and native `std:net/http`
