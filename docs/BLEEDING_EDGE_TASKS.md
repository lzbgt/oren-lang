														        ...
														        ~3.31s, `list_int` down from ~1.81s to ~1.46s, and reserve down from ~0.64s
															      ...
															      - Optimizer fold follow-up (2026-05-05): `optimizer.summary` now carries
															        ...
															        fold cleanup hoists stable list lengths in fold-local loops, fast-returns
																	        ...
																	        clone-of-clone environments in isolated `if`/`for` folding, and avoids a
																	        redundant empty environment push for function-expression bodies. The measured
																	        profile moves optimizer total to ~1.55s, `fold_ms` to ~449ms,
															        ...
															        `if` nodes, and only `22` other nodes.
