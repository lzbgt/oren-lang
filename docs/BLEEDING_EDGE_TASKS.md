...
- Reweight (2026-05-05): list-int lowering now tracks whether the candidate map still has any active safe candidate instead of treating unsafe candidate entries as work. The retained guard skips use scanning, touch marking, and rewrite loops after all current candidates have been invalidated, while preserving later candidate discovery in the same block. The measured generator profile moves optimizer total to about `712ms`, `list_int_ms` to about `153ms`, and `list_int_touch_ms` / rewrite buckets to `0ms`; `list_int_use_scan_ms` rises to about `56ms`, so the next list-int target should reduce active-candidate checks or remaining lower/use-scan structure only with proof that net wall time improves.
