...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     and trim helpers plus JSON/CBOR/regex tag equality now use direct string
     ...
     constants, and native Mach-O/ELF object string payloads now use direct
     source-string byte reads or builder copies too.
