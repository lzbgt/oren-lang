...
2. **Native performance parity**
   ...
   - Current native ops plus ARM64 Mach-O/ELF, ARM64 expression/program/global/root
     ...
     program-entry orchestration scans cache immutable argument, expression, module,
   ...
   - Current x64 index/list lowering separates map-vs-generic runtime-fast
     ...
     records. Index-get operand prep, call-object ABI/direct-call, spread,
     indirect, spawn, and varargs helper records, LCG sum while, lambda literal
     env, and native allocation kind helpers use positional records too.
   ...
   - Current compiler loop-list analysis, arena, reset, and list-int lowering
     optimizer traversals cache immutable AST list counts before continue/exit,
     ...
     rewrite scans, with source guards rejecting direct `oren_list_len` loop
     bounds in those passes.
   - Current shared optimizer helper list-touch, assignment, for-detection, and
     if-detection traversals cache immutable AST list counts before recursive
     scans, with source guards rejecting direct `oren_list_len` loop bounds in
     that helper pass.
   - Current optimizer TCO self-call discovery, call-effect scans, parameter
     validation, and tail-recursive rewrite traversals cache immutable AST list
     counts before iteration, with source guards rejecting direct `oren_list_len`
     loop bounds in that optimizer pass.
   - Current compiler function/yield analysis caches immutable program, block,
     parameter, expression, yield-point, generator-finalize, and function-list
     traversal counts before iteration, with source guards rejecting direct
   ...
   - Current serde JSON/YAML/CBOR lowering and module-visibility validation
     cache immutable attr/field/format/statement/module/expression traversal
     counts before iteration, with source guards rejecting direct
   ...
   - Current C transpiler constructor, lambda/capture analysis, list-get
     analysis, fast-while matching/emission, block, branch, call, array,
     ...
     bounds in those backend helper paths.
   ...
   - Current module-linking prelude parse/import scans, parse-stage
     dependency/worklist/include aggregation, pipeline worker/merge/import/table
     traversals, and tail diagnostic aggregation cache immutable list counts
     before traversal, with source guards rejecting direct `oren_list_len` loop
     bounds in those link stages.
   - Current compiler build-cache scan-cache load/save, include-closure hashing,
     runtime C include walks, build-key dependency expansion, and stable key-field
     assembly cache immutable list counts before traversal, with source guards
     rejecting direct `oren_list_len` loop bounds in that cache-key path.
   - Current compiler build-pipeline FFI/link/export scans, Python/MSVC flag
     probing, dump-command parse diagnostics, bytecode link/error/constant
     assembly, and native link command assembly cache immutable list counts before
     traversal, with source guards rejecting direct `oren_list_len` loop bounds in
     those build stages.
   - Current compiler frontend/support help rendering, JSON key-field formatting,
     yield-lowering diagnostics/tracing, syscall policy scans, include-stack
     copies, path/completion emission, and dump metadata merge scans cache
     immutable list counts before traversal, with source guards rejecting direct
     `oren_list_len` loop bounds in those support stages.
   - Current compiler ASTBIN module key scans/emission and native
     runtime-object sidecar C-string/debug-function encoding cache immutable
     list counts before traversal, with source guards rejecting direct
     `oren_list_len` loop bounds in those artifact support paths.
   - Current compiler debug-sugar lowering and capsule policy/domain scans cache
     immutable list counts before traversal, with source guards rejecting direct
     `oren_list_len` loop bounds in those support passes.
   - Current `std:sys` capability-domain id/name mask helpers cache immutable
     list counts before traversal, with source guards rejecting direct
     `oren_list_len` loop bounds in the pure AVM-safe sys facade.
   - Current optimizer DCE env copies, use/read collection, trivial-var probes,
     and rewrite walks cache immutable list counts before traversal, with source
     guards rejecting direct `oren_list_len` loop bounds in that pass.
   - Current type-name resolution expression/statement walks cache immutable AST
     list counts before arg/element/pair/field/case/statement traversal, with
     source guards rejecting direct `oren_list_len` loop bounds in that semantic
     pass.
   - Current native runtime-object cache file metadata, runtime fragment hashing,
   ...
   - Current global DCE attr, expression, statement, root, and filter scans cache
     ...
     direct `oren_list_len` loop bounds in that pass.
   - Current impl lowering trait/statement, receiver-arg insertion, function
     parameter, array/hash/call, block, and top-level traversals cache immutable
     list counts before iteration, with source guards rejecting direct
     `oren_list_len` loop bounds in that semantic lowering pass.
   - Current compiler metadata JSON attribute/list/yield/package/capability/serde,
     function/trait/global/struct manifest traversals cache immutable list counts
     before iteration, with source guards rejecting direct `oren_list_len` loop
     bounds in that metadata path.
   - Current typecheck and nil-compare guard expression, statement, switch-case,
     function signature, parameter, and top-level statement traversals cache
     immutable list counts before iteration, with source guards rejecting direct
     `oren_list_len` loop bounds in that semantic checker.
   - Current shared native callable lowering caches immutable statement,
     named-function value-use, local-function rewrite, lambda declared/used/capture,
     lambda collection, parameter, and wrapper-body list counts before traversal,
     with source guards rejecting direct `oren_list_len` loop bounds in that
     x64/ARM64 callable helper.
   - Current optimizer list-reserve candidate collection, branch/body push scans,
     unchecked-push rewrites, array/hash/switch traversal, and block statement walks
     cache immutable AST list counts before iteration, with source guards rejecting
     direct `oren_list_len` loop bounds in that reserve pass.
   - Current generic monomorphization clone, template discovery, instantiation
     collection, constraint validation, specialization emission, and linked-statement
     rewrite traversals cache immutable list counts before iteration, with source
     guards rejecting direct `oren_list_len` loop bounds in that pass.
   - Current optimizer fold block scans, C transpiler signature assembly, and
     top-level renamer scans cache immutable list counts before traversal, with
     source guards rejecting direct `oren_list_len` loop bounds in those compact
     compiler support paths.
   - Current CoreIR function declaration, numeric argument/list/map/statement,
     and iterative parameter-trait inference scans cache immutable list counts
     before traversal, with source guards rejecting direct `oren_list_len` loop
     bounds in that IR support path.
   - Current type-annotation lowering module-graph, function parameter
     normalization, expression/statement rewrite, switch, field-kind collection,
   ...
   - Current native top-level partitioning, ARM64 wrapper-body emission, and
     ARM64 GC safepoint spill-pair emission cache immutable list counts before
     traversal, with source guards rejecting direct `oren_list_len` loop bounds
     in those native support paths.
   ...
   - Current ELF artifact abstraction shares identical ELF64 file-header,
     ...
     string-table, DT_NEEDED unique-list helpers, and minimal
     `.text`/`.data`/`.shstrtab` tooling section-table byte emission through
     `elf_artifact.oren` while keeping machine IDs, section-table policy,
     relocation policy, codegen, and ABI rules in the arch emitters.
   ...
   - Current PE header cleanup routes signature and linker-version zero fields
     through exact zero-extension spans instead of separate zero-byte pushes.
   ...
   - Current ELF dynamic-symbol/header cleanup routes `st_other` and OSABI zero
     fields through exact zero-extension spans.
   ...
   - Current ARM64 Mach-O symbol-table cleanup routes the initial string-table
     terminator and undefined-symbol `n_sect` fields through zero-extension spans.
   ...
   - Current Windows x64 `sys_stat` regular-file success cleanup separates
     stat-mode fill, best-effort size query, handle close, and zero rc emission.
   ...
   - Current x64 string-batch input cleanup separates input-list extraction from
     derived count/state calculation.
   - Current Windows x64 entry envblock cleanup separates the
...
1. **Native IR / LLVM enablement**
   - Current verdict: do not replace the hand x64/ARM64 native emitters with
     ...
     generated `oren_llvm_helper_print` body with descriptor-backed string
     globals plus libc `puts`; `make verify-native-ir-llvm-helper-runtime` proves printed-output
     ...
     list runtime gate proves a parent-list -> child-list -> dynamic-string
	     ...
	     `oren_u8_buf_from_bytes_slice`, `oren_string_from_bytes_slice`,
		     ...
		     headroom.
		     LLVM-native now also lowers empty/non-empty hash literals and
		     map-shaped struct constructor records into `%oren_llvm_map { len,
		     keys, values, key_kinds, owner_kind, capacity }` descriptors;
		     non-empty literal pairs and struct fields become explicit native-IR
		     `index_set` map writes, and member get/set lowers to string-key map
		     operations. LLVM-native maps descriptor-backed exact keys and semantic
		     string descriptor keys to generated get/set helpers, lowers proven
...
2. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     exact-size writes; Base64/Base64URL encode hoists shared byte-view backing pointers once per call and maps sextets arithmetically, argparse ASCII lowercasing writes exact-size byte buffers instead of scanning alphabet strings, and decode also rejects malformed padding and
								     ...
								     CoreGraphics draw-only opcodes skip vertex/texture/text-cache/lookup/depth-order work,
									     active empty-scissor/empty-clip spans skip Metal and CoreGraphics draw-only
												     ...
												     plus unused pipeline binding when no prepared runs exist or no valid prepared run
													     reaches vertex binding, current render-pipeline state is cached across geometry/image/text prepared runs, repeated same-scissor/texture/opacity Metal prepared runs skip redundant encoder state writes, failed Metal run/container allocation frees taken vertex bytes during flush, failed growable vertex builders reset at continuing state-boundary flushes, and Metal frame-state overflow or mismatched-pop no-ops avoid splitting prepared geometry runs,
												     malformed zero-size/count-mismatched retained image/text draws reject before retained
									     resource lookup when the command is otherwise a no-op, Metal/CoreGraphics retained image uploads validate width/height byte counts with overflow-safe dimension helpers before retained map, image, or texture allocation work, Metal/CoreGraphics retained image/text/model uploads guard resource allocation before map insertion, Metal text atlas creation and retained-image uploads guard wrapper/descriptor allocation before GPU texture work, Metal pipeline startup shares guarded shader-function and descriptor preflight before geometry/text pipeline creation, CoreGraphics retained image creation releases provider-owned bytes if RGB color-space allocation fails before `CGImageCreate`, Metal standalone text texture fallback releases glyph pixels if descriptor allocation fails before upload, Metal image/text appends report lazy run-array allocation failure, text-cache misses guard cache-key/cache-order/cache-entry/run allocation before dictionary/UIKit/glyph/vertex work, image-run constructors guard run allocation before vertex writes or exact heap allocation, renderer startup leaves retained-resource, touch, and Metal text cache storage nil until shared helpers allocate them on first use, runtime multicast graphics-frame observer storage is lazy until first registration, live-network session/maps are lazy until first live fetch or network-session registration, prepared run arrays cap their initial frame-derived reservation, and Metal prepared-run coalescing preserves original run lists if the optional coalesced output array cannot be allocated while capping optional coalesced output-array initial reservation, CoreGraphics truncated OGF0 payloads break traversal after restoring saved CGContext state, iOS GFX input event helpers reject oversized segmented/UTF-8 payloads before 16-bit length casts, immediate text opcodes reject empty/trailing-byte/invalid-UTF-8 payloads before attribute/texture preparation, retained text uploads reject empty payloads before map/string/resource work, and Metal/CoreGraphics zero-area
					     rectangles/circles/ellipses plus exact degenerate triangles skip before vertex/path work,
											     parser diagnostic and ARM64 source-location scans cache source string length before byte-offset line/column walks, generic-call declaration scans cache statement counts plus specialization-name lengths, regex byte-class matching caches compiled range counts before per-byte probes, and XML DOM traversal/text extraction plus JSON/CBOR/YAML codec, argparse, HPACK, linalg, regex, URL allow-list, HTTP/2 decoded-header, `std:sys`, parser-generator core/yield traversal, module-linking prelude/parse/pipeline/tail, build-cache scan/key, build-pipeline, compiler support, compiler artifact-support, debug-sugar, capsule-policy, compiler analysis, bytecode-tail emission, ARM64 runtime-object replay/build, packed bytes view lowering, serde/module-visibility lowering, optimizer-loop-list, optimizer-loop-arena, optimizer-helper, optimizer-TCO, optimizer-constprop, optimizer-list-int, optimizer-list-reserve, generic-monomorphization, optimizer-DCE, optimizer-fold, global-DCE, impl-lowering, metadata, typecheck, native-callable, C-transpiler support, C-transpiler fast-while, C-transpiler entry, renamer, CoreIR, type-annotation lowering, type-name resolution, and native runtime-object cache loops cache
				     immutable list lengths before iteration, PPM header/body output, PPM RGBA encoding hoists validated byte-view
     ...
     `std:bytes` u8/endian writes with unrolled u64 `u8_buf` stores, unrolled `std:buffer` raw i64 stores for typed-buffer builders, straight-line CBOR/WebSocket u64 protocol byte operations, compiler bytecode/ASTBIN u64 serializer byte operations, x64 PE section-name literal byte plus data-directory/import-thunk/export-name-pointer/header-field zero emission, ARM64/x64 ELF alignment/header/symbol zero extension, compiler artifact whole-string byte-builder extension, bytecode and OBC NIL zero tags, ARM64 Mach-O C-string and symbol-table zero helper emission, ARM64 ELF/Mach-O runtime debug zero helpers, ARM64 compiler fixed pointer-slot zero extension, ARM64 Mach-O fixed UUID zero-word and 16-byte name padding emission, concat/copy u8-buffer spans, overlap-safe
...
4. **Scientific stdlib math breadth**
   ...
   - Current retained surface includes `fmod`, IEEE-style nearest-even
     ...
     angle wrapping, shortest-angle delta, and shortest-path angle interpolation,
	     pure finite `std:math/vec2` helpers including perpendicular vectors, unsigned/signed angle-between helpers, length clamping, and projection/rejection/reflection,
	     pure finite `std:math/vec3` helpers including length clamping and projection/rejection/reflection,
	     pure finite `std:math/vec4` helpers including length clamping and projection/rejection/reflection,
     ...
     real-valued `erf`/`erfc`, scalar `lgamma`/`tgamma` Lanczos/reflection, and
     component-wise vector gamma bytecode/C/native coverage.
