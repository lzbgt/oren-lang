...
- Reweight (2026-05-05): the fold pass now has gated shape counters in `optimizer.summary`, and the retained fold cleanup hoists stable lengths in fold-local loops, fast-returns leaf expressions, skips recursive folding for leaf children, removes clone-of-clone environments from isolated `if`/`for` folding, and avoids a redundant empty environment push for function-expression bodies. The profile moved optimizer total to about `1.57s`, `fold_ms` to about `456ms`, fold environment clones from `2592` to `1317`, and fold environment pushes from `3754` to `3192`. Remaining fold evidence is concrete: `8398` non-leaf folded expressions, including `2644` calls, `2489` infix nodes, `1271` expression-level `if` nodes, and only `22` remaining other expressions.
														      ...
														      - Follow-up (2026-05-04): the local Mach-O resolver now stores unique-target
														        ...
														        ~3.31s, `list_int` down from ~1.81s to ~1.46s, and reserve down from ~0.64s
															      ...
															      - Optimizer fold follow-up (2026-05-05): `optimizer.summary` now carries
															        ...
															        leaf expressions, skips recursive folding for leaf children, avoids
															        ...
															        profile moves optimizer total to ~1.57s, `fold_ms` to ~456ms,
