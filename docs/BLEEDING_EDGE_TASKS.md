...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     `std:bytes` u8/endian writes with unrolled u64 `u8_buf` stores, unrolled `std:buffer` raw i64 stores for typed-buffer builders, concat/copy u8-buffer spans, overlap-safe
