...
2. **Native performance parity**
   ...
   - Current native ops plus ARM64 Mach-O/ELF, ARM64 statement/list-loop
     emitter, and x64 ELF/PE traversal scans cache immutable argument,
     expression, op, key, library, debug-function, import/export, fixup,
     thunk-position, attribute, break/continue patch-list, slow-jump, list-use,
     load, mul, push, and fresh-list counts before iteration, with source guards
     rejecting direct `oren_list_len` loop bounds in those native shards.
   ...
   - Current x64 system/data helper cleanup separates GC-root sorted insertion,
     ...
     `fcntl_getfl` flag translation, Windows read-handle selection, and Windows
     open state setup.
