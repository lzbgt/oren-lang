...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - Follow-up distribution design: after the GUI bridge gate, publish a curated
     ...
     Oren source in-browser with syntax highlighting and an AST outline.
	     ...
	     `cylinders_z`/`cones_z`/`spheres_xyz`/`ellipsoids_xyz` authoring that lower to existing
...
4. **Scientific stdlib math breadth**
   ...
   - Current retained surface includes `pow` / `power` for integer, negative,
    ...
    `sinh`, `cosh`, and `tanh`, plus release-gated `sin`, `cos`, `tan`, `atan`,
    `atan2`, `asin`, and `acos` bytecode coverage.
