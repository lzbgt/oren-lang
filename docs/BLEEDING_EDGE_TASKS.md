...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     path on exact-size `u8_buf` output. HPACK Huffman string encode/decode
     now also writes exact-size `u8_buf` payloads instead of materializing
     Oren byte lists; the full HPACK header-block encoder remains the next
     protocol byte-packing cleanup.
