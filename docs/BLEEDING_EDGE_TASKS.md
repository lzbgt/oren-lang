   ...
   - Follow-up distribution design: after the GUI bridge gate, publish a curated
     ...
     Oren source in-browser with syntax highlighting and an AST outline.
	     ...
	     `prisms_xy` extruded polygon authoring, bounded `heightfields_xy` terrain grids,
		     ...
		     elliptical `ellipses_xy`/`ellipse_rings_xy`, thick `segments_xy`/`paths_xy`, sampled `beziers_xy`, partial `sectors_xy`/`arc_bands_xy`, and
	     ...
	     Procedural mesh packers are split into focused `std:ui/scene3d_shapes`
	     and `std:ui/scene3d_shapes_grid` helpers so broader package formats can
