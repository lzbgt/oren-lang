...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - High-priority cleanup: remove legacy stdlib byte/string conversion paths from
     ...
     nonzero trailing pad bits before returning exact-size `u8_buf` decoded bytes. `std:strings` prefix/suffix/search/equality
	     ...
	     encoding through growable `u8_buf` output plus cached u8 carrier pointers
	     for byte-string encode and recursive/sequence decode, and full regex
     ...
     through `oren_memcpy` after validation;
	     ...
	     package magic/header/table reads, and PPM/software-raster RGBA sampling
     ...
     Compiler source-policy
	     scans, scan-cache line/number parsing plus bounded load/save and delimiter writes, C-runtime include scanning,
		     ...
		     loops reuse cached byte views for u8 carriers, OBX
