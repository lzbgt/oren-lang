...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - Follow-up distribution design: after the GUI bridge gate, publish a curated
     ...
     `prisms_xy` extruded polygon authoring, and bounded `cylinders_z` authoring
     that lower to existing byte-native triangle mesh records for AVM/iOS without
     renderer ABI changes.
