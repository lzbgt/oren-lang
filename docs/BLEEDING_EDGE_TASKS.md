...
- Rejected (2026-05-06): factoring `_task_group_validate_policy_shape(...)` through a new public `task.validate_stop_policy(...)` wrapper removed the duplicated task-group validation body, but repeated the known bytecode-only mixed task-group failure at `191096`; task, task-group, C, and native checks stayed green. Source was reverted. Do not route task-group runtime preflight through `std:task` policy helpers until the bytecode/AVM mismatch behind `191096` is understood.
