...
- Reweight (2026-05-07): libavm is not production-ready for an iOS app yet. Inspection found no iOS/Xcode/Swift app integration target for libavm in this repo, `make avm` builds a host arm64 Mach-O executable instead of an embeddable static library/xcframework, and `avm.h` / allocator internals still expose rolling limits, budgets, capability state, fixed stack/frame storage, and global allocation-owner state. Curated `make avm && make test-avm` passes, but the documented wildcard `AVM_TESTS="tests/avm/*.oren"` path fails at `test_budget_io_fs` because fixture-specific env/expected-failure policy is not manifest-driven. A direct `AVM_IO_BYTES=128` run proves that fixture itself returns the expected `AVM_ERR_BUDGET`; the gap is release harness maturity. Promote iOS libavm embeddability, AVM full-suite manifest coverage, and compiler-in-AVM packaging/default gates to W4 before claiming production readiness.
...
- Reweight (2026-05-05): the fold pass now has gated shape counters in `optimizer.summary`, and the retained fold cleanup hoists stable lengths in fold-local loops, fast-returns leaf expressions, skips recursive folding for leaf children, skips child loops for calls whose callee/args/spread are all leaves, removes clone-of-clone environments from isolated `if`/`for` folding, avoids a redundant empty environment push for function-expression bodies, and gates fold trace string construction behind the already-computed `OREN_TRACE_OPTIMIZER` flag. The profile moved optimizer total to about `1.35s`, `fold_ms` to about `252ms`, fold environment clones from `2592` to `1317`, and fold environment pushes from `3754` to `3192`. A broader leaf-route skip for leaf-only prefix/index/member/infix expressions was tested next; early profiles showed `fold_ms` around `437-438ms`, but the final exact-tree profile regressed to `451ms` and optimizer total `1.58s`, so the source probe was reverted. Remaining fold evidence is concrete: `8398` non-leaf folded expressions, including `2644` calls, `2489` infix nodes, `1271` expression-level `if` nodes, and only `22` remaining other expressions.
- Reweight (2026-05-05): const-prop is no longer an opaque residual optimizer bucket. The gated `optimizer.summary` now reports const-prop expression/statement/env/rewrite counters and the build-phase report prints an `optimizer const pass counts` table. The current generator profile showed `const_mod_rewrite=0` but the old pass still walked `14482` expressions, `4126` statements, and cloned/intersected envs `3803` times; the retained `% identifier` prefilter skips the full env-mutating traversal unless the exact rewrite shape exists, moving `const_ms` from about `144ms` to about `20ms` and optimizer total to about `1.23s`. Positive rewrite behavior is covered by `tests/fixtures/optimizer_const_mod_rewrite_main.oren` in native quick integration. Next optimizer work should move past const/fold/list-int no-op traversal and target residual pass costs only when the summary shows a concrete active bucket.
- Reweight (2026-05-05): native build profiles now also print phase-specific top ARM64 function bodies for `user_decls`, `fnwrap`, and `lambda_wrap`, so wrapper/codegen hot rows no longer require manual function-log greps. The latest generator-surface fixture pass splits the remaining large timeout policy wait, nil/collect iteration, and declaration counter/collect assertion clusters while preserving dispatcher order and return-code semantics. This is intentionally scoped to the measured fixture/codegen shape; true compiler reductions should still target residual optimizer/link buckets or a non-generic-map Mach-O resolver design.
- Rejected (2026-05-05): a narrow ARM64 direct-call lowering for `oren_list_get(list, <constant>)` preserved runtime helper semantics and reduced emitted wrapper bytes (`fnwrap` about `18.6KB -> 15.8KB`, `lambda_wrap` about `50.7KB -> 47.1KB`), but it did not improve the measured wall-time target. `fnwrap` moved from about `1.10s` to about `1.15s`, `lambda_wrap` stayed about `2.03s`, `user_decls` moved up, and the probe `make stage2` ran materially longer than the recent baseline. The follow-up statement profile still shows the true wrapper shape: `lambda_wrap` spends about `890ms / 114 calls` in generic `oren_list_get`, with about `770ms` in `arg_eval`; `fnwrap` spends about `691ms / 89 calls`, with about `590ms` in `arg_eval`. A later direct lambda-wrapper local-binding probe removed the visible `lambda_wrap` `oren_list_get` statement bucket, but default wrapper wall time regressed (`lambda_wrap` about `2.03s -> 2.28s`, `fnwrap` about `1.10s -> 1.31s`) because the cost moved into wrapper body work instead of disappearing. Keep both wrapper shortcut routes closed unless a later design reduces compiler-side wrapper binding/lowering time in the default profile.
- Rejected (2026-05-05): caching repeated `oren_type_tag(...)` results inside `_task_group_validate_policy_shape(...)` passed `make stage2` and focused task-group/task surfaces, but it expanded the parse/codegen shape instead of reducing it. The default profile moved `lib/std/task_group.oren` from about `471ms / 64 stmts` to about `616ms / 64 stmts`, and `user_decls` moved from about `3.38s` to about `4.22s`. Do not reopen local tag-temporary rewrites in the residual task/task-group parser-body rows unless the proposal also removes larger structure.
- Rejected (2026-05-05): removing the redundant private policy-map normalization inside `_task_group_validate_policy_shape(...)` passed `make stage2` and focused task/task-group surfaces, but did not move the measured hot body; the exact profile put `lib/std/task_group.oren` at about `477ms / 65 stmts` with `_task_group_validate_policy_shape` still the top parse row. Source was reverted. Residual task-group work needs larger control/data-shape deletion, not private precondition tightening.
- Reweight (2026-05-05): a retained `std:task_group` structural cleanup now shares the runtime stop snapshot path between `stop_policy(...)` and `stop_policy_wait(...)` without changing the empty-preflight early return or the runtime preflight-before-take ordering. The focused task-group/task/generator/coroutine surfaces pass, and the exact native profile moves `lib/std/task_group.oren` from `486ms / 64 stmts / parse_body_ms=296` to `471ms / 65 stmts / parse_body_ms=289`; `stop_policy` and `stop_policy_wait` no longer appear in the top three task-group parse rows. Remaining task/task-group parse work should delete larger structure in `_task_group_validate_policy_shape`, `task.stop_policy_wait`, or `_task_stop_fields`, not add local temporaries or helper splits that only move cost.
- Reweight (2026-05-06): `_task_group_runtime_stop_snapshot(...)` now returns its private stop snapshot as `[members, member_kinds, policy]` instead of a temporary string-keyed map that runtime `stop_policy(...)` / `stop_policy_wait(...)` immediately re-indexed. The public `task_group.snapshot(...)` surface remains map-shaped. Focused task/task-group/generator/coroutine gates pass, and the exact profile moves `lib/std/task_group.oren` to about `433ms / 62 stmts` with `parse_body_ms=265`; the remaining task-group row is still `_task_group_validate_policy_shape`, so future stdlib work needs validation-shape deletion rather than more stop-snapshot carrier tweaks.
- Rejected (2026-05-06): extending the compact private runtime snapshot carrier beyond stop snapshots preserved behavior and passed `make stage2` plus focused task/task-group/generator/coroutine surfaces, including the mixed bytecode task-group/task gate. The exact profiles still regressed module shape: the helper-heavy form moved `lib/std/task_group.oren` to about `445ms / 63 stmts`, and the tightened public-wrapper form stayed around `444ms / 62 stmts` while `_task_group_validate_policy_shape` remained the top row. Source was reverted; do not broaden the snapshot carrier unless it also deletes validation shape.
- Rejected (2026-05-06): unrolling `_task_group_merge_policy(...)` from the private key-list loop into six direct field merges passed `make stage2` and focused task/task-group/generator/coroutine gates, but worsened the exact module profile. `lib/std/task_group.oren` moved from the retained `~433ms / 62 stmts` band to about `448ms / 62 stmts`, and `_task_group_merge_policy` became the second hot parse row at about `12ms`. Source was reverted; do not reopen loop unrolling as a parser-body fix unless it deletes larger validation/merge structure.
- Rejected (2026-05-06): wrapping runtime preflight merge plus validation in `_task_group_merge_policy_validated(...)` passed `make stage2` and focused task/task-group/generator/coroutine gates, but only added another function boundary. The exact profile moved `lib/std/task_group.oren` to about `447ms / 63 stmts`, and `_task_group_validate_policy_shape` stayed the top row at about `17ms`. Source was reverted; future work needs to remove the validation data shape itself, not compose existing merge/validate helpers differently.
- Rejected (2026-05-06): factoring `_task_group_validate_policy_shape(...)` through a `task.validate_stop_policy(...)` wrapper removed the duplicated task-group validation body, but repeated the bytecode-only mixed task-group failure at `191096`; task, task-group, C, and native checks stayed green. A narrowed bytecode probe showed this is a latency-sensitive live-runtime issue: after earlier runtime stop/cancel cases, the helper-routed invalid-policy preflight took about `278ms`, then `cancel_wait` took about `321ms`, so the observer task returned its uncancelled seed `1800` instead of `2712`. Source was reverted. Do not route task-group runtime preflight through `std:task` policy helpers unless bytecode preflight latency is removed.
- Reweight (2026-05-05): `std:task.stop_policy(...)` and `stop_policy_wait(...)` now use the normalized `delay_ms` from `_task_stop_fields(...)` for request/cancel modes instead of branching through deadline-specific wrappers that revalidate and recompute the same delay. This removes the unused returned `deadline_ns` field and keeps stop/request/cancel policy execution on one normalized-delay shape. The exact profile keeps `lib/std/task.oren` in-band but slightly lower (`~321ms -> 318ms`) and moves `stop_policy_wait` to about `24ms`. A wider task-group `extend` loop unification plus watcher-join helper probe was reverted after the exact profile worsened task-group module parse; keep task/task-group work as structural deletion with measured module wins.
- Reweight (2026-05-06): `_task_stop_fields(...)` now returns the private normalized stop-policy fields as a positional list instead of a temporary string-keyed map. The public policy API remains map-shaped, but `stop_policy(...)` / `stop_policy_wait(...)` no longer build then immediately re-index a private map. Focused task/task-group/generator/coroutine gates pass, and the exact profile moves `lib/std/task.oren` to about `286ms / 49 stmts`, with `stop_policy_wait` about `19ms` and `_task_stop_fields` about `14ms`; remaining stdlib parser work should shift back to larger task-group validation shape or new measured task hot rows.
- Reweight (2026-05-05): `std:task` / `std:task_group` now bypass `std:list` wrappers for simple internal `len`/`push` operations and call `oren_list_len(...)` / `oren_list_push(...)` directly. `std:task` drops its dead list import. The final exact profile moves `lib/std/task_group.oren` from `476ms / 65 stmts / parse_body_ms=299` to `454ms / 65 stmts / parse_body_ms=274`, and `lib/std/task.oren` from `310ms / 51 stmts` to `300ms / 50 stmts`; remaining parser-body work still needs larger shape deletion in `_task_group_validate_policy_shape`, `task.stop_policy_wait`, or `_task_stop_fields`.
- Reweight (2026-05-05): the remaining local predicate/tag wrappers in `std:generator`, `std:task`, and `std:task_group` have been removed where they only forwarded to primitive runtime predicates or `oren_type_tag(...)`. This deletes `_generator_is_int`, `_generator_is_string`, `_task_handle_ok`, `_task_group_is_runtime`, and `_task_group_target_ok` without adding local tag temporaries. Focused task/task-group/generator/coroutine surfaces pass, and the exact profile reduces source shape to `lib/std/generator.oren` `135` stmts, `lib/std/task.oren` `49` stmts, and `lib/std/task_group.oren` `63` stmts while keeping parser-body timings and codegen in-band (`user_decls` about `2.68s / 328 funcs`). Remaining stdlib parser work still needs larger control/data-shape deletion in the hot policy bodies, not new local wrapper layers.
- Reweight (2026-05-06): internal task/task-group hot paths now finish the direct-predicate cleanup by calling `oren_task_is_handle(...)` / `oren_task_is_done(...)` directly in `_task_timeout_detach_result(...)`, `_task_request_then_stop_handle(...)`, and `_task_group_target_kind(...)` instead of routing through exported `std:task` predicate wrappers. The public predicates remain unchanged. Focused task/task-group/generator/coroutine surfaces pass, and the exact profile keeps statement counts stable while moving `lib/std/task.oren` to about `285ms / 49 stmts`, `lib/std/task_group.oren` to about `444ms / 63 stmts`, and `user_decls` in-band at about `2.67s / 328 funcs`. Remaining stdlib parser work still needs larger control/data-shape deletion in `_task_group_validate_policy_shape`, `task.stop_policy_wait`, or `_task_stop_fields`.
- Reweight (2026-05-05): the reserve pass now uses a conservative `stmt_may_assign_name(...)` guard before running post-statement safe-int maintenance on non-top statements. This skips no-op `_opt_safe_ints_after_stmt(...)` and assigned-name collection for shapes such as `Return` and plain `ExprStmt` that cannot update tracked locals, while leaving `Var`/`Assign`/`Set` and control-flow bodies on the original path. The exact native profile moves `optimizer.summary total_ms` from about `1175ms` to `1140ms`, `reserve_ms` from `227ms` to `219ms`, and `reserve_safe_ms` from `198ms` to `190ms`; continue optimizer work only where the summary exposes an active bucket.
- Reweight (2026-05-05): residual optimizer passes now have bounded hot-body visibility. `optimizer.summary` embeds top bodies for `tco_mod`, `tco`, `for`, `sink`, and `dce` in addition to `fold`, `list_int`, and `reserve`, and `make profile-native-build-phases` prints an `optimizer pass totals` table. Hot-body ranking ignores sub-1ms bodies to avoid perturbing phase profiles with thousands of meaningless updates. Repeated profiles keep optimizer total in the recent band at about `1.18-1.25s` and show `tco` (~60ms), `for` (~30ms), and `dce` (~55ms) are distributed across functions with no >=1ms body; only one `sink` body reaches 1-2ms. Do not chase those residual buckets with fixture splitting or local temporaries; a viable follow-up needs structural pass-level overhead removal.
- Reweight (2026-05-05): the first retained residual structural cleanup now uses one direct self-call scan in the optimizer driver to skip both TCO passes when a function cannot self-recurse, and DCE now skips read collection/removal work for current blocks with no trivially-removable local `Var` after recursively processing children. The known native TCO fixtures still pass. The measured generator profile moves `tco_mod+tco` from the prior `~9ms + ~62ms` shape to `tco_scan=19ms` and zero rewrite work, drops `dce_ms` from about `55ms` to `11ms`, and moves optimizer total to about `1052ms`. Remaining residual pass work should continue at this structural pass level.
- Reweight (2026-05-05): reserve-safe maintenance now uses a precise recursive `stmt_may_assign_name(...)` helper instead of treating all control-flow statements as assignment-capable. Real `Var`/`Assign`/`Set` and nested assignments still invalidate list/safe-int facts, but assignment-free control-flow subtrees no longer pay `_opt_safe_ints_after_stmt(...)` plus assigned-name collection. The measured profile moves optimizer total to about `857ms`, `reserve_ms` to `46ms`, and `reserve_safe_ms` to `15ms`; remaining optimizer work should move to non-reserve residuals unless a new profile exposes a concrete reserve bucket.
- Reweight (2026-05-05): residual loop/list optimizer work now has another pass-level reduction instead of fixture splitting. The optimizer driver skips simple-`for` lowering unless `stmt_may_contain_for(...)` finds a real `For`, skips list-literal sinking unless the body both touches list shapes and contains an `if`, and reports the new `for_scan_ms` / `sink_scan_ms` guard costs in `optimizer.summary`. List-int touch marking also checks the shared list-touch detector before cloning safe-int state. The measured generator profile keeps optimizer total around `852ms`, drops `for_ms` to about `1ms`, `sink_ms` to about `19ms`, and moves `list_int_ms` to about `177ms` with `list_int_touch_ms` about `28ms`; remaining optimizer work should target the still-active fold/list-int scan/rewrite buckets, not reserve/TCO/DCE.
- Reweight (2026-05-05): fold block scoping now avoids dead non-nil fact frames. `fold_stmt(Block)` scans only the block's direct statements and pushes a frame when direct `Var`/`Assign` statements can update fold facts; assignment-free blocks reuse the current environment, while nested functions and branch/loop isolation keep their existing fresh environments. The measured generator profile moves optimizer total to about `760ms`, `fold_ms` to about `203ms`, and `fold_env_push` from `3204` to `1667`; remaining fold work should use similarly structural evidence rather than reopening the rejected broad leaf-route probe.
- Reweight (2026-05-05): list-int lowering now tracks whether the candidate map still has any active safe candidate instead of treating unsafe candidate entries as work. The retained hybrid guard keeps the cheap any-candidate precheck before use scanning, but skips touch marking and rewrite loops after all current candidates have been invalidated, while preserving later candidate discovery in the same block. The measured generator profile moves optimizer total to about `694ms`, `list_int_ms` to about `133ms`, `list_int_use_scan_ms` to about `36ms`, and `list_int_touch_ms` / rewrite buckets to `0ms`; the next list-int target should reduce remaining lower/use-scan structure only with proof that net wall time improves.
- Reweight (2026-05-05): fold control-body isolation now relies on `fold_stmt(Block)`'s existing local fact frame instead of cloning and pushing a separate environment for every block branch/loop body. Non-block bodies still use the old clone/push path. The measured generator profile moves optimizer total to about `514ms`, `fold_ms` to about `60ms`, `fold_env_push` to `352`, and `fold_env_clone` to `4`; remaining optimizer work should shift back to list-int lower/use-scan or non-optimizer link/codegen buckets unless a new fold profile exposes a concrete active shape.
- Reweight (2026-05-05): nested list-int lowering now uses a narrower candidate-start detector before recursing into block/control bodies instead of the shared conservative list-touch detector. It mirrors the existing lowerer surface and descends only when a nested `Var`/`Assign` can introduce a list/list-int constructor or potentially-safe list literal; use scanning and rewrite guards stay unchanged. The measured generator profile moves optimizer total to about `422ms`, `list_int_ms` to about `80ms`, and `list_int_lower_ms` from about `67ms` to `4ms`; remaining list-int work is now the smaller use-scan/candidate path and should be treated as lower leverage than parser-body or Mach-O work unless a new profile exposes a larger bucket.
- Reweight (2026-05-05): list-int use scanning now avoids unconditional safe-int map clones inside block scans. It clones only for statement/expression shapes that can mutate branch-local safe-int state during the scan, and only recomputes post-statement safe-int state for assignment-capable shapes using `stmt_may_assign_name(...)`. The measured generator profile moves optimizer total to about `404ms`, `list_int_ms` to about `68ms`, `list_int_scan_ms2` to about `45ms`, and `list_int_use_scan_ms` to about `24ms`; remaining list-int work is now small enough that parser-body deletion or the Mach-O first-touch design should outrank further local list-int probes unless a new profile exposes a larger target.
- Rejected (2026-05-05): another generator-surface fixture split separated defer-finalize, collect/for-in, declaration collect, and local-generator clusters. Direct bytecode/native fixture builds and the native build profile passed, but the profile did not improve aggregate codegen: the split only relabeled hot rows into smaller for-in/local bodies, while `user_decls` stayed roughly neutral/slightly worse (`~2.96s` prior band vs raw `~2.99s`) and optimizer total rose (`~404ms -> 456ms`) because function count increased. Source was reverted; do not continue fixture splitting unless the aggregate wall-time target improves.
- Rejected (2026-05-05): gating the remaining disabled `OREN_TRACE_LIST_INT` string construction in `optimizer_list_int.oren` passed `make stage2`, list-int parity, reserve, and trace-enabled generator-surface gates, but the added per-site trace-flag checks regressed the default profile (`reserve_safe_ms` about `201ms -> 280ms`, `user_decls` about `3.38s -> 5.01s`). Do not reopen list-int trace gating as a scatter of local guards; a viable design would need a lower-overhead trace API or broader structural deletion.
- Reweight (2026-05-05): the Mach-O local resolver path now has a retained lower-level target-cache cleanup that avoids the rejected generic Oren-map design. The local BL/ADR-code unique-target cache is kept sorted by compact metadata key and resolved by binary search plus same-key string comparison, cutting stats-enabled scan steps from about `875080` to about `107739`. The default profile trims the later local BL resolve tail (`~97ms + ~126ms` to `~28ms + ~57ms`), but the first 4096 local BL fixups still cost about `1.9s`; a scalar 4-entry MRU probe reduced scan counters but not wall time and was not retained. Next Mach-O work should target first-touch target materialization or a lower-level symbol-id representation, not generic maps, scan-order changes, or tiny MRUs.
- Rejected (2026-05-05): prebuilding the local Mach-O resolver table from all known user/import and runtime function offsets removed first-touch lookup misses in the BL loop, but the naive insertion-sorted table construction took about `96-97s` for `1891` targets. The resolved loop then fell to tens of milliseconds, proving the target but rejecting the implementation. A viable symbol-id/materialization design must build the table in linear time or carry compact ids from emission; do not ship whole-table insertion preseed.
- Reweight (2026-05-05): the stats-enabled Mach-O resolver profile now also reports top local BL/ADR-code targets. The exact final tree still spends about `1.95s` in the first `4096` local BL resolutions, and the hot target set is concrete: `oren_panic` and `oren_ensure_tracked` at `2305` refs each, `oren_call_depth_exit` at `1631`, `native_list_len_panic` at `474`, and `oren_find_node` at `438`. A follow-up direct-known-BL probe that asked `arm64_lookup_function(...)` for these names at each emit site passed `make stage2` but made both default and stats native profiles time out at `180s` before `user_decls` completed, proving that hot codegen-side map lookups are the wrong lower-level path. Next Mach-O work should carry compact symbol ids/offsets from emission or materialize direct target arrays in linear time before fixup application.
- Rejected (2026-05-05): a sparse BL sid lane tried to annotate only the measured hot local targets (`oren_panic`, `oren_ensure_tracked`, `oren_call_depth_exit`, `native_list_len_panic`, `oren_find_node`) and resolve the five ids once in the Mach-O pass. This avoided per-emit function-map lookups but still added Oren-list/fixup-shape overhead on hot codegen paths; `make stage2` passed, then the stats-enabled native profile timed out at `180s` before `arm64.codegen.user_decls.done`. Do not reopen symbol ids as extra generic fixup map/list fields. The next viable Mach-O design should carry compact target metadata in a lower-level side vector or build a direct target array in one linear post-codegen materialization pass.
- Rejected (2026-05-05): a Mach-O-loop hot-name bypass precomputed the five hot target offsets once and matched existing BL target strings before the sorted resolver. It completed and reduced stats counters (`lookups 11131 -> 3978`, `scan_steps 106353 -> 35716`, `hot_prefilled=7153`), but the wall-time boundary did not move: the exact final stats run still needed about `213ms` before `i=0` plus about `1945ms` to reach `i=4096`, effectively the same first-touch target after accounting for the precompute. Do not reopen string-name hot-target bypasses; lower-level compact target ids/offsets are still required.
- Reweight (2026-05-06): removed the now-dead `_macho_str_eq_bytes(...)` helper from `arm64_macho.oren` and the unused `_task_group_runtime_surface(...)` sentinel from `std:task_group`. The retained sorted resolver uses the known-length byte compare over same-key candidate ranges, and task-group map validation still uses `_task_group_surface(...)`; this is stale source-shape cleanup, not a fix for the first-touch BL bucket or the remaining parser-body rows. Keep the next Mach-O attempt focused on compact target-id/offset materialization below generic maps, string-name bypasses, and fixup-side sid fields.
- Reweight (2026-05-06): local Mach-O BL fixups now resolve and apply in a single pass when no valid prefilled target lane exists. This deletes the old nil-filled `bl_target` materialization and second apply loop while keeping prefilled-target compatibility and avoiding the rejected generic maps, string-name bypasses, and emit-side sid fields. The default profile moves the first local BL span to about `1.41s` from the prior `~1.9s` boundary; stats still show about `11123` BL lookups and `106069` scan steps, so remaining Mach-O work is below repeated target lookup or needs a genuinely compact target-id/offset representation.
- Reweight (2026-05-06): Mach-O stats profiles now count sorted-cache `insert_shifts` in the local target resolver, and the profile script prints those counters in a `Mach-O local resolve stats` table. The first retained stats run shows `21120` shifts by `i=4096` and `37056` shifts across the BL pass, alongside `106069` scan steps and `16163` byte compares. This confirms lazy insertion work is nontrivial but not enough alone to explain the remaining `~1.4s` first-touch bucket, so the next compact target-id/offset design should bypass both insertion maintenance and repeated string/name lookup.
- Rejected (2026-05-06): inserting a newly resolved target at the end of its same-metadata-key range instead of the front reduced same-key candidate comparisons (`16163 -> 12294`) but did not materially reduce the real target. Insert shifts stayed essentially flat (`37056 -> 36925`), and the default first-touch BL span remained about `1.40s`. Source was reverted; do not reopen local insertion-order tweaks without a different mechanism that removes tail shifts or name lookup entirely.
- Rejected (2026-05-06): reusing the existing compact `bl_target` lane as sparse `(fixup_index, target)` pairs for hot runtime BL targets avoided a new sid array shape and passed `make stage2`, but the stats-enabled native profile timed out at `180s` before `arm64.codegen.user_decls.done`. Source was reverted. Do not carry direct offsets through Oren map/list writes on hot emit paths; the remaining Mach-O work needs lower-level target metadata or one linear post-codegen direct-target materialization pass.
- Rejected (2026-05-06): pre-seeding the append-only first-byte Mach-O target buckets from `ctx["function_names"]` passed `make stage2`, but moved the default first-4096 BL span from the retained `~598ms` to about `1039ms`. The stats profile showed `422` seeded targets, about `518ms` spent before `i=0`, and higher BL scan/candidate counts (`56888` / `1430`) while only `40` targets would have been lazily appended. Keep the shipped lazy bucketed resolver; do not reopen user-function preseed unless construction cost is proven below the lazy path.
- Reweight (2026-05-06): hot runtime BL targets now use compact negative ids in the existing `fixups_local_compact["bl_name"]` lane instead of adding new emit-side metadata. Legacy fixup maps still keep strings, and the Mach-O pass precomputes six runtime offsets once before resolving ids. The stats profile shows `hot_id=7531`, BL string lookups `11123 -> 3592`, scan steps `106069 -> 32627`, and byte compares `16163 -> 4077`; the default first `4096` BL span moves to about `1.29s` from the recent `~1.40s` band. Insert shifts remain high at about `36598`, so the next Mach-O target is insertion-free/direct-array materialization for remaining names, not another hot emit-path side lane.
- Reweight (2026-05-06): extended the same compact BL id lane to the next measured runtime target set instead of adding a new side lane. The compact `bl_name` slot now carries ids for `oren_is_err`, `oren_map_get_str`, `oren_new_list`, `oren_list_get`, `oren_list_push_unchecked`, `_list_find_node_known_kind`, `oren_func`, `oren_list_push`, `oren_add`, and `oren_map_get_int` while legacy fixup maps still carry strings. The stats profile moves `hot_id` to `9412`, string lookups to `1711`, scan steps to `14478`, and byte compares to `1565`; the default first `4096` BL span moves to about `1.05s`. Remaining Mach-O work should target direct target arrays/insertion-free materialization for the long tail, not Oren-level maps, side lists, or another broad emit-path metadata lane.
- Reweight (2026-05-06): after compact ids cut the hot set, the Mach-O local target cache now appends newly resolved names instead of maintaining sorted insertion order. This deletes lazy tail-shift maintenance (`insert_shifts=0`) and keeps byte compares low via metadata-key filtering, while accepting more cheap raw scan steps (`14478 -> 268198`) over the now-small long-tail table. Default first-4096 BL timing stays slightly better at about `1.03s`; next work should move from cache-shape tweaks to scan-free direct target arrays/materialization for the remaining long tail.
- Reweight (2026-05-06): the existing compact Mach-O BL id lane now covers the measured long-tail leaders (`STD_generator_next`, `STD_generator_start`, `strcmp`, `STD_generator_send`, `STD_generator_is_done`) without adding new fixup-side lists/maps. Stats move `hot_id` to `9727`, string lookups to `1396`, scan steps to `205608`, and byte compares to `1381`; the default first-4096 BL span remains slightly better/in-band at about `1.02s`. Treat further name-by-name ids as diminishing returns unless a profile exposes a larger clustered target; the next real design target is scan-free direct target arrays/materialization.
- Reweight (2026-05-06): a final compact-id batch covers the next residual Mach-O BL leaders (`oren_map_set_str`, `oren_type_name`, `STD_generator_cancel_reason`, `_oren_generator_context_exchange`, `STD_generator_is_cancel_requested`) plus direct `oren_map_set_str` emit sites. Stats move `hot_id` to `9923`, string lookups to `1200`, scan steps to `177442`, and byte compares to `1122`; the default first-4096 BL span moves to about `0.97s`. The current top residual names are now sub-32-reference scattered targets, so do not keep extending ids one name at a time unless a future profile reveals a larger cluster.
- Reweight (2026-05-06): compact Mach-O BL ids now dispatch through a precomputed target-offset array instead of a long branch ladder in the hot local BL loop. Counters stay the same (`hot_id=9923`, lookups `1200`), but the default first-4096 BL span nudges from about `0.97s` to about `0.96s` by reducing per-id dispatch overhead. Treat local id-dispatch cleanup as exhausted; remaining Mach-O leverage requires a scan-free direct target-array/materialization design for the string long tail.
- Reweight (2026-05-06): the remaining append-only Mach-O target cache now keeps fixed first-byte buckets. String BL/ADR-code resolution still uses metadata-key filtering and byte equality, but it scans only the matching byte bucket instead of every appended local target. This keeps the design below rejected generic maps/sorting/sid lanes/future rewrites, moves BL scan steps from `177442` to `52381`, and moves the default first-4096 BL span to about `0.60s`. Further Mach-O work should require a linear direct-target-array construction path; more one-off name ids are still low leverage.
- Rejected (2026-05-06): grouping each first-byte Mach-O resolver bucket by metadata key reduced stats scan steps (`52381 -> 39725`) but regressed the default first-4096 BL span from the retained `~0.60s` band to about `1.01s`. The nested Oren list shape is more expensive than the flat bucket despite lower counters. Keep the retained flat first-byte buckets unless a future design is truly lower-level or linear-direct-target materialization.
- Cleanup (2026-05-06): split the Mach-O resolver decode/lookup/stat helpers into `arm64_macho_resolve.oren`. This is source-shape only: retained first-byte bucket counters stay aligned (`BL lookups=1193`, `scan_steps=52381`, `insert_shifts=0`), while `arm64_macho.oren` drops from `1921` to `1694` lines and the new resolver module is `227` lines.
- Cleanup (2026-05-06): split `native_expr_chunk_noncall(...)` out of `arm64_native_expr/010_lowering_a.oren` into the new include `arm64_native_expr/015_lowering_a_noncall.oren`. The facade include order preserves the original concatenated source, and a concatenation check confirmed the source-order split before whitespace normalization of the moved block. This moves the near-limit lowering file from `1938` lines to `470` plus a `1468`-line non-call module; stage2, generator/coroutine surfaces, native profiling, and full `make test` pass.
- Cleanup (2026-05-06): split `native_call_chunk_030_lowering_c(...)` out of `arm64_native_expr/030_lowering_c.oren` into `arm64_native_expr/035_lowering_c_calls.oren`. The facade include keeps the original order between `030` helpers and `040` call lowering, and concatenation proved the source-order split before moved-block whitespace normalization. This drops `030_lowering_c.oren` from `1904` lines to `59`, with the extracted call module at `1844` lines; stage2, generator/coroutine surfaces, native profiling, and full `make test` pass.
- Cleanup (2026-05-06): split the extracted ARM64 `035` call dispatcher into a small ordered wrapper plus `native_call_chunk_035_list_ops(...)` and `native_call_chunk_036_list_simd_ops(...)`. The split keeps whole `fn_name` blocks intact, moves reduce/dot/call-object/SIMD/GEMM cases into `036`, and leaves list/list-int len/get/set/push cases in `035`. Both resulting files are comfortably below the guardrail (`927` and `925` lines), and stage2, generator/coroutine surfaces, native profiling, and full `make test` pass.
- Cleanup (2026-05-06): split the C backend transpiler entry/part-collection tail into `transpiler/090_entry.oren` using textual include order. Concatenating the facade prefix with the extracted entry module matches the original `transpiler.oren`, moving the facade away from the `1999`-line guardrail edge to `1725` lines.
- Cleanup (2026-05-06): split two more near-limit source-order clusters with textual include expansion: green runtime scheduler steal/pop-any helpers now live in `runtime_native/263_green/010_green_core_steal.oren`, and parser impl-declaration lowering now lives in `parser_parse/000_prelude_impl.oren`. Expanding each include back into its parent file matches the original source order, moving `010_green_core.oren` to `1880` lines and `parser_parse/000_prelude.oren` to `1742`.
- Cleanup (2026-05-06): split another guardrail batch at whole-function boundaries. Windows x64 NET IOCP intrinsics now live in `x64_native_program/046_emit_sys_intrinsics_windows_net_iocp.oren`, x64 while-loop dot fast paths now live in `x64_native_program/057_emit_ops_while_dot.oren`, and runtime task-group helpers now live in `runtime_native/260_threads_task_group.oren`. Source-order expansion checks match the originals, moving the parent files to `1095`, `1318`, and `1399` lines.
- Cleanup (2026-05-06): split the packed-SIMD and late memory-helper encoder tail out of `x64_core.oren` into `x64_core_simd_tail.oren`. Expanding the include matches the original source order; `x64_core.oren` now sits at `1662` lines with scalar/core encoders retained in the parent.
- Cleanup (2026-05-06): split four more near-limit source-order clusters: green scheduler netpoll/runq/sleep/zombie/switch helpers into `010_green_core_sched.oren`, parser generator-lowering helpers into `000_prelude_generator.oren`, list-int lowering/touch/rewrite helpers into `optimizer_list_int_lower.oren`, and list-loop reset/reserve helpers into `optimizer_loops_list_reset.oren`. Expansion checks match the original parents, dropping the parent files to `1252`, `896`, `927`, and `864` lines.
- Cleanup (2026-05-06): split five more backend/runtime source-order clusters with expansion checks: ARM64 statement inner dispatch into `arm64_native_stmt_inner.oren`, x64 integer expression evaluation into `040_emit_expr_eval.oren`, C transpiler fast-while emitters into `transpiler/050_fast_while.oren`, bytecode list-loop specializations into `030_tail_loops.oren`, and runtime crash-footer/GC-mode init into `100_time_core_crash_gc.oren`. Parent files now sit at `529`, `293`, `802`, `655`, and `911` lines.
- Reweight (2026-05-06): `_generator_stop_policy_fields(...)` now mirrors the retained task stop-field carrier and returns `[mode, delay_ms, grace_ms, reason, join_timeout_ms]` instead of a temporary string-keyed private map. Deadline-based policy input is normalized to `delay_ms` once, so `stop_policy(...)` / `stop_policy_wait(...)` dispatch through the existing after/wait paths without re-indexing a private map. The exact profile moves `lib/std/generator.oren` to about `302ms / 135 stmts` from the prior `~327ms` band, and `STD_generator_stop_policy` drops out of the top user-decls rows (`~140ms` to about `46ms`). Remaining generator stdlib work should target larger helper-call/codegen shape rather than adding private carriers or helper splits.
- Rejected (2026-05-06): unifying generator deadline `*_at` wrappers by validating the deadline once and routing through the existing `*_after` wrappers passed `make stage2` and focused generator/coroutine surfaces, but only moved local source shape while worsening the whole profile: generator parse improved narrowly (`302ms -> 297ms`), but optimizer moved `~400ms -> 459ms`, `user_decls` moved `~2532ms -> 2695ms`, and the first BL span moved `~965ms -> 1040ms`. Source was reverted; do not re-open public deadline-wrapper unification unless it removes runtime/helper calls rather than adding another facade hop.
- Rejected (2026-05-06): a post-codegen BL name materialization probe rewrote future identical compact `bl_name` entries to the first resolved positive target offset, avoiding new emit-side metadata but scanning the BL fixup list once per newly resolved string name. `make stage2` passed, but the stats profile timed out at `180s` before the first `i=4096` local BL progress line after reaching `i=0`; the O(unique-names * fixups) rewrite is too expensive. Do not repeat future-name rewrites over the full BL list; direct target arrays need a linear construction path.
- Rejected (2026-05-06): the adjacent local ADR-code single-pass probe removed its nil-filled target lane and second apply loop too, but measurement did not justify retaining it. `make stage2`, default profile, and stats profile passed, yet the small ADR-code bucket stayed in the same noisy `~70-95ms` range and the phase ordering became less comparable. Source was reverted; do not spend more turns on ADR-code fixup shape unless a future profile makes it a large target.
- Rejected (2026-05-06): a one-entry previous-target reuse in the local BL resolver found `1055` adjacent same-name BL fixups, but the wall-time boundary did not move materially (`~1.42s` baseline vs `~1.41s` probe for the first 4096 local BL fixups). Source was reverted. Do not spend more turns on small MRU/previous-target caches; the remaining Mach-O work needs compact target ids/offsets or linear post-codegen materialization below the existing string/name path.
- Rejected (2026-05-06): an Oren-level whole-table materialization probe rebuilt local target entries from `ctx["functions"]` plus `runtime_functions_enc` and shell-sorted them by the existing metadata key. It proved direct target hits would make the BL loop cheap (`appended=0`, roughly `100ms` across all local BL lookups), but the preseed/sort step itself cost about `71.7s` for `1889` entries before `i=0`. Do not reopen whole-table target sorting in Oren; the remaining Mach-O path needs target ids/offsets carried from lower-level codegen metadata or a direct target-array materialization that avoids generic maps, string-name bypasses, fixup-side sid fields, and Oren-level global table sorting.
...
- Cleanup (2026-05-06): split the split-invariant list-push optimizer tail into `optimizer_loops_split.oren` and kept `optimizer_loops.oren` as the facade. This drops `optimizer_loops_list.oren` from the near-limit `1988`-line band to `1759` lines while preserving the same env-gated pass; focused optimizer/generator/coroutine gates and full `make test` pass.
...
- Cleanup (2026-05-06): removed four repo-proven stale x64 private helpers (`_elf_hash_sysv_u32`, `_x64_env_enabled_default_true`, `_emit_write_newline`, `_emit_parse_u64_decimal_from_cstr_ptr_x64`) after confirming no repo callers. This keeps the x64 module split lean without changing active codegen paths.
- Cleanup (2026-05-06): removed another repo-proven stale private-helper batch across compiler and stdlib modules: `_arm64_cmp_may_use_strcmp`, `_attr_list`, `_arm64_rtobj_encode_fixups`, `_astbin_g_read_string`, `_astbin_read_string_ref_v2`, `_is_meta`, `_u32be_set`, `_table_len`, `_store_u8_buf_direct`, `_load_u8_buf_direct`, `_pack_list_like_with`, `_new_i32_buf`, `_new_i64_buf`, `_new_f32_buf`, and `_new_f64_buf`. The static token scan and repo `rg` checks showed each name appeared only at its definition before removal, so this is source-shape reduction rather than a semantic rewrite.
- Cleanup (2026-05-06): removed three unused ARM64 statement imports from non-prelude files after excluding textual-include preludes and the imports-only AVM stdlib bundle from the alias scan. `arm64_native_stmt.oren` no longer imports unused `abi` / `labi`, and `arm64_native_stmt_set.oren` no longer imports unused `loopm`.
- Rejected (2026-05-06): a runtime-native dead-helper deletion probe removed private helpers that appeared only at their definitions in source (`_green_p_runq_cap`, green tick/word-wait accessors, runq/sleep/wake wrappers, `_darwin_thr_bytes`, `_linux_thr_bytes`, and `_netpoll_iocp_post_recvfrom`). `make stage2` initially passed, but task-surface builds immediately segfaulted; after a cold runtime-seed rebuild, the remaining IOCP wrapper deletion reproduced the stage2 seed segfault. Source was reverted. Do not treat runtime-native single-occurrence private helpers as safe dead code without stage2 plus fixture-build proof; runtime bundle/rooting can depend on non-textual reachability.
														      ...
														      - Follow-up (2026-05-04): the local Mach-O resolver now stores unique-target
																        ...
																        equality and avoids the rejected cache families, but the generator
																	      ...
																	      - Dead-vector cleanup (2026-05-05): the combined metadata-key resolver no
																	        ...
																	        reduce target-cache entries visited per lookup.
																		      ...
																		      - Rejected newest-first resolver scan (2026-05-05): reversing the
																		        local target-cache scan order from oldest-first to newest-first made
																		        `make stage2` exceed the recent stage2 band with no additional
																		        progress past self-hosted stage2 build startup, so it was terminated
																		        and reverted. Do not rely on assumed BL temporal locality here.
														        ...
														        ~3.31s, `list_int` down from ~1.81s to ~1.46s, and reserve down from ~0.64s
															        to ~0.53s. A narrower list-int-only prefilter probe reduced candidates
																        ...
																        source probe was reverted. A fold-pass stable-list-length hoist also
																        passed stage2 and focused optimizer/generator gates, but repeated
															      ...
															      - Optimizer fold follow-up (2026-05-05): `optimizer.summary` now carries
															        ...
															        fold cleanup hoists stable list lengths in fold-local loops, fast-returns
																	        leaf expressions, skips recursive folding for leaf children, skips child loops
																	        for calls whose callee/args/spread are all leaves, avoids
																	        clone-of-clone environments in isolated `if`/`for` folding, avoids a
																	        redundant empty environment push for function-expression bodies, and gates
																	        fold trace string construction behind the already-computed `OREN_TRACE_OPTIMIZER`
																	        flag. The measured profile moves optimizer total to ~1.35s, `fold_ms` to ~252ms,
															        `fold_env_clone` from `2592` to `1317`, and `fold_env_push` from `3754`
															        to `3192`; remaining fold work is now evidenced by `8398` non-leaf folded
															        expressions, led by `2644` calls, `2489` infix nodes, `1271` expression
															        `if` nodes, and only `22` other nodes. A broader leaf-only prefix/index/member/infix
															        route probe was reverted after its final exact-tree profile regressed to
															        `fold_ms=451ms` and optimizer total `1.58s`.
														        ...
														        `rtobj.miss.build.start` despite compatible schema-3 cache artifacts. Isolated
															        ...
															        compiler binary emitted the runtime object.
																	      - Module parse profile follow-up (2026-05-05): serial module parsing now emits
																	        ...
																	        lexer/parser/merge/prepare plus parser-body and generator-core sub-buckets.
																		        The parser now caches the generated generator-core source string and a parsed
																		        generator-core statement template on the normal non-forced-GC compiler path,
																		        cloning the cached AST before prepending it; both caches are disabled for
																		        `OREN_COMPILER_GC=1|true` because compiler globals are not yet guaranteed
																		        native GC roots under forced compiler GC. The refreshed profile moves
																		        `lib/std/generator.oren` from ~1.40s with `gen_core_ms=1112` to ~317-330ms
																		        with `gen_core_ms=25-26` / `gen_core_parser_ms=0`; the prior source-string-only
																		        boundary was ~481ms with `gen_core_ms=188`. The new hot-statement
																	        rows show task/task-group cost is top-level function parsing:
																	        `task_group.stop_policy_wait`, `task_group.terminal_results`,
																	        `_task_group_validate_policy_shape`, `task.stop_policy_wait`,
																	        `_task_stop_fields`, and `task.cancel_after_wait`. Reweight module-parse
																		        work toward those parser-body shapes or a deeper generator-core
																		        representation only if it beats the cloned-AST cache without importing
																		        ASTBIN codec codegen, instead of file-read/cache/prepare work or the small tail. An
																		        additional primitive-tag cleanup removes the remaining local task/task-group
																		        int/string wrappers; the refreshed profile puts `lib/std/task_group.oren`
																		        around ~444ms / 64 stmts and `lib/std/task.oren` around ~306ms / 51 stmts.
																			        An opt-in serial module-cache write probe was rejected after writing only the
																	        first two ASTBIN cache entries pushed the profile to the 180s timeout.
																	        A parser-side generator-core ASTBIN template-cache probe was also rejected:
																	        it reduced visible `gen_core_ms` to ~41ms but made the native profile time
																	        out later in emit/codegen.
