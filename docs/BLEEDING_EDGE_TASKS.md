...
2. **Native performance parity**
   ...
   - Current module-linking prelude parse/import scans and tail diagnostic
     aggregation cache immutable list counts before traversal, with source
     guards rejecting direct `oren_list_len` loop bounds in those link stages.
