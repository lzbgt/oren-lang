...
2. **Native performance parity**
   ...
   - Current compiler loop-list analysis, reset, and list-int lowering optimizer
     traversals cache immutable AST list counts before continue/exit,
     side-effect, new-list, identifier, unsafe-use, sink/rewrite, hoist/reset,
     reserve, candidate, touch, and rewrite scans, with source guards rejecting
