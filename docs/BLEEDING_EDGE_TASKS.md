...
- Reweight (2026-05-06): Mach-O stats profiles now count sorted-cache `insert_shifts` in the local target resolver. The first retained stats run shows `21120` shifts by `i=4096` and `37056` shifts across the BL pass, alongside `106069` scan steps and `16163` byte compares. This confirms lazy insertion work is nontrivial but not enough alone to explain the remaining `~1.4s` first-touch bucket, so the next compact target-id/offset design should bypass both insertion maintenance and repeated string/name lookup.
