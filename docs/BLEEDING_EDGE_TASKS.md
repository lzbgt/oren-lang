...
1. **AVM iOS embeddability and compiler-in-AVM release gate**
   ...
   - Follow-up distribution design: after the GUI bridge gate, publish a curated
     ...
     Oren source in-browser with syntax highlighting and an AST outline.
	     ...
	     glTF 2.0 JSON and GLB source lowering with URI/BIN buffers, sparse accessors, static `POSITION` and `COLOR_0` morph target weights, material colors multiplied by `COLOR_0`, triangle/strip/fan topology, and explicit node or scene TRS/matrix hierarchy selection,
	     ...
	     core 3MF ZIP mesh/build plus basematerial `displaycolor` lowering, plus
